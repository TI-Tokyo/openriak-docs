...
sidebar_label: Get Bucket Properties
pagination_label: "PBC Get Bucket Properties"
...
The bucket's name (`bucket`) must be specified. The [bucket type](./../../../../using/cluster-operations/bucket-types) parameter (`type`) is optional. If it is not specified,
...
[configuration file](./../../../../configuring/reference/#default-bucket-properties) documentation.
...
<RiakDocsNote title="Note on `RpbReplMode`">
The `RpbReplMode` is of use only to users of Riak CS's [Multi-Datacenter Replication capabilities](/cs/2.1.1/cookbooks/multi-datacenter-overview/)
</RiakDocsNote>
