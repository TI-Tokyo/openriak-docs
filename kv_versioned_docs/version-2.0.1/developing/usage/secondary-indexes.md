...
sidebar_label: Using Secondary Indexes
pagination_label: "Using Secondary Indexes (2i)"
...
recommend [Riak Search](./../search) rather than secondary indexes for
...
be used to find multiple objects in Riak. If you're storing [user data](./../../data-modeling/#user-accounts), for example, you could tag each object
...
[Riak Search](./../search) serves analogous purposes but is quite
...
* Query results can be used as input to a [MapReduce](./../mapreduce)
  query
...
[strongly consistent buckets](./../../app-guide/strong-consistency/#creating-a-strongly-consistent-bucket-type) and attach
...
mechanism that does not require a schema (as does [Riak Search](./../../../using/reference/search/#schemas)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search](./../search).
...
obj.getIndexes().getIndex(StringBinIndex.named("email")).add("jsmith@a-riak-user.com");
...
obj.indexes['email_bin'] = %w{ jsmith@a-riak-user.com }
obj.store
...
$object = (new \Basho\Riak\Object('{"user_data":{ ... }}', ['Content-type' => 'application/json']))
  ...
  ->addValueToIndex('email_bin', 'jsmith@a-riak-user.com');
...
obj.add_index('email_bin', 'jsmith@a-riak-user.com')
obj.store()
```
...
obj.BinIndex("email").Set"jsmith@a-riak-user.com");
...
riakObj.addToIndex('email_bin', 'jsmith@a-riak-user.com');
...
MD2 = riakc_obj:set_secondary_index(
    ...
    [{{binary_index, "twitter"}, [<<"jsmith123">>]},
     {{binary_index, "email"}, [<<"jsmith@a-riak-user.com">>]}]),
...
obj.AddToIndex("email_bin", "jsmith@a-riak-user.com")
...
curl -XPOST localhost:8098/types/default/buckets/users/keys/john_smith \
  ...
  -H 'x-riak-index-email_bin: jsmith@a-riak-user.com' \
...
>
> If you are connecting to Riak using one of Basho's official [client libraries](./../../client-libraries), you can find more information about getting started with
your client in the [Developing with Riak KV: Getting Started](./../../getting-started) section.
...
>
> For all 2i queries, the [R](./../../app-guide/replication-properties#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs](./../../../learn/glossary/#hinted-handoff) and related operations are underway may not
