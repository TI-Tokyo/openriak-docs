...
pagination_label: "Conflict Resolution"
...
[usage bucket types]: ./../bucket-types
[use ref strong consistency]: //2.0.1/using/reference/strong-consistency

One of Riak's [central goals](../../../learn/why-riak-kv) is high availability. It was built as a [clustered](./../../../learn/concepts/clusters) system in which any [node](./../../../learn/glossary/#node) is capable of receiving requests without requiring that
every node participate in each request.

If you are using Riak in an [eventually consistent](./../../../learn/concepts/eventual-consistency) way, conflicts between object values on different nodes is
unavoidable. Often, Riak can resolve these conflicts on its own
internally if you use causal context, i.e. [vector clocks](./../../../learn/concepts/causal-context#vector-clocks) or [dotted version vectors](./../../../learn/concepts/causal-context#dotted-version-vectors), when updating objects. Instructions on this can be found in the section [below](#siblings).

<RiakDocsNote title="Important note on terminology">
...
>
> * [Using Strong Consistency](./../../app-guide/strong-consistency) --- A guide for developers
> * [Managing Strong Consistency](./../../../configuring/strong-consistency) --- A guide for operators
...
While Riak _does_ have a set of [defaults](./../../app-guide/replication-properties#available-parameters), there are a variety of general
...
[using bucket types][usage bucket types]. The most important [bucket properties](./../../../learn/concepts/buckets)
...
`[last_write_wins](./../../../learn/concepts/buckets)`. If `last_write_wins` is set to `false`,
...
<RiakDocsNote title="Undefined behavior warning">
...
for any [bucket types](./../bucket-types) that you create. This means
that if you wish to avoid client-side sibling resolution, you have a few
options:

* Explicitly create and activate [bucket types](./../bucket-types)
  that set `allow_mult` to `false`
* Use Riak's [Configuration Files](./../../../configuring/reference) to change the [default bucket properties](./../../../configuring/reference#default-bucket-properties) for your
...
on what value you set for `dvv_enabled`. If set to `true`, [dotted version vectors](./../../../learn/concepts/causal-context#dotted-version-vectors) will be used; if set to `false` (the default), [vector clocks](./../../../learn/concepts/causal-context#vector-clocks) will be used.
...
[sibling explosion](./../../../using/performance/latency-reduction#siblings), which can
...
Basho's official [client libraries](./../../client-libraries). Examples can be found for each
client library in the [Object Updates](./../updating-objects) document.
...
[causal context](./../../../learn/concepts/causal-context). This is a less likely scenario if a client updates
...
[client libraries](./../../client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started](./../../getting-started) section.
...
<RiakDocsNote title="Concurrent conflict resolution">
...
the entire node. Other issues include [increased cluster latency](./../../../using/performance/latency-reduction) as the object is replicated and out-of-memory errors.
...
<RiakDocsNote title="Note on combining `allow_mult` and `last_write_wins`">
...
![Vclock Pruning](/riakdocs-resources/images/vclock-pruning.png)
