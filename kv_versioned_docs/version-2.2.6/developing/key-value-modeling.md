...
sidebar_label: Key/Value Modeling
pagination_label: "Riak KV Key/Value Modeling"
...
that can be useful in application development, such as [Search](./../usage/search), [secondary indexes (2i)](./../usage/secondary-indexes), and [Riak Data Types](./../data-types), Riak almost always performs best when you
...
for each of them or consulting our guide to [building applications with Riak](./../app-guide) for a better sense of which features you might need.
...
* The object's [key](./../../learn/concepts/keys-and-objects#keys), which can be anything you
  want as long as it is [Unicode compliant](http://www.unicode.org/)
* The [bucket](./../../learn/concepts/buckets) which houses the object and its key (bucket
  names are also Unicode compliant)
* The [bucket type](./../usage/bucket-types) that determines the bucket's
  [replication](./../app-guide/replication-properties) and other properties
...
look like (for the [HTTP API](./../api/http)):
...
One way to determine this is to [list all keys](./../api/protocol-buffers/list-keys) in the
...
A better possibility is to use [Riak sets](./../data-types/#sets) to
store lists of keys in a bucket. Riak sets are a [Riak Data Type](./../data-types) that enable you to store lists of binaries or strings in Riak.
...
`usernames`. The following will also assume that we've [set up a bucket type](./../data-types/#setting-up-buckets-to-use-riak-data-types) called
`sets`.
...
>
> If you are connecting to Riak using one of Basho's official [client libraries](./../client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started](./../getting-started).
...
Riak [bucket types](./../usage/bucket-types) have two essential functions:
they enable you to manage [bucket configurations](./../../learn/concepts/buckets) in an
...
that simply extend Riak's [defaults](./../usage/bucket-types/#bucket-types-as-namespaces) or multiple bucket types that have
