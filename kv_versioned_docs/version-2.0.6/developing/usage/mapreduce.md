...
sidebar_label: Using MapReduce
pagination_label: "Using MapReduce"
...
<RiakDocsNote title="Use MapReduce sparingly">
...
on key querying, alongside [secondary indexes](./../secondary-indexes)
and [Search](./../search). MapReduce jobs can be submitted through the
[HTTP API](./../../api/http) or the [Protocol Buffers API](./../../api/protocol-buffers), although we
...
* When you know the set of objects over which you want to MapReduce
  (i.e. the locations of the objects, as specified by [bucket type](./../bucket-types), bucket, and key)
* When you want to return actual objects or pieces of objects and not
  just the keys. [Search](./../search) and [secondary indexes](./../secondary-indexes) are other means of returning objects based on
...
[bucket type](./../bucket-types), bucket, and key. The elements of the
...
objects stored in those keys and instruct the proper [vnode](./../../../learn/glossary/#vnode) to
...
![MapReduce Diagram](/riakdocs-resources/images/MapReduce-diagram.png)
...
`training` (which does not bear a [bucket type](./../bucket-types)).
...
in conjunction with Riak's [HTTP API](./../../api/http) to store the objects:
...
checking out our [Advanced MapReduce](./../../app-guide/advanced-mapreduce) guide.
