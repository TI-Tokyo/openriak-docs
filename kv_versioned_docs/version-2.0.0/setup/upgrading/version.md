...
pagination_label: "Upgrading to Riak KV 2.0"
...
included in version 2.0, check out our guide to [Riak 2.0](./../../../introduction).
...
those older clients will not able to take advantage of [new features](./../../../introduction) like [data types](./../../../developing/data-types) or the new [Riak Search](./../../../using/reference/search).

## Bucket Types

In versions of Riak prior to 2.0, the location of objects was
determined by objects' [bucket](./../../../learn/concepts/buckets) and [key](./../../../learn/concepts/keys-and-objects), while all bucket-level configurations were managed by setting [bucket properties](./../../../developing/usage/bucket-types).

In Riak 2.0, [bucket types](./../../../using/cluster-operations/bucket-types) are both an additional namespace for locating objects _and_ a new way of configuring bucket properties in a systematic fashion. More comprehensive details on usage can be found in the documentation on [using bucket types](./../../../using/reference/bucket-types).
...
A full tutorial can be found in [Using Bucket Types](./../../../using/reference/bucket-types).
...
* Accept Riak's [default bucket configurations](./../../../using/reference/bucket-types/#buckets-as-namespaces)
* Change Riak's defaults using your [configuration files](./../../../configuring/reference/#default-bucket-properties)
...
* [Strong consistency](./../../../using/reference/strong-consistency) --- Using Riak's strong consistency subsystem
  requires you to set the `consistent` parameter on a bucket type to
  `true`
* [Riak Data Types](./../../../developing/data-types) --- In order to use Riak Data
  Types, you must [create bucket types](./../../../developing/data-types/#setting-up-buckets-to-use-riak-data-types) specific to the
  Data Type you are using

#### Bucket types and downgrades

If you decide to use bucket types, please remember that you
cannot [downgrade](./../../downgrade) your cluster to a version of
...
[siblings](./../../../learn/concepts/causal-context/#siblings) behavior.
...
object replica [conflicts](./../../../developing/usage/conflict-resolution) between nodes on its
...
This means that the default when [using bucket types](./../../../using/reference/bucket-types) is to handle [conflict resolution](./../../../developing/usage/conflict-resolution) on the client side using
either traditional [vector clocks](./../../../learn/concepts/causal-context/#vector-clocks) or the newer [dotted version vectors](./../../../learn/concepts/causal-context/#dotted-version-vector).
...
on [conflict resolution](./../../../developing/usage/conflict-resolution).

## Enabling Security

The [authentication and authorization](./../../../using/security/basics) mechanisms included with Riak 2.0 should only be turned
...
* Index data to be used in conjunction with the new [Riak Search](./../../../using/reference/search).
* Create _and_ activate one or more [bucket types](./../../../using/reference/bucket-types). By extension, you will not be able to downgrade your cluster if you have used the following features, both of which rely on bucket types:
    - [Strong consistency](./../../../using/reference/strong-consistency)
    - [Riak Data Types](./../../../developing/data-types)

If you use other new features, such as [Riak Security](./../../../using/security/basics) or the new [configuration files](./../../../configuring/reference), you can still
...
[Configuration Files](./../../../configuring/reference).
...
[configuration files](./../../../configuring/reference) documentation, as many configuration parameters have changed names, some no longer exist, and others have been added that were not previously available.
...
For a listing of those parameters, see our documentation on [advanced configuration](./../../../configuring/reference/#advanced-configuration).
...
4. Run [`riak-admin reip <old_nodename> <new_nodename>`](./../../../using/admin/riak-admin/#reip) on each node in your cluster, for each node in your
