...
sidebar_label: Data Type Map Store
pagination_label: "PBC Data Type Map Store"
...
The `MapField` parameter is explained above. The operations used to update fields depend on the Data Type in that field, i.e. `CounterOp` messages to update counters, `SetOp` messages to update sets, etc. Updating counters is covered in [PBC Data Type Counter Store](./../dt-counter-store) while updating sets is covered in [PBC Data Type Set Store](./../dt-set-store).
