...
For reasons explained in the [Introduction to conflict resolution](./..), we strongly recommend adopting a
...
application will be stored in buckets that bear the [bucket type](./../../bucket-types) `siblings`, and for this bucket type `allow_mult` is set
...
the official Ruby client library, in the [Developing with Riak KV: Usage](./../..) section.
...
resolution is that Riak offers a variety of [Data Types](./../../../data-types) that have
specific conflict resolution mechanics built in. If you have data that
can be modeled as a [counter](./../../../data-types/#counters), [set](./../../../data-types/#sets), or [map](./../../../data-types/#maps), then you should seriously
...
resolution that we implemented above, see our [tutorial on Riak sets](./../../../data-types/#sets).
