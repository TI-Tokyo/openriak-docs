...
sidebar_label: Managing Security Sources
pagination_label: "Managing Security Sources"
...
be best to start with our general guide to [authentication and authorization](./../basics).
...
[created](./../basics/#user-management) and that
security has been [enabled](./../basics/#the-basics).

<RiakDocsNote title="Note on SSL connections">
...
interface, and can be used only through the [protocol buffers interface](./../../../developing/api/protocol-buffers).
...
a different directory in your [`riak.conf`](./../../../configuring/reference) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`](./../../../configuring/reference) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your Riak cluster, you need to perform a [rolling restart](./../../repair-recovery/rolling-restart). Once that process is complete, you can use the client
