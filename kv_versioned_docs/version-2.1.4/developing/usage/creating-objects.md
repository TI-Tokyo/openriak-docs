...
which bears the `animals` [bucket type](./../bucket-types).
...
Now, run the same read operation in [Reading Objects](./../reading-objects). If the write operation was successful, you should be able to successfully read the object. Your Riak cluster is no
longer empty!
