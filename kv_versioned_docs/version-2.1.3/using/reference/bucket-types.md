...
sidebar_label: Bucket Types
pagination_label: "Bucket Types Reference"
...
older configuration system based on [bucket properties](./../../../developing/usage/bucket-types/#bucket-properties-and-operations).

<RiakDocsNote title="Important note on cluster downgrades">
...
[HTTP](./../../../developing/api/http/set-bucket-props) or [Protocol Buffers](./../../../developing/api/protocol-buffers/set-bucket-props). With this approach, you can take any given bucket and
...
* Nearly all bucket properties can be updated using bucket types, except the
  ...
  [Riak data types](./../../../developing/data-types), [strong consistency](./../../../developing/app-guide/strong-consistency), and
  [write-once buckets](./../../../developing/app-guide/write-once) respectively
...
way that they are configured when using [bucket properties](./../../../developing/usage/bucket-types/#bucket-properties-and-operations). You cannot simply take a
...
* Some special usecases -- [strong consistency](./../../../configuring/strong-consistency),
  [data types](./../../../developing/data-types), and [write-once buckets](./../../../developing/app-guide/write-once) -- are only
...
>
> If you are connecting to Riak using one of Basho's official [client libraries](./../../../developing/client-libraries), you can find more information about getting started with your client in our [Developing with Riak KV: Getting Started](./../../../developing/getting-started) section.
...
<RiakDocsNote title="Immutable Configurations">
...
<RiakDocsNote title="Note on object location">
...
Prior to Riak 2.0, Riak created [siblings](./../../../learn/concepts/causal-context/#siblings) in the case of conflicting updates only when explicitly instructed to do so, i.e. when `allow_mult` is to `true`. The default `allow_mult` setting was `false`.
...
do so. New applications, however, are encouraged to retain and [resolve siblings](./../../../developing/usage/conflict-resolution) with the appropriate application-side business logic.
...
`user_account_bucket` with a [pre-commit hook](./../../../developing/usage/commit-hooks/#pre-commit-hooks) called `syntax_check` and two [post-commit
hooks](./../../../developing/usage/commit-hooks/#Post-Commit-Hooks) called `welcome_email` and `update_registry`. This would involve four steps:
...
because buckets act as a [separate namespace](#buckets-as-namespaces) in Riak, in addition to [buckets](./../../../learn/concepts/buckets) and [keys](./../../../learn/concepts/keys-and-objects).
