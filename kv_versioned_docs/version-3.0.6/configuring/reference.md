...
as <a href="/kv/3.0.6/learn/glossary/#active-anti-entropy-aae">active anti-entropy</a> data, and cluster metadata.
</td>
<td>
<code>./data</code>
</td>
...
<td>
A process will become busy when its heap exceeds this size
(in bytes).
</td>
<td>
<code>160444000</code>
...
<td>
A directory containing the Javascript source files which will be
...
Sets the throttling tiers for Active Anti-Entropy. Each tier is a minimum 
vnode mailbox size and a time-delay that the throttle should observe 
at that size and above. For example: 
<pre>anti_entropy.throttle.tier1.mailbox_size = 0
anti_entropy.throttle.tier1.delay = 0ms
anti_entropy.throttle.tier2.mailbox_size = 40
anti_entropy.throttle.tier2.delay = 5ms</pre>
If configured, there must be a tier which includes a mailbox size of 0.
...
<td>
Changes TicTacAAE from Passive or Active. If you want to run TicTac AAE alongside legacy AAE, set both to Active. Can be <code>active</code> or <code>passive</code>
...
associated <a href="/kv/3.0.6/learn/glossary/#vnode">vnode</a> before assuming that the vnode
...
href="/kv/3.0.6/learn/glossary/#vnode">vnodes</a> attempt to trigger <a
href="/kv/3.0.6/learn/glossary/#hinted-handoff">handoff</a> between
...
Enterprise Edition 1.3.2 - 2.2.3 and then from Riak KV 2.2.6 onwards.
</td>
<td>
<code>15</code>
</td>
...
only in Riak KV Enterprise Edition 2.0 and later as well as Riak KV 2.2.6 onwards.
</td>
<td>
<code>true</code>
</td>
