...
const sidebarSearchPartialSource = fs.readFileSync(path.join(repositoryRoot, 'layouts', 'common-docs', 'layouts', 'partials', 'sidebar-search.html'), 'utf8');
...
const mailingListSearchPartialSource = fs.readFileSync(path.join(repositoryRoot, 'layouts', 'archive-mailing-list', 'partials', 'mailing-list-sidebar-tools.html'), 'utf8');
...
assert.match(configurationReferenceShortcodeSource, /partial "configuration-datatype-values.html"/, 'configuration tables must use the shared datatype value renderer');
assert.match(configurationReferenceItemShortcodeSource, /partial "configuration-datatype-values.html"/, 'configuration details must use the shared datatype value renderer');
const datatypeValuesSource = fs.readFileSync(path.join(themeRoot, 'layouts/partials/configuration-datatype-values.html'), 'utf8');
assert.match(datatypeValuesSource, /<ul[\s\S]*<li>[\s\S]*if eq \.kind "value"[\s\S]*data-copy-label="Copy value[\s\S]*else[\s\S]*configuration-reference-type/, 'datatype lists must distinguish copyable literals from plain-text type alternatives');
...
assert.match(sharedHeaderSource, /data-theme-picker[\s\S]*class="theme-trigger"[^>]*aria-label="Display preferences"[\s\S]*class="display-trigger-icon"[^>]*>Aa<[\s\S]*class="display-trigger-label">Display<[\s\S]*class="picker-chevron"/, 'Display preferences must use a clear Aa trigger and shared picker');
...
assert.match(baseSource, /js\/page-tools\.js[^"\n]+\?v=20260907-search-code-tabs[\s\S]*js\/search-highlight\.js/, 'search highlighting must load after the cache-busted page and code tools runtime');
...
assert.match(sharedHeaderSource, /data-share-native[\s\S]*data-share-target="email"[\s\S]*class="share-separator"[\s\S]*data-share-target="bluesky"/, 'Email must appear in the utility group above the social-network separator');
assert.equal((sharedHeaderSource.match(/class="share-option-icon(?: share-option-icon--brand)?"/g) || []).length, 7, 'every Share menu action must have an icon');
assert.match(shareSource, /dataset\.selectedOs[\s\S]*searchParams\.set\('os', selectedOs\)/, 'share destinations must include the current OS');
assert.match(shareSource, /twitter\.com\/intent\/tweet[\s\S]*bsky\.app\/intent\/compose[\s\S]*reddit\.com\/submit[\s\S]*linkedin\.com\/sharing\/share-offsite/, 'social sharing must use explicit privacy-preserving links');
assert.match(shareSource, /navigator\.clipboard[\s\S]*execCommand\('copy'\)/, 'copying must use the Clipboard API with a compatibility fallback');
assert.match(shareSource, /copyLabel\.textContent = 'Copied!'[\s\S]*copyLabel\.textContent = original/, 'copy status must update only the label so its icon remains visible');
assert.match(shareSource, /role="menuitem"[\s\S]*ArrowDown[\s\S]*ArrowUp[\s\S]*Home[\s\S]*End[\s\S]*Escape[\s\S]*close\(true\)/, 'the Share menu must support keyboard navigation and focus restoration');
assert.match(shareSource, /Link copied to clipboard\.[\s\S]*data-share-status|data-share-status[\s\S]*Link copied to clipboard\./, 'copying a share link must produce an assistive status announcement');
for (const [name, source] of [['product', baseSource], ['community', siteSectionBaseSource], ['mailing list', mailingListBaseSource], ['technical blog', blogBaseSource], ['homepage', homepageSource]]) {
  assert.match(source, /js\/share\.js[^"\n]+\?v=20260903-share-icons/, `${name} pages must load the shared Share menu runtime`);
}
assert.match(runtimeSource, /dataset\.selectedOs = os\.id[\s\S]*searchParams\.delete\('os'\)[\s\S]*replaceState\(window\.history\.state/, 'OS selection must be stored for sharing while being removed from the visible URL');
...
assert.match(themeSource, /openriak-docs-font-size[\s\S]*localStorage\.getItem\(fontSizeKey\)[\s\S]*localStorage\.setItem\(fontSizeKey, value\)/, 'the selected font size must persist in local storage');
assert.match(themeSource, /document\.documentElement\.dataset\.fontSize = selectedValue/, 'the selected font size must be applied to the document root');
assert.match(themeSource, /syncHeaderHeight[\s\S]*removeProperty\('--site-header-height'\)[\s\S]*getBoundingClientRect\(\)\.height[\s\S]*requestAnimationFrame\(syncHeaderHeight\)/, 'font-size changes must synchronize sidebar offsets to the rendered header height');
assert.match(headSource, /openriak-docs-font-size[\s\S]*dataset\.fontSize/, 'the stored font size must be applied before styles load to avoid a layout shift');
assert.match(sharedHeaderSource, /data-font-size-value="100"[\s\S]*data-font-size-value="110"[\s\S]*data-font-size-value="125"[\s\S]*data-font-size-value="150"[\s\S]*data-font-size-value="200"[\s\S]*data-font-size-value="250"/, 'the appearance menu must offer every requested font size');
assert.match(sharedHeaderCss, /data-font-size="100"[\s\S]*data-font-size="110"[\s\S]*data-font-size="125"[\s\S]*data-font-size="150"[\s\S]*data-font-size="200"[\s\S]*data-font-size="250"/, 'shared styling must map every font-size preference');
assert.match(themeSource, /fontSizes = \['100', '110', '125', '150', '200', '250'\]/, 'font-size preference validation must accept every menu option');
assert.match(shellSource, /setAttribute\('aria-modal', 'true'\)[\s\S]*makeInert\(navBackground\(\)\)[\s\S]*visibleFocusable\(sidebar\)[\s\S]*event\.key !== 'Tab'/, 'mobile navigation must behave as a modal drawer with inert background and contained focus');
assert.match(themeSource, /mobileAppearance[\s\S]*setAttribute\('role', 'dialog'\)[\s\S]*setAttribute\('aria-modal', 'true'\)[\s\S]*makeInert\(appearanceBackground\(\)\)[\s\S]*event\.key !== 'Tab'/, 'the mobile appearance panel must behave as a modal dialog with contained focus');
assert.match(sharedHeaderSource, /data-theme-panel role="region" aria-label="Appearance preferences"/, 'the desktop appearance disclosure must not claim modal dialog semantics');
assert.match(sharedHeaderCss, /--global-orange: #9b4d00[\s\S]*--global-focus: #006a75/, 'light shared-header accents and focus indicators must use contrast-safe colours');
assert.match(docsCssSource, /--orange: #9b4d00[\s\S]*--focus-ring: #006a75[\s\S]*:focus-visible \{ outline: 3px solid var\(--focus-ring\)/, 'light documentation accents and focus indicators must use contrast-safe colours');
assert.match(sidebarSearchPartialSource, /data-search-status role="status" aria-live="polite" aria-atomic="true"[\s\S]*data-search-results role="region" aria-label="Search results"/, 'shared search must expose announcements and a labelled results region');
assert.match(sidebarSearchPartialSource, /aria-keyshortcuts="\/ Control\+K Meta\+K"[\s\S]*<kbd class="search-shortcut" aria-hidden="true">\/<\/kbd>/, 'shared search must advertise and display its keyboard shortcut');
...
assert.match(homepageSource, /<main id="main-content"[^>]*tabindex="-1"/, 'homepage skip-link targets must be programmatically focusable');
assert.match(sharedHeaderCss, /@media \(prefers-reduced-motion: reduce\)[\s\S]*scroll-behavior: auto !important[\s\S]*transition-duration: \.01ms !important/, 'shared styling must respect reduced-motion preferences');
assert.match(sharedHeaderCss, /--site-header-height: max\(64px, calc\(42px \+ 1\.1rem\)\)/, 'the shared header height must grow with large font settings');
...
assert.match(docsCssSource, /\.version-option:hover[^}]*color: var\(--orange\)/, 'version-picker text must highlight on hover');
...
assert.match(docsCssSource, /@media \(max-width: 760px\)[\s\S]*\.picker-panel \{[^}]*top: calc\(var\(--site-header-height\) \+ 1rem\)[^}]*bottom: 1rem[^}]*overflow-y: auto/, 'mobile picker panels must remain below the scaled header and scroll within the viewport');
