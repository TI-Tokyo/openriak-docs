...
const portalSource = fs.readFileSync(path.join(repositoryRoot, 'layouts', 'portal', 'index.html'), 'utf8');
const faviconPath = path.join(commonAssets, 'images', 'ui', 'favicon.ico');
const projectLogoPath = path.join(commonAssets, 'images', 'branding', 'openriak-project.png');
...
assert.match(headSource, /images\/ui\/favicon\.ico[^"\n]+\?v=/, 'documentation pages must use the shared OpenRiak favicon');
assert.match(portalSource, /images\/ui\/favicon\.ico[^"\n]+\?v=/, 'the portal must use the shared OpenRiak favicon');
...
assert.match(sharedHeaderSource, /class="brand-home" href="\{\{ \$page\.Site\.Home\.RelPermalink \}\}"[\s\S]*openriak-project\.png/, 'the OpenRiak project mark must always link to the site homepage');
...
assert.match(portalSource, /aria-label="Site sections"/, 'the homepage list must be named Site sections');
assert.match(portalSource, /range (?:where )?hugo\.Data\.site_sections/, 'the homepage and header must share the same Site sections data');
assert.match(portalSource, /partial "site-header\.html"[\s\S]*js\/theme\.js[\s\S]*js\/docs-shell\.js/, 'the homepage must use the shared header and theme controls');
assert.match(portalSource, /css\/site-header\.css[^"\n]+\?v=/, 'the homepage must load the cache-busted shared header stylesheet');
assert.match(portalSource, /where hugo\.Data\.site_sections "showOnHomepage" "ne" false/, 'the Homepage section must not link to itself in the homepage card grid');
...
assert.deepEqual(siteSections.map((section) => section.id), ['homepage', 'openriak-kv', 'openriak-cs', 'openriak-ts', 'community', 'archived-technical-blog'], 'Site sections must be ordered Homepage, products, Community, then Archived Technical Blog');
...
assert.match(hugoConfig, /source: 'portal\/pages'/, 'the homepage must be mounted into the unified project');
