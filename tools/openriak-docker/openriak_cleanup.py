...
def configure_parser(parser):
    ...
    parser.add_argument('--remove-all', action='store_true',
                        help='Also remove older current caches, Docker metadata/downloads, images and build cache; stop older workers')
...
def file_plan(tool, cutoff, remove_all=False):
...
def main(options, tool):
    ...
    trees, files = file_plan(tool, cutoff, options.remove_all)
    ...
    if options.remove_all and options.delete:
        ...
        trees, files = file_plan(tool, cutoff, True)
        changes = metadata_plan(tool, trees)
        check_write_access(tool, trees, files, changes)
