...
def configure_parser(parser, tool):
    ...
    plan.add_argument('--output', type=Path,
                      help='New plan JSON filename (default: ./openriak-docker-distribute-{UTC-timestamp}.json)')
...
def create_plan(tool, options):
    ...
    if options.output is None:
        options.output = Path.cwd() / f'openriak-docker-distribute-{tool.run_id(started)}.json'
...
def run_worker(tool, options, arguments):
    ...
    import openriak_worker_queue
    return openriak_worker_queue.execute(tool, __import__(__name__), plan, root, options.worker)
...
def validate_result(tool, plan, job, root, receipt):
    ...
    from openriak_push import archive_metadata
