...
This command controls Riak CS's [garbage collection]({{<baseurl>}}openriak-cs/2.0.1/cookbooks/garbage-collection) system.
...
begin listening on the host and port specified in your [configuration files]({{<baseurl>}}openriak-cs/2.0.1/cookbooks/configuration/reference).
...
More information can be found in [Riak CS Supercluster Support]({{<baseurl>}}openriak-cs/2.0.1/cookbooks/supercluster).
...
When a member's weight is updated, that weight is stored in the [master member]({{<baseurl>}}openriak-cs/2.0.1/cookbooks/supercluster/#the-master-member) and cached in Riak CS. Riak CS fetches weights from the master member only periodically. The
