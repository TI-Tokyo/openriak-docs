...
sitemap:
  priority: 0.9
aliases:
  - /riakcs/3.0.1/cookbooks/configuration/configuration-reference
  - /openriak-cs/3.0.1/cookbooks/configuration/configuration-reference
  - /openriak-cs/latest/cookbooks/configuration/reference/
...
{{% note title="Note on Legacy Configuration Usage" %}}
  **If you choose to use the legacy `app.config` files for Riak CS and/or
  ...
  and so on. More details can be found at [configuring Riak CS]({{<baseurl>}}openriak-cs/3.0.1/cookbooks/configuration/riak-cs).
{{% /note %}}
...
recommend consulting the [configuring Riak CS]({{<baseurl>}}openriak-cs/3.0.1/cookbooks/configuration/riak-cs) tutorial.
...
corresponding settings in the <a href="{{<baseurl>}}openriak-cs/3.0.1/theory/stanchion">Stanchion</a> node's <code>stanchion.conf</code>.
</td>
<td>
<code>admin-key</code>
</td>
...
a particular API to requests in the the native <a href="{{<baseurl>}}openriak-cs/3.0.1/references/apis/storage">Riak CS storage API</a>. We do
...
corresponding settings in the <a href="{{<baseurl>}}openriak-cs/3.0.1/theory/stanchion">Stanchion</a> node's
...
made using a particular API to requests in the the native <a href="{{<baseurl>}}openriak-cs/3.0.1/references/apis/storage">Riak CS storage API</a>. We do not recommend changing this setting unless you
...
These settings relate to Riak CS's [access logs]({{<baseurl>}}openriak-cs/3.0.1/cookbooks/usage-and-billing-data).
...
Settings related to Riak CS's [garbage collection]({{<baseurl>}}openriak-cs/3.0.1/cookbooks/garbage-collection) /(GC) process.
...
features for the current version to better support <a href="{{<baseurl>}}openriak-cs/3.0.1/cookbooks/rolling-upgrades">rolling upgrades</a>. New
...
enable new features for the current version to better support <a href="{{<baseurl>}}openriak-cs/3.0.1/cookbooks/rolling-upgrades">rolling upgrades</a>. New installs shouldn't need to modify this. If you're performing a rolling upgrade, keep the original value (if not defined, Riak CS uses <code>0</code>) of the old <code>app.config</code> until all nodes have been upgraded. At that point, set to the new value
</td>
<td>
<code></code>
</td>
