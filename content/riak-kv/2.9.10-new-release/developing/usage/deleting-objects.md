...
you first fetch the [causal context]({{<baseurl>}}openriak-kv/2.9.10/learn/concepts/causal-context) of the object prior to updating.
This can be done by setting the `deletedvclock` parameter to `true` as
part of the [fetch operation]({{<baseurl>}}openriak-kv/2.9.10/developing/api/protocol-buffers/fetch-object). This can also be done
