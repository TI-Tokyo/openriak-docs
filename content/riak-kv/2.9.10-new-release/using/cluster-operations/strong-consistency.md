...
status of a node. This data is available by running the [`riak-admin status`]({{<baseurl>}}openriak-kv/2.9.10/using/admin/riak-admin/#status) command. That data now
includes statistics specific to strongly consistent operations.

A full listing of these stats is available in [Inspecting a Node]({{<baseurl>}}openriak-kv/2.9.10/using/cluster-operations/inspecting-node).
