...
aliases:
  ...
  - /openriak-kv/2.0.8/dev/search/document-store
linkTitle: "Implementing a Document Store"
weight: 112
---

Although Riak wasn't explicitly created as a document store, two
features recently added to Riak---[Riak Search]({{<baseurl>}}openriak-kv/2.0.8/developing/usage/search/) and [Riak Data Types]({{<baseurl>}}openriak-kv/2.0.8/developing/data-types/)---make it possible to use Riak as a
...
[Riak maps]({{<baseurl>}}openriak-kv/2.0.8/developing/data-types/#maps).
...
however, we will store data in [Riak maps]({{<baseurl>}}openriak-kv/2.0.8/developing/data-types/#maps),
...
we're not interested in running normal [key/value queries]({{<baseurl>}}openriak-kv/2.0.8/developing/key-value-modeling) on these objects, we'll allow Riak to assign [keys]({{<baseurl>}}openriak-kv/2.0.8/learn/concepts/keys-and-objects) automatically. This means that all we have to do is worry about the bucket type and/or bucket when storing objects.
...
In the documentation on [search schemas]({{<baseurl>}}openriak-kv/2.0.8/developing/usage/search-schemas), you'll find a
...
described in [Data Modeling with Riak Data Types]({{<baseurl>}}openriak-kv/2.0.8/developing/data-modeling), we can construct a
