...
toc: true
aliases:
  - /riak/3.0.7/ops/building/installing/Installing-on-Mac-OS-X
  - /openriak-kv/3.0.7/ops/building/installing/Installing-on-Mac-OS-X
  - /riak/3.0.7/installing/mac-osx/
  - /openriak-kv/3.0.7/installing/mac-osx/
linkTitle: "Mac OS X"
weight: 305
---
...
```bash
curl -O https://files.tiot.jp/riak/kv/3.0/3.0.7/osx/10.11/riak-3.0.7-OSX-x86_64.tar.gz
tar xzvf riak-3.0.7-osx-x86_64.tar.gz
```
...
Installing Riak 3.0.7 with [Homebrew](http://brew.sh/) is easy:

```bash
brew install --devrel riak
```

By default, this will place a `3.0.7` folder in
...
```bash
curl -O https://files.tiot.jp/riak/kv/3.0/3.0.7/riak-3.0.7.tar.gz
