...
toc: true
aliases:
  - /riak/2.0.0/dev/references/protocol-buffers/get-bucket-props
  - /docs/openriak-kv/2.0.0/dev/references/protocol-buffers/get-bucket-props
...
The bucket's name (`bucket`) must be specified. The [bucket type]({{<baseurl>}}openriak-kv/2.0.0/using/cluster-operations/bucket-types) parameter (`type`) is optional. If it is not specified,
...
[configuration file]({{<baseurl>}}openriak-kv/2.0.0/configuring/reference/#default-bucket-properties) documentation.
...
The `RpbReplMode` is of use only to users of Riak CS's [Multi-Datacenter Replication capabilities]({{<baseurl>}}riak/cs/latest/cookbooks/mdc-overview/)
{{% /note %}}
