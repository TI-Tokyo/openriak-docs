...
aliases:
  ...
  - /openriak-kv/2.2.2/dev/search/custom-extractors
...
There are also built-in extractors for [Riak Data Types]({{<baseurl>}}openriak-kv/2.2.2/developing/usage/searching-data-types).
...
can be found in our documentation on [advanced]({{<baseurl>}}openriak-kv/2.2.2/configuring/reference/#advanced-configuration)). Here's an
example:
...
create a schema that extends the [default schema]({{<baseurl>}}openriak-kv/2.2.2/developing/usage/search-schemas/#creating-a-custom-schema). The following fields should be added
...
Now, we can create and activate a [bucket type]({{<baseurl>}}openriak-kv/2.2.2/developing/usage/bucket-types)
