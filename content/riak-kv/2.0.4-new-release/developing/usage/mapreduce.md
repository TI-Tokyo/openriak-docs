...
aliases:
  ...
  - /openriak-kv/2.0.4/dev/using/mapreduce
...
on key querying, alongside [secondary indexes]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/secondary-indexes/)
and [Search]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/search/). MapReduce jobs can be submitted through the
[HTTP API]({{<baseurl>}}openriak-kv/2.0.4/developing/api/http) or the [Protocol Buffers API]({{<baseurl>}}openriak-kv/2.0.4/developing/api/protocol-buffers/), although we
...
* When you know the set of objects over which you want to MapReduce
  (i.e. the locations of the objects, as specified by [bucket type]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/bucket-types), bucket, and key)
* When you want to return actual objects or pieces of objects and not
  just the keys. [Search]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/search/) and [secondary indexes]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/secondary-indexes) are other means of returning objects based on
...
[bucket type]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/bucket-types), bucket, and key. The elements of the
...
objects stored in those keys and instruct the proper [vnode]({{<baseurl>}}openriak-kv/2.0.4/learn/glossary/#vnode) to
...
`training` (which does not bear a [bucket type]({{<baseurl>}}openriak-kv/2.0.4/developing/usage/bucket-types)).
...
in conjunction with Riak's [HTTP API]({{<baseurl>}}openriak-kv/2.0.4/developing/api/http) to store the objects:
...
checking out our [Advanced MapReduce]({{<baseurl>}}openriak-kv/2.0.4/developing/app-guide/advanced-mapreduce) guide.
