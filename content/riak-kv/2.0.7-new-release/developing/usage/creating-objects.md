...
which bears the `animals` [bucket type]({{<baseurl>}}openriak-kv/2.0.7/developing/usage/bucket-types).
...
Now, run the same read operation in [Reading Objects]({{<baseurl>}}openriak-kv/2.0.7/developing/usage/reading-objects). If the write operation was successful, you should be able to successfully read the object. Your Riak cluster is no
longer empty!
...
[`default`]({{<baseurl>}}openriak-kv/2.0.7/developing/usage/bucket-types) will be applied.
