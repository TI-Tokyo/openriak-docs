...
toc: true
aliases:
  - /riak/3.0.14/dev/references/protocol-buffers/dt-fetch
  - /openriak-kv/3.0.14/dev/references/protocol-buffers/dt-fetch
linkTitle: "Data Type Fetch"
weight: 114
---

The equivalent of [`RpbGetReq`]({{<baseurl>}}openriak-kv/3.0.14/developing/api/protocol-buffers/fetch-object) for [Riak Data Types]({{<baseurl>}}openriak-kv/3.0.14/developing/data-types). This request results in a `DtFetchResp`
...
`type` | The [Using Bucket Types]({{<baseurl>}}openriak-kv/3.0.14/using/cluster-operations/bucket-types) of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map)
...
per-bucket basis. Please refer to the documentation on [setting bucket properties]({{<baseurl>}}openriak-kv/3.0.14/developing/api/protocol-buffers/set-bucket-props) for more information.
...
The response to a fetch request ([`DtFetchReq`]({{<baseurl>}}openriak-kv/3.0.14/developing/api/protocol-buffers/dt-fetch)) is a `DtFetchResp` message.
...
would send a [vclock]({{<baseurl>}}openriak-kv/3.0.14/learn/glossary/#vector-clock) for standard KV updates.
