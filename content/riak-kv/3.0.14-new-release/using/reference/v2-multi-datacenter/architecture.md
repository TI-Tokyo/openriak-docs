...
toc: true
aliases:
  - /riak/3.0.14/ops/mdc/v2/architecture
  - /openriak-kv/3.0.14/ops/mdc/v2/architecture
linkTitle: "Architecture"
weight: 100
---

{{% note title="Deprecation Warning" %}}
v2 Multi-Datacenter Replication is deprecated and will be removed in a future version. Please use [v3]({{<baseurl>}}openriak-kv/3.0.14/using/reference/v3-multi-datacenter/architecture/) instead.
{{% /note %}}
...
2. The site node in the secondary cluster initiates fullsync replication
   ...
   the primary cluster
3. The site and listener nodes iterate through each [vnode]({{<baseurl>}}openriak-kv/3.0.14/learn/glossary/#vnode) in their respective clusters and compute a hash for
...
same [ring size]({{<baseurl>}}openriak-kv/3.0.14/learn/concepts/clusters/#the-ring); if you are using fullsync
replication, every bucket's [`n_val`]({{<baseurl>}}openriak-kv/3.0.14/developing/app-guide/replication-properties#n-value-and-replication) must be the same in both the
