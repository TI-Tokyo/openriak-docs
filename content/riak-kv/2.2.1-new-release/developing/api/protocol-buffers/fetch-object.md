...
toc: true
aliases:
  - /riak/2.2.1/dev/references/protocol-buffers/fetch-object
  - /openriak-kv/2.2.1/dev/references/protocol-buffers/fetch-object
...
[`RpbPutResp`]({{<baseurl>}}openriak-kv/2.2.1/developing/api/protocol-buffers/store-object), respectively):
...
* `content_encoding` --- The content encoding of the object, e.g.
  `video/mp4`
* `vtag` --- The object's [vtag]({{<baseurl>}}openriak-kv/2.2.1/learn/glossary/#vector-clock)
...
* `usermeta` --- This field stores user-specified key/value metadata
  ...
  send metadata of this sort are structured like this:
    ...
    `RpbPair` messages are also used to attach [secondary indexes]({{<baseurl>}}openriak-kv/2.2.1/developing/usage/secondary-indexes) to objects (in the optional
    `indexes` field).
