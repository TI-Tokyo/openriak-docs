...
aliases:
  - /riak/2.2.1/dev/references/protocol-buffers/yz-index-put
  - /openriak-kv/2.2.1/dev/references/protocol-buffers/yz-index-put
...
binary (as `name`). Optionally, you can specify a [`schema`]({{<baseurl>}}openriak-kv/2.2.1/developing/usage/search-schemas) name and/or an `n_val`, i.e. the number of nodes on which the index is stored (for GET requests) or on which you wish the index to be stored (for PUT requests). An index's `n_val` must match the associated bucket's `n_val`.

## Response

Returns a [RpbPutResp]({{<baseurl>}}openriak-kv/2.2.1/developing/api/protocol-buffers/#message-codes) code with no data on success.
