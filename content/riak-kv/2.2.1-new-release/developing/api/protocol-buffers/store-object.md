...
toc: true
aliases:
  - /riak/2.2.1/dev/references/protocol-buffers/store-object
  - /openriak-kv/2.2.1/dev/references/protocol-buffers/store-object
linkTitle: "Store Object"
weight: 106
---

Stores an object under the specified location, as determined by the
intended [key]({{<baseurl>}}openriak-kv/2.2.1/learn/concepts/keys-and-objects), [bucket]({{<baseurl>}}openriak-kv/2.2.1/learn/concepts/buckets), and [bucket type]({{<baseurl>}}openriak-kv/2.2.1/developing/usage/bucket-types). A bucket must always be specified (via
...
[bucket type]({{<baseurl>}}openriak-kv/2.2.1/developing/usage/bucket-types) is assigned, Riak will assign
`default`, which means that the [default bucket configuration]({{<baseurl>}}openriak-kv/2.2.1/configuring/reference/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object]({{<baseurl>}}openriak-kv/2.2.1/developing/api/protocol-buffers/fetch-object)
...
as well as the object's [causal context]({{<baseurl>}}openriak-kv/2.2.1/learn/concepts/causal-context), in the `vclock`
