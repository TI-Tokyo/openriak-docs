...
included in version 2.0, check out our guide to [Riak 2.0]({{<baseurl>}}openriak-kv/2.1.3/introduction).
...
those older clients will not able to take advantage of [new features]({{<baseurl>}}openriak-kv/2.1.3/introduction) like [data types]({{<baseurl>}}openriak-kv/2.1.3/developing/data-types) or the new [Riak Search]({{<baseurl>}}openriak-kv/2.1.3/using/reference/search).

## Bucket Types

In versions of Riak prior to 2.0, the location of objects was
determined by objects' [bucket]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/buckets) and [key]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/keys-and-objects), while all bucket-level configurations were managed by setting [bucket properties]({{<baseurl>}}openriak-kv/2.1.3/developing/usage/bucket-types/).

In Riak 2.0, [bucket types]({{<baseurl>}}openriak-kv/2.1.3/using/cluster-operations/bucket-types) are both an additional namespace for locating objects _and_ a new way of configuring bucket properties in a systematic fashion. More comprehensive details on usage can be found in the documentation on [using bucket types]({{<baseurl>}}openriak-kv/2.1.3/using/reference/bucket-types).
...
A full tutorial can be found in [Using Bucket Types]({{<baseurl>}}openriak-kv/2.1.3/using/reference/bucket-types).
...
* Accept Riak's [default bucket configurations]({{<baseurl>}}openriak-kv/2.1.3/using/reference/bucket-types/#buckets-as-namespaces)
* Change Riak's defaults using your [configuration files]({{<baseurl>}}openriak-kv/2.1.3/configuring/reference/#default-bucket-properties)
...
* [Strong consistency]({{<baseurl>}}openriak-kv/2.1.3/using/reference/strong-consistency) --- Using Riak's strong consistency subsystem
  requires you to set the `consistent` parameter on a bucket type to
  `true`
* [Riak Data Types]({{<baseurl>}}openriak-kv/2.1.3/developing/data-types) --- In order to use Riak Data
  Types, you must [create bucket types]({{<baseurl>}}openriak-kv/2.1.3/developing/data-types/#setting-up-buckets-to-use-riak-data-types) specific to the
  Data Type you are using

#### Bucket types and downgrades

If you decide to use bucket types, please remember that you
cannot [downgrade]({{<baseurl>}}openriak-kv/2.1.3/setup/downgrade) your cluster to a version of
...
[siblings]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/causal-context/#siblings) behavior.
...
object replica [conflicts]({{<baseurl>}}openriak-kv/2.1.3/developing/usage/conflict-resolution) between nodes on its
...
This means that the default when [using bucket types]({{<baseurl>}}openriak-kv/2.1.3/using/reference/bucket-types/) is to handle [conflict resolution]({{<baseurl>}}openriak-kv/2.1.3/developing/usage/conflict-resolution) on the client side using
either traditional [vector clocks]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/causal-context/#vector-clocks) or the newer [dotted version vectors]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/causal-context/#dotted-version-vector).
...
on [conflict resolution]({{<baseurl>}}openriak-kv/2.1.3/developing/usage/conflict-resolution).

## Enabling Security

The [authentication and authorization]({{<baseurl>}}openriak-kv/2.1.3/using/security/basics) mechanisms included with Riak 2.0 should only be turned
...
* Index data to be used in conjunction with the new [Riak Search]({{<baseurl>}}openriak-kv/2.1.3/using/reference/search).
* Create _and_ activate one or more [bucket types]({{<baseurl>}}openriak-kv/2.1.3/using/reference/bucket-types/). By extension, you will not be able to downgrade your cluster if you have used the following features, both of which rely on bucket types:
    - [Strong consistency]({{<baseurl>}}openriak-kv/2.1.3/using/reference/strong-consistency)
    - [Riak Data Types]({{<baseurl>}}openriak-kv/2.1.3/developing/data-types)

If you use other new features, such as [Riak Security]({{<baseurl>}}openriak-kv/2.1.3/using/security/basics) or the new [configuration files]({{<baseurl>}}openriak-kv/2.1.3/configuring/reference/), you can still
...
[Configuration Files]({{<baseurl>}}openriak-kv/2.1.3/configuring/reference/).
...
[configuration files]({{<baseurl>}}openriak-kv/2.1.3/configuring/reference/) documentation, as many configuration parameters have changed names, some no longer exist, and others have been added that were not previously available.
...
For a listing of those parameters, see our documentation on [advanced configuration]({{<baseurl>}}openriak-kv/2.1.3/configuring/reference/#advanced-configuration).
...
[Search upgrade guide]({{<baseurl>}}openriak-kv/2.1.3/setup/upgrading/search).
...
4. Run [`riak-admin reip <old_nodename> <new_nodename>`]({{<baseurl>}}openriak-kv/2.1.3/using/admin/riak-admin/#reip) on each node in your cluster, for each node in your
