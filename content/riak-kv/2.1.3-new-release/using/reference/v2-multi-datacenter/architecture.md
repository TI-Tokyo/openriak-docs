...
toc: true
aliases:
  - /riak/2.1.3/ops/mdc/v2/architecture
  - /openriak-kv/2.1.3/ops/mdc/v2/architecture
...
2. The site node in the secondary cluster initiates fullsync replication
   ...
   the primary cluster
3. The site and listener nodes iterate through each [vnode]({{<baseurl>}}openriak-kv/2.1.3/learn/glossary/#vnode) in their respective clusters and compute a hash for
...
same [ring size]({{<baseurl>}}openriak-kv/2.1.3/learn/concepts/clusters/#the-ring); if you are using fullsync
replication, every bucket's [`n_val`]({{<baseurl>}}openriak-kv/2.1.3/developing/app-guide/replication-properties#n-value-and-replication) must be the same in both the
