...
aliases:
  ...
  - /openriak-kv/3.0.1/ops/mdc/v2/architecture
...
v2 Multi-Datacenter Replication is deprecated and will be removed in a future version. Please use [v3]({{<baseurl>}}openriak-kv/3.0.1/using/reference/v3-multi-datacenter/architecture/) instead.
{{% /note %}}
...
2. The site node in the secondary cluster initiates fullsync replication
   ...
   the primary cluster
3. The site and listener nodes iterate through each [vnode]({{<baseurl>}}openriak-kv/3.0.1/learn/glossary/#vnode) in their respective clusters and compute a hash for
...
same [ring size]({{<baseurl>}}openriak-kv/3.0.1/learn/concepts/clusters/#the-ring); if you are using fullsync
replication, every bucket's [`n_val`]({{<baseurl>}}openriak-kv/3.0.1/developing/app-guide/replication-properties#n-value-and-replication) must be the same in both the
