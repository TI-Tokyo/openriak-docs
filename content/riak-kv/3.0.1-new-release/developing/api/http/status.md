...
toc: true
aliases:
  - /riak/3.0.1/dev/references/http/status
  - /openriak-kv/3.0.1/dev/references/http/status
...
The output of `/stats` contains the output of `riak admin status` detailed in the [Inspecting a Node]({{<baseurl>}}openriak-kv/3.0.1/using/cluster-operations/inspecting-node) doc, plus the below stats generated by the Riak Core application.
