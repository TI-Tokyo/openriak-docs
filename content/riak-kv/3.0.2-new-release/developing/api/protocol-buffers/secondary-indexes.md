...
toc: true
aliases:
  - /riak/3.0.2/dev/references/protocol-buffers/secondary-indexes
  - /openriak-kv/3.0.2/dev/references/protocol-buffers/secondary-indexes
...
`type` | The bucket type of the bucket that is being queried. If not set, the bucket type `default` will be used. Learn more about [using bucket types]({{<baseurl>}}openriak-kv/3.0.2/developing/usage/bucket-types).
...
`results` | If `return_terms` is specified with range queries, used to return matched index values as key/value pairs in `RpbPair` messages. More on `RpbPair` messages can be found in [PBC Fetch Object]({{<baseurl>}}openriak-kv/3.0.2/developing/api/protocol-buffers/fetch-object).
