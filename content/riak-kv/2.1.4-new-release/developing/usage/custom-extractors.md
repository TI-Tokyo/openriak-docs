...
aliases:
  - /riak/2.1.4/dev/search/custom-extractors
  - /openriak-kv/2.1.4/dev/search/custom-extractors
...
There are also built-in extractors for [Riak Data Types]({{<baseurl>}}openriak-kv/2.1.4/developing/usage/searching-data-types).
...
can be found in our documentation on [advanced]({{<baseurl>}}openriak-kv/2.1.4/configuring/reference/#advanced-configuration)). Here's an
example:
...
create a schema that extends the [default schema]({{<baseurl>}}openriak-kv/2.1.4/developing/usage/search-schemas/#creating-a-custom-schema). The following fields should be added
...
Now, we can create and activate a [bucket type]({{<baseurl>}}openriak-kv/2.1.4/developing/usage/bucket-types)
