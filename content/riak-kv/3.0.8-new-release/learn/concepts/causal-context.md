...
toc: true
aliases:
  - /riak/3.0.8/theory/concepts/context
  - /openriak-kv/3.0.8/theory/concepts/context
...
[dev key value]: {{<baseurl>}}openriak-kv/3.0.8/developing/key-value-modeling
[glossary read rep]: {{<baseurl>}}openriak-kv/3.0.8/learn/glossary/#read-repair
[perf latency reduc]: {{<baseurl>}}openriak-kv/3.0.8/using/performance/latency-reduction
...
[usage protocol buffers]: {{<baseurl>}}openriak-kv/3.0.8/developing/api/protocol-buffers
[usage updating objects]: {{<baseurl>}}openriak-kv/3.0.8/developing/usage/updating-objects
...
In the case of outcome 2, the choice between **a**, **b** and **c** is determined by settings. If you set the `allow_mult` parameter to `true` for a [bucket type]({{<baseurl>}}openriak-kv/3.0.8/developing/usage/bucket-types), all non-CRDT writes to that bucket type will create siblings in the case of concurrent writes (and occasionally under other
