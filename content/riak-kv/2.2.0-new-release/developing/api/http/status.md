...
toc: true
aliases:
  - /riak/2.2.0/dev/references/http/status
  - /openriak-kv/2.2.0/dev/references/http/status
linkTitle: "Status"
weight: 111
---
...
The output of `/stats` contains the output of `riak-admin status` detailed in the [Inspecting a Node]({{<baseurl>}}openriak-kv/2.2.0/using/cluster-operations/inspecting-node) doc, plus the below stats generated by the Riak Core application.
