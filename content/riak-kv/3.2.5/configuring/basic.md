...
[use running cluster]: {{< product-version-root >}}using/running-a-cluster
[use admin riak admin#member-status]: {{< product-version-root >}}using/admin/riak-admin/#member-status
[perf erlang]: {{< product-version-root >}}using/performance/erlang
[plan start]: {{< product-version-root >}}setup/planning/start
[plan best practices]: {{< product-version-root >}}setup/planning/best-practices
[cluster ops backup]: {{< product-version-root >}}using/cluster-operations/backing-up
[cluster ops add remove node]: {{< product-version-root >}}using/cluster-operations/adding-removing-nodes
[plan backend]: {{< product-version-root >}}setup/planning/backend
...
[perf benchmark]: {{< product-version-root >}}using/performance/benchmarking
[perf open files]: {{< product-version-root >}}using/performance/open-files-limit
[perf index]: {{< product-version-root >}}using/performance
[perf aws]: {{< product-version-root >}}using/performance/amazon-web-services
[Cluster Capacity Planning]: {{< product-version-root >}}setup/planning/cluster-capacity/#ring-size-number-of-partitions
...
directly impacted, although the mechanism described in [HTTP Reset Bucket Properties]({{< product-version-root >}}developing/api/http/reset-bucket-props) can be used to force them to pick up the new
defaults.
