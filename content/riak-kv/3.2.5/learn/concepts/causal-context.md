...
[dev key value]: {{< product-version-root >}}developing/key-value-modeling
[glossary read rep]: {{< product-version-root >}}learn/glossary/#read-repair
[perf latency reduc]: {{< product-version-root >}}using/performance/latency-reduction
...
[usage protocol buffers]: {{< product-version-root >}}developing/api/protocol-buffers
[usage updating objects]: {{< product-version-root >}}developing/usage/updating-objects
...
In the case of outcome 2, the choice between **a**, **b** and **c** is determined by settings. If you set the `allow_mult` parameter to `true` for a [bucket type]({{< product-version-root >}}developing/usage/bucket-types), all non-CRDT writes to that bucket type will create siblings in the case of concurrent writes (and occasionally under other
