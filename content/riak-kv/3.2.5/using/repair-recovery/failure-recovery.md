...
aliases:
  ...
  - /riak/kv/3.2.5/ops/running/recovery/failure-recovery
---
...
returns to service (also called [hinted handoff]({{< product-version-root >}}learn/glossary/#hinted-handoff)).
...
Sometimes extreme latency spikes can be caused by [sibling explosion]({{< product-version-root >}}developing/usage/conflict-resolution#siblings). This condition occurs when the client application does not resolve conflicts properly or in a timely fashion. In that scenario, the size of the value on disk grows in proportion to
...
See [Changing Cluster Information]({{< product-version-root >}}using/cluster-operations/changing-cluster-info/#clusters-from-backups) for instructions on cluster recovery.
