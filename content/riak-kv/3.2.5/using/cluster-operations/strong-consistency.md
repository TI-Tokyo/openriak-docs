...
status of a node. This data is available by running the [`riak admin status`]({{< product-version-root >}}using/admin/riak-admin/#status) command. That data now
includes statistics specific to strongly consistent operations.

A full listing of these stats is available in [Inspecting a Node]({{< product-version-root >}}using/cluster-operations/inspecting-node).
