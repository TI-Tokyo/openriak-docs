...
aliases:
  ...
  - /riak/kv/3.2.5/dev/using/conflict-resolution
---

[usage bucket types]: {{< product-version-root >}}developing/usage/bucket-types
[use ref strong consistency]: {{< product-version-root >}}using/reference/strong-consistency

One of Riak's [central goals](../../../learn/why-riak-kv) is high availability. It was built as a [clustered]({{< product-version-root >}}learn/concepts/clusters) system in which any [node]({{< product-version-root >}}learn/glossary/#node) is capable of receiving requests without requiring that
every node participate in each request.

If you are using Riak in an [eventually consistent]({{< product-version-root >}}learn/concepts/eventual-consistency) way, conflicts between object values on different nodes is
unavoidable. Often, Riak can resolve these conflicts on its own
internally if you use causal context, i.e. [vector clocks]({{< product-version-root >}}learn/concepts/causal-context#vector-clocks) or [dotted version vectors]({{< product-version-root >}}learn/concepts/causal-context#dotted-version-vectors), when updating objects. Instructions on this can be found in the section [below](#siblings).
...
1. If your data can be modeled as one of the currently available [Riak
   Data Types]({{< product-version-root >}}developing/data-types), we recommend using one of these types,
...
>
> * [Using Strong Consistency]({{< product-version-root >}}developing/app-guide/strong-consistency) - A guide for developers
> * [Managing Strong Consistency]({{< product-version-root >}}configuring/strong-consistency) - A guide for operators
...
While Riak _does_ have a set of [defaults]({{< product-version-root >}}developing/app-guide/replication-properties#available-parameters), there are a variety of general
...
[using bucket types][usage bucket types]. The most important [bucket properties]({{< product-version-root >}}learn/concepts/buckets)
...
[`last_write_wins`]({{< product-version-root >}}learn/concepts/buckets). If `last_write_wins` is set to `false`,
...
* [Java]({{< product-version-root >}}developing/usage/conflict-resolution/java)
* [Ruby]({{< product-version-root >}}developing/usage/conflict-resolution/ruby)
* [Python]({{< product-version-root >}}developing/usage/conflict-resolution/python)
* [C#]({{< product-version-root >}}developing/usage/conflict-resolution/csharp)
* [Node.js]({{< product-version-root >}}developing/usage/conflict-resolution/nodejs)

In Riak versions 2.0 and later, `allow_mult` is set to `true` by default
for any [bucket types]({{< product-version-root >}}developing/usage/bucket-types) that you create. This means
that if you wish to avoid client-side sibling resolution, you have a few
options:

* Explicitly create and activate [bucket types]({{< product-version-root >}}developing/usage/bucket-types)
  that set `allow_mult` to `false`
* Use Riak's [Configuration Files]({{< product-version-root >}}configuring/reference) to change the [default bucket properties]({{< product-version-root >}}configuring/reference#default-bucket-properties) for your
...
on what value you set for `dvv_enabled`. If set to `true`, [dotted version vectors]({{< product-version-root >}}learn/concepts/causal-context#dotted-version-vectors) will be used; if set to `false` (the default), [vector clocks]({{< product-version-root >}}learn/concepts/causal-context#vector-clocks) will be used.
...
[sibling explosion]({{< product-version-root >}}using/performance/latency-reduction#siblings), which can
...
Basho's official [client libraries]({{< product-version-root >}}developing/client-libraries). Examples can be found for each
client library in the [Object Updates]({{< product-version-root >}}developing/usage/updating-objects) document.
...
[causal context]({{< product-version-root >}}learn/concepts/causal-context). This is a less likely scenario if a client updates
...
[client libraries]({{< product-version-root >}}developing/client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started]({{< product-version-root >}}developing/getting-started) section.
...
the entire node. Other issues include [increased cluster latency]({{< product-version-root >}}using/performance/latency-reduction) as the object is replicated and out-of-memory errors.
