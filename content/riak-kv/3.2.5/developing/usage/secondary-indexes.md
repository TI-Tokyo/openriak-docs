...
recommend [Riak Search]({{< product-version-root >}}deprecated/riak-search/) rather than secondary indexes for
...
be used to find multiple objects in Riak. If you're storing [user data]({{< product-version-root >}}developing/data-modeling/#user-accounts), for example, you could tag each object
...
[Riak Search]({{< product-version-root >}}deprecated/riak-search/) serves analogous purposes but is quite
...
* Query results can be used as input to a [MapReduce]({{< product-version-root >}}developing/usage/mapreduce/)
  query
...
[strongly consistent buckets]({{< product-version-root >}}developing/app-guide/strong-consistency/#creating-a-strongly-consistent-bucket-type) and attach
...
mechanism that does not require a schema (as does [Riak Search]({{< product-version-root >}}deprecated/riak-search/#schemas)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search]({{< product-version-root >}}deprecated/riak-search/).
* When you want to use composite queries. A query like
  ...
  [MapReduce]({{< product-version-root >}}developing/usage/mapreduce/)).
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{< product-version-root >}}developing/client-libraries), you can find more information about getting started with
your client in the [Developing with Riak KV: Getting Started]({{< product-version-root >}}developing/getting-started) section.
...
>
> For all 2i queries, the [R]({{< product-version-root >}}developing/app-guide/replication-properties#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs]({{< product-version-root >}}learn/glossary/#hinted-handoff) and related operations are underway may not
