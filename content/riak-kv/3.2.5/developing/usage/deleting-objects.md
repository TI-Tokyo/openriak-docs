...
you first fetch the [causal context]({{< product-version-root >}}learn/concepts/causal-context) of the object prior to updating.
This can be done by setting the `deletedvclock` parameter to `true` as
part of the [fetch operation]({{< product-version-root >}}developing/api/protocol-buffers/fetch-object). This can also be done
