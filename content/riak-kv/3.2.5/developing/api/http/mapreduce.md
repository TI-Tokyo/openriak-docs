...
aliases:
  ...
  - /riak/kv/3.2.5/dev/references/http/mapreduce
---

[MapReduce]({{< product-version-root >}}developing/usage/mapreduce) is a generic way to query Riak by specifying inputs and constructing a set of map, reduce, and link phases through which data will flow.
...
Important headers:
* `Content-Type` - must always be `application/json`.  The format of the request body is described in detail on the [MapReduce]({{< product-version-root >}}developing/usage/mapreduce) page.
