...
aliases:
  ...
  - /riak/kv/3.2.5/dev/references/protocol-buffers/dt-fetch
---

The equivalent of [`RpbGetReq`]({{< product-version-root >}}developing/api/protocol-buffers/fetch-object) for [Riak Data Types]({{< product-version-root >}}developing/data-types). This request results in a `DtFetchResp`
...
`type` | The [Using Bucket Types]({{< product-version-root >}}using/cluster-operations/bucket-types) of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map)
...
per-bucket basis. Please refer to the documentation on [setting bucket properties]({{< product-version-root >}}developing/api/protocol-buffers/set-bucket-props) for more information.
...
The response to a fetch request ([`DtFetchReq`]({{< product-version-root >}}developing/api/protocol-buffers/dt-fetch)) is a `DtFetchResp` message.
...
would send a [vclock]({{< product-version-root >}}learn/glossary/#vector-clock) for standard KV updates.
