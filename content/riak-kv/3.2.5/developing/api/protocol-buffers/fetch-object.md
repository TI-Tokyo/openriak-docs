...
aliases:
  ...
  - /riak/kv/3.2.5/dev/references/protocol-buffers/fetch-object
---
...
[`RpbPutResp`]({{< product-version-root >}}developing/api/protocol-buffers/store-object), respectively):
...
* `content_encoding` - The content encoding of the object, e.g.
  `video/mp4`
* `vtag` - The object's [vtag]({{< product-version-root >}}learn/glossary/#vector-clock)
...
* `usermeta` - This field stores user-specified key/value metadata
  ...
  send metadata of this sort are structured like this:
    ...
    `RpbPair` messages are also used to attach [secondary indexes]({{< product-version-root >}}developing/usage/secondary-indexes) to objects (in the optional
    `indexes` field).
