...
aliases:
  ...
  - /riak/kv/3.2.5/dev/references/protocol-buffers/dt-store
---

A request to update the value of a [Riak Data Type]({{< product-version-root >}}developing/data-types).
...
performed. More on that in the [PBC Data Type Union]({{< product-version-root >}}developing/api/protocol-buffers/dt-union) document.
...
`type` | The bucket type of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map). Learn more about [using bucket types]({{< product-version-root >}}using/cluster-operations/bucket-types).
...
[counter]({{< product-version-root >}}developing/api/protocol-buffers/dt-counter-store), [set]({{< product-version-root >}}developing/api/protocol-buffers/dt-set-store), or [map]({{< product-version-root >}}developing/api/protocol-buffers/dt-map-store).
...
`context` | The opaque binary "context" that informs Riak which version of a data type the client has seen, analogous to [vector clocks]({{< product-version-root >}}learn/glossary/#vector-clock)
...
[`RpbPutResp`]({{< product-version-root >}}developing/api/protocol-buffers/store-object) for KV operations. If the
