...
aliases:
  ...
  - /riak/kv/3.2.5/dev/data-modeling/key-value/
---

While Riak enables you to take advantage of a wide variety of features
that can be useful in application development, such as [Search]({{< product-version-root >}}deprecated/riak-search/), [secondary indexes (2i)]({{< product-version-root >}}developing/usage/secondary-indexes/), and [Riak Data Types]({{< product-version-root >}}developing/data-types/), Riak almost always performs best when you
...
for each of them or consulting our guide to [building applications with Riak]({{< product-version-root >}}developing/app-guide/) for a better sense of which features you might need.
...
* The object's [key]({{< product-version-root >}}learn/concepts/keys-and-objects#keys), which can be anything you
  want as long as it is [Unicode compliant](http://www.unicode.org/)
* The [bucket]({{< product-version-root >}}learn/concepts/buckets) which houses the object and its key (bucket
  names are also Unicode compliant)
* The [bucket type]({{< product-version-root >}}developing/usage/bucket-types) that determines the bucket's
  [replication]({{< product-version-root >}}developing/app-guide/replication-properties) and other properties
...
look like (for the [HTTP API]({{< product-version-root >}}developing/api/http)):
...
One way to determine this is to [list all keys]({{< product-version-root >}}developing/api/protocol-buffers/list-keys) in the
...
A better possibility is to use [Riak sets]({{< product-version-root >}}developing/data-types/#sets) to
store lists of keys in a bucket. Riak sets are a [Riak Data Type]({{< product-version-root >}}developing/data-types) that enable you to store lists of binaries or strings in Riak.
...
`usernames`. The following will also assume that we've [set up a bucket type]({{< product-version-root >}}developing/data-types/#setting-up-buckets-to-use-riak-data-types) called
`sets`.
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{< product-version-root >}}developing/client-libraries), you can find more information about getting started with your client in [Developing with Riak KV: Getting Started]({{< product-version-root >}}developing/getting-started).
...
Riak [bucket types]({{< product-version-root >}}developing/usage/bucket-types) have two essential functions:
they enable you to manage [bucket configurations]({{< product-version-root >}}learn/concepts/buckets) in an
...
that simply extend Riak's [defaults]({{< product-version-root >}}developing/usage/bucket-types/#bucket-types-as-namespaces) or multiple bucket types that have
