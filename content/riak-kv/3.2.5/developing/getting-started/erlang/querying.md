...
A more comprehensive discussion can be found in [Key/Value Modeling]({{< product-version-root >}}developing/key-value-modeling).
...
{{% note %}}
Secondary indexes in Riak KV require a sorted backend: [Memory]({{< product-version-root >}}setup/planning/backend/memory) or [LevelDB]({{< product-version-root >}}setup/planning/backend/leveldb). [Bitcask]({{< product-version-root >}}setup/planning/backend/bitcask) does not support secondary indexes.

See [Using Secondary Indexes (2i)]({{< product-version-root >}}developing/usage/secondary-indexes) for more information on developing with secondary indexes.
{{% /note %}}
