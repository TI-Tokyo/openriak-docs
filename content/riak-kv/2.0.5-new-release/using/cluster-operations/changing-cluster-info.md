...
toc: true
aliases:
  - /riak/2.0.5/ops/running/nodes/renaming
  - /openriak-kv/2.0.5/ops/running/nodes/renaming
...
Previous to Riak version 1.2, a cluster node's name could only be changed with the [`riak-admin reip`]({{<baseurl>}}openriak-kv/2.0.5/using/admin/riak-admin/#reip) command, which involves downtime for the entire cluster. As of Riak version 1.2, that method has been superseded by [`riak-admin cluster force-replace`]({{<baseurl>}}openriak-kv/2.0.5/using/admin/riak-admin/#cluster-force-replace), which is safer and does not require cluster wide downtime.
