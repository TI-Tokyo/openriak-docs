...
toc: true
aliases:
  - /riak/3.0.4/dev/using/2i
  - /openriak-kv/3.0.4/dev/using/2i
...
be used to find multiple objects in Riak. If you're storing [user data]({{<baseurl>}}openriak-kv/3.0.4/developing/data-modeling/#user-accounts), for example, you could tag each object
...
* Query results can be used as input to a [MapReduce]({{<baseurl>}}openriak-kv/3.0.4/developing/usage/mapreduce/)
  query
...
[strongly consistent buckets]({{<baseurl>}}openriak-kv/3.0.4/developing/app-guide/strong-consistency/#creating-a-strongly-consistent-bucket-type) and attach
...
* When you want to use composite queries. A query like
  ...
  [MapReduce]({{<baseurl>}}openriak-kv/3.0.4/developing/usage/mapreduce/)).
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{<baseurl>}}openriak-kv/3.0.4/developing/client-libraries), you can find more information about getting started with
your client in the [Developing with Riak KV: Getting Started]({{<baseurl>}}openriak-kv/3.0.4/developing/getting-started) section.
...
>
> For all 2i queries, the [R]({{<baseurl>}}openriak-kv/3.0.4/developing/app-guide/replication-properties#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs]({{<baseurl>}}openriak-kv/3.0.4/learn/glossary/#hinted-handoff) and related operations are underway may not
