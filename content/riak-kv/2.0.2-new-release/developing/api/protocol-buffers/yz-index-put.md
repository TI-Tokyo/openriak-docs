...
aliases:
  - /riak/2.0.2/dev/references/protocol-buffers/yz-index-put
...
binary (as `name`). Optionally, you can specify a [`schema`]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search-schemas) name and/or an `n_val`, i.e. the number of nodes on which the index is stored (for GET requests) or on which you wish the index to be stored (for PUT requests). An index's `n_val` must match the associated bucket's `n_val`.
