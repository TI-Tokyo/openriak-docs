...
Creates a new Riak Search [index]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search/#simple-setup).
...
will create a new Search index that uses the [default Search schema]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search-schemas/#the-default-schema), i.e. `_yz_default`.
...
the schema to use. If you've [stored a schema]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search-schemas/#custom-schemas) called `my_custom_schema`, the following `PUT`
...
More information can be found in [Using Search]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search).
