...
version_history:
  in: "2.0.0-2.1.999"
aliases:
  - /openriak-kv/2.0.2/intro-v20
  - /riak/2.0.2/intro-v20
...
* **Riak Data Types** --- Riak's new CRDT-based [Data Types]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types) can
...
* **Strong Consistency, Riak Security, and the New Riak Search** ---
  ...
  Riak.
* **Security** --- [Authentication and authorization]({{<baseurl>}}openriak-kv/2.0.2/using/security/basics) can be enabled
  or disabled at any time.
* **Configuration management** --- Riak's [configuration files]({{<baseurl>}}openriak-kv/2.0.2/configuring/reference/) have
...
* **Bucket Types** --- While we strongly recommend [using bucket types]({{<baseurl>}}openriak-kv/2.0.2/using/reference/bucket-types) when creating new buckets, they are not required.
* **Dotted Version Vectors (DVVs)** --- This alternative to traditional
  [vector clocks]({{<baseurl>}}openriak-kv/2.0.2/learn/concepts/causal-context/#vector-clocks) is enabled by default
  in all [bucket types]({{<baseurl>}}openriak-kv/2.0.2/using/reference/bucket-types), but DVVs can be disabled
...
* [Cluster metadata]({{<baseurl>}}openriak-kv/2.0.2/developing/app-guide/cluster-metadata) --- This is a subsystem of Riak added in 2.0 that
  reduces the amount of inter-node gossip in Riak clusters, which can
  reduce network congestion.
* [Active Anti-Entropy]({{<baseurl>}}openriak-kv/2.0.2/learn/glossary/#active-anti-entropy-aae) --- While Riak has had an Active Anti-Entropy
...
More on upgrading can be found in our [Riak 2.0 upgrade guide]({{<baseurl>}}openriak-kv/2.0.2/setup/upgrading/version).
...
cases in the form of [Riak Data Types]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types). Instead of
...
Riak: [flags]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types/maps#flags), [registers]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types/maps#registers),
[counters]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types/counters), [sets]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types/sets), and
[maps]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types/maps).

#### Relevant Docs

* [Using Data Types]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types) explains how to use Riak Data Types on the
...
* [Data Types]({{<baseurl>}}openriak-kv/2.0.2/developing/data-types) explains some of the theoretical concerns that drive
...
* [Using Search]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search) provides an overview of how to use the new
  Riak Search.
* [Search Schema]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/search-schemas) shows you how to create and manage custom search
  schemas.
* [Search Details]({{<baseurl>}}openriak-kv/2.0.2/using/reference/search) provides an in-depth look at the design
...
* [Using Strong Consistency]({{<baseurl>}}openriak-kv/2.0.2/using/cluster-operations/strong-consistency) shows you how to enable Riak's strong
  consistency subsystem and to apply strong consistency guarantees to
  data stored in specified buckets.
* [Strong Consistency]({{<baseurl>}}openriak-kv/2.0.2/using/reference/strong-consistency) provides a theoretical treatment of how a
  strongly consistent system differs from an [eventually consistent]({{<baseurl>}}openriak-kv/2.0.2/learn/concepts/eventual-consistency) system, as well as details about how
  strong consistency is implemented in Riak.
* [Managing Strong Consistency]({{<baseurl>}}openriak-kv/2.0.2/configuring/strong-consistency) is a guide to strong consistency for
  Riak operators.
...
* [Authentication and Authorization]({{<baseurl>}}openriak-kv/2.0.2/using/security/basics) explains how Riak Security can be
...
* [Managing Security Sources]({{<baseurl>}}openriak-kv/2.0.2/using/security/managing-sources/) is an in-depth tutorial on how to
...
* [Configuration Files]({{<baseurl>}}openriak-kv/2.0.2/configuring/reference/) lists and describes all of the configurable
...
* [Using Bucket Types]({{<baseurl>}}openriak-kv/2.0.2/using/reference/bucket-types) explains how to create, modify, and activate
...
In prior versions of Riak, [conflict resolution]({{<baseurl>}}openriak-kv/2.0.2/developing/usage/conflict-resolution) was managed using
[vector clocks]({{<baseurl>}}openriak-kv/2.0.2/learn/concepts/causal-context/#vector-clocks), which track object update causality.

Riak 2.0 has added support for dotted version vectors (DVVs).
DVVs serve an analogous role to vector
clocks but are more effective at containing [sibling explosion]({{<baseurl>}}openriak-kv/2.0.2/learn/concepts/causal-context/#sibling-explosion) and can reduce Riak cluster latency.

#### Relevant Docs

* [Dotted Version Vectors]({{<baseurl>}}openriak-kv/2.0.2/learn/concepts/causal-context/#dotted-version-vectors) explains some of the theoretical nuances behind the distinction between DVVs and vector clocks and offers instructions on implementing DVVs.

## New Client Libraries

While Basho offered official [client libraries]({{<baseurl>}}openriak-kv/2.0.2/developing/client-libraries) for Java, Ruby,
