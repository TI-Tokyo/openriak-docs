...
aliases:
  ...
  - /openriak-kv/2.1.1/ops/running/logging
linkTitle: "Logging"
weight: 100
---
...
* `file` --- Console logs will be emitted to a file. This is Riak's
    ...
    `./log/console.log` on an installation from [source]({{<baseurl>}}openriak-kv/2.1.1/setup/installing/source), but will differ on platform-specific installation,
...
* `console` --- Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`]({{<baseurl>}}openriak-kv/2.1.1/using/admin/riak-cli/#attach-direct) command
