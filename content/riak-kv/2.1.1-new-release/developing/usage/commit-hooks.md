...
toc: true
aliases:
  - /riak/2.1.1/dev/using/commit-hooks
  - /openriak-kv/2.1.1/dev/using/commit-hooks
...
Pre- and post-commit hooks are applied at the [bucket]({{<baseurl>}}openriak-kv/2.1.1/learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{<baseurl>}}openriak-kv/2.1.1/developing/usage/bucket-types)
...
documentation on [object deletion]({{<baseurl>}}openriak-kv/2.1.1/using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` --- The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{<baseurl>}}openriak-kv/2.1.1/developing/api/http)) along with a generic error message about why the write was blocked.
