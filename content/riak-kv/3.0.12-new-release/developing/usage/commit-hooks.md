...
aliases:
  ...
  - /openriak-kv/3.0.12/dev/using/commit-hooks
...
Pre- and post-commit hooks are applied at the [bucket]({{<baseurl>}}openriak-kv/3.0.12/learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{<baseurl>}}openriak-kv/3.0.12/developing/usage/bucket-types)
...
documentation on [object deletion]({{<baseurl>}}openriak-kv/3.0.12/using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{<baseurl>}}openriak-kv/3.0.12/developing/api/http)) along with a generic error message about why the write was blocked.
