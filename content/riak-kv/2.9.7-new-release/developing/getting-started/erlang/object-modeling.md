...
aliases:
  ...
  - /openriak-kv/2.9.7/dev/taste-of-riak/object-modeling-erlang
...
We'll be using the bucket `Users` to store our data. We won't be [using bucket types]({{<baseurl>}}openriak-kv/2.9.7/developing/usage/bucket-types) here, so we don't need to specify one.
