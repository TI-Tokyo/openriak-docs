...
your Riak cluster (which is different from [recovering a failed node]({{<baseurl>}}openriak-kv/3.0.11/using/repair-recovery)). Here is the recommended way to go
...
3. Start the new `riak7` node with [`riak start`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-cli/#start):
...
participating in the cluster; for example `riak0` with the [`riak admin cluster join`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#cluster) command executed on the new `riak7` node:
...
`riak7` node using the [`riak admin cluster replace`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#cluster) command:
...
6. Examine the proposed cluster changes with the [`riak admin cluster plan`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#cluster) command executed on the new
`riak7` node:
...
[`riak admin cluster commit`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#cluster) command:

    ```bash
    riak admin cluster commit
    ```

    If you need to clear the proposed plan and start over, use [`riak admin cluster clear`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#cluster):
...
with the [`riak admin ringready`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#ringready)
and [`riak admin member-status`]({{<baseurl>}}openriak-kv/3.0.11/using/admin/riak-admin/#member-status)
commands.
