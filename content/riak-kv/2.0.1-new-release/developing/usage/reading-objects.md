...
object that's stored there---including its [siblings]({{<baseurl>}}openriak-kv/2.0.1/developing/usage/conflict-resolution/#siblings) \(more on that later)---or it returns `not found` (the
...
`dogs`, which bears the [bucket type]({{<baseurl>}}openriak-kv/2.0.1/developing/usage/bucket-types) `animals`:
