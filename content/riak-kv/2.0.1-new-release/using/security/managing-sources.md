...
aliases:
  ...
  - /openriak-kv/2.0.1/ops/running/security-sources
linkTitle: "Managing Security Sources"
weight: 101
---

If you're looking for more general information on Riak Security, it may
be best to start with our general guide to [authentication and authorization]({{<baseurl>}}openriak-kv/2.0.1/using/security/basics).
...
[created]({{<baseurl>}}openriak-kv/2.0.1/using/security/basics/#user-management) and that
security has been [enabled]({{<baseurl>}}openriak-kv/2.0.1/using/security/basics/#the-basics).
...
interface, and can be used only through the [protocol buffers interface]({{<baseurl>}}openriak-kv/2.0.1/developing/api/protocol-buffers/).
...
a different directory in your [`riak.conf`]({{<baseurl>}}openriak-kv/2.0.1/configuring/reference/) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`]({{<baseurl>}}openriak-kv/2.0.1/configuring/reference/) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your Riak cluster, you need to perform a [rolling restart]({{<baseurl>}}openriak-kv/2.0.1/using/repair-recovery/rolling-restart/). Once that process is complete, you can use the client
