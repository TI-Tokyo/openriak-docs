...
description: 'Explain conflict-free replicated data types, its trade-offs, and its effect on application design.'
weight: 5
...
Explain conflict-free replicated data types, its trade-offs, and its effect on application design.
...
[data types converg]: {{< product-version-root >}}explanation/data-model/distributed-data-types/#convergence
[crdts reading list]: http://christophermeiklejohn.com/crdt/2014/07/22/readings-in-crdts.html
[data types impl]: {{< product-version-root >}}explanation/data-model/distributed-data-types/#implementation
[concept causal context dvv]: {{< product-version-root >}}explanation/data-model/causal-context/#dotted-version-vectors
[concept causal context sib]: {{< product-version-root >}}explanation/data-model/causal-context/#siblings
[concept causal context vc]: {{< product-version-root >}}explanation/data-model/causal-context/#vector-clocks
[concept eventual consistency]: {{< product-version-root >}}explanation/consistency/eventual-consistency/
[concept strong consistency]: {{< product-version-root >}}explanation/consistency/strong-consistency/
[dev data types]: {{< product-version-root >}}reference/data/distributed-data-types/
[riak_dt]: https://github.com/basho/riak_dt
[dev data types context]: {{< product-version-root >}}reference/data/distributed-data-types/#data-types-and-context
[glossary node]: {{< product-version-root >}}explanation/foundations/glossary/#node
...
The difference between Riak Data Types and typical key/value data stored in OpenRiak KV is that Riak Data Types are operations-based from the standpoint of OpenRiak KV clients.
...
will be ignored by OpenRiak KV. Sets can be used either on their own or
embedded in a map.
...
[Conflict resolution][usage conflict resolution] in OpenRiak KV can be difficult because it involves reasoning about concurrency, [eventual consistency][concept eventual consistency], [siblings][concept causal context sib], and other issues that many other databases don't require you to consider.

One of the core purposes behind data types is to relieve developers
using OpenRiak KV of the burden of producing data convergence at the
application level by absorbing a great deal of that complexity into OpenRiak KV
itself. OpenRiak KV manages this complexity by building eventual consistency
into the data types themselves instead of requiring clients to do so.

You can still build applications with OpenRiak KV that treat it as a highly
...
timestamps, [vector clocks][concept causal context vc], [dotted version vectors][concept causal context dvv], or some other means. With data types, conflicts are resolved by OpenRiak KV itself, using a subsystem called [`riak_dt`][riak_dt].

#### Convergence

The benefit of data types is that OpenRiak KV knows how to resolve value
conflicts by applying data type-specific rules.

OpenRiak KV does this by remembering the history of a value and broadcasting that
history along with the current value in the form of a [context object][dev data types context] that is similar to a [vector clock][concept causal context vc] or [dotted version vectors][concept causal context dvv]. OpenRiak KV uses the history of each data type to make deterministic judgments about which value should be deemed correct.
...
In this case OpenRiak KV would declare the set with two elements the winner.
...
1. Check for divergence. If the data types have the same value, OpenRiak KV
   does nothing. But if divergence is noted...
2. OpenRiak KV applies data type-specific merge rules, like in the `fruits`
...
Convergence means that data type conflicts are weighted in a certain direction. OpenRiak's Data Types have their own internal weights that dictate what happens in case of conflict:
...
In a production OpenRiak KV cluster being hit by lots and lots of concurrent
