...
description: 'Explain causal context, its trade-offs, and its effect on application design.'
weight: 3
...
Explain causal context, its trade-offs, and its effect on application design.
...
[dev key value]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[glossary read rep]: {{< product-version-root >}}explanation/foundations/glossary/#read-repair
[perf latency reduc]: {{< product-version-root >}}how-to/tune/reduce-latency/
...
[usage protocol buffers]: {{< product-version-root >}}reference/protocol-buffers/
[usage updating objects]: {{< product-version-root >}}how-to/develop/update-object/
...
In the case of outcome 2, the choice between **a**, **b** and **c** is determined by settings. If you set the `allow_mult` parameter to `true` for a [bucket type]({{< product-version-root >}}how-to/develop/use-bucket-types/), all non-CRDT writes to that bucket type will create siblings in the case of concurrent writes (and occasionally under other
...
* [Conflict Resolution][usage conflict resolution] in OpenRiak KV
...
conflict resolution strategy in your OpenRiak cluster, using DVVs in some
