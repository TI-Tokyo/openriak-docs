---
title: Authentication messages
description: Authentication is a connection-level exchange. A client requests TLS upgrade, verifies the server,
  then sends the authentication request before ordinary operations. Use a client that implements this sequence;
  sending cre
weight: 590
diataxis: reference
product: OpenRiak KV
product_version: 3.4.0
status: editorially-rewritten
draft: true
audience:
- developers
source_material:
- legacy-3.2.5
- live-3.2.5
- proposed-kv
legacy_3_2_5_sources:
- C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\developing\api\protocol-buffers\auth-req.md
tags:
- diataxis
- kv
- reference
editorial_review: complete
technical_review: required
last_reviewed: '2026-09-22'
review_scope: diataxis-content-and-navigation
restructured_from:
- reference/protocol-buffers/authentication.md
related:
- how-to/security/authenticate-an-application-client
- how-to/security/configure-authentication-sources
- reference/protocol-buffers-api/protocol-framing-message-codes-and-errors
- foundations/data-and-consistency/objects-keys-and-buckets
- foundations/data-and-consistency/causality-version-vectors-and-siblings
---

Authentication is a connection-level exchange. A client requests TLS upgrade, verifies the server, then sends the authentication request before ordinary operations. Use a client that implements this sequence; sending credentials as an ordinary object request does not authenticate the connection.

## Message contract

{{< protocol-message name="RpbStartTls" >}}

{{< protocol-message name="RpbAuthReq" >}}

{{< protocol-message name="RpbAuthResp" >}}

## Framing and failures

All messages use the framing and error response in [Protocol framing, message codes, and errors]({{< product-version-root >}}reference/protocol-buffers-api/protocol-framing-message-codes-and-errors/). Field cardinality and explicitly declared schema defaults are shown above; omitted request options can also be resolved by bucket policy or the server. A `bytes` value is not automatically UTF-8 text.
