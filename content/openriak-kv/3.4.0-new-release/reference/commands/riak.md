---
title: 'riak command reference'
description: 'List every supported riak command with syntax, options, output, and exit behavior.'
weight: 4
diataxis: 'reference'
product: 'OpenRiak KV'
...
legacy_3_2_5_sources:
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\using\admin\riak-cli.md'
...
List every supported riak command with syntax, options, output, and exit behavior.

## Details

### riak Command Line Interface

[configuration file]: {{< product-version-root >}}reference/configuration/
[escript]: http://www.erlang.org/doc/man/escript.html
[`riak admin`]: {{< product-version-root >}}reference/commands/riak-admin/#top
[configuration]: {{< product-version-root >}}reference/configuration/

#### riak

This is the primary script for controlling the processes associated with an OpenRiak node. Running the `riak` command by itself will output a listing of available commands:
...
In OpenRiak KV 3.2.5, riak daemon replaces riak start.

Starts the OpenRiak node in the background. If the node is already started, you will receive the message `Node is already running!` If the node is not already running, no output will be given.
...
Stops the running OpenRiak node. Prints `ok` when successful or `Node <nodename> not responding to pings.` when the node is already stopped or not responding.
...
Stops and then starts the running OpenRiak node without exiting the Erlang VM.
...
Checks that the OpenRiak node is running. Prints `pong` when successful or `Node <nodename> not responding to pings.` when the node is stopped or not responding.
...
Starts the OpenRiak node in the foreground, giving access to the Erlang shell and
...
Attaches to the console of an OpenRiak node running in the background, giving access to the Erlang shell and runtime messages. Prints `Node is not running!` when the node cannot be reached.
...
Note: `riak getpid` has been replaced by `riak pid` in OpenRiak KV 3.0.1 onwards.
...
Provides information about the current [configuration][configuration] of an OpenRiak node, i.e. the parameters and values in the node's riak.conf configuration.
...
```
    ...
    Specifies the storage engine used for OpenRiak's key-value data
...
Adding the `-l debug` flag to any `riak config` command will produce additional debugging information that can be used in advanced troubleshooting of "cuttlefish", OpenRiak's configuration subsystem.
