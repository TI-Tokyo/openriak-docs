---
title: 'List resources with the HTTP API'
...
List available HTTP resources for the OpenRiak node. This can be used by clients to
automatically recognize the location of the resources for specific operations.

The standard resources are:

* `riak_kv_wm_buckets` - [Bucket Operations]({{< product-version-root >}}reference/http-api/)
* `riak_kv_wm_index` - [HTTP Secondary Indexes]({{< product-version-root >}}reference/http-api/secondary-indexes/)
* `riak_kv_wm_link_walker` - [HTTP Link Walking]({{< product-version-root >}}reference/http-api/link-walking/)
* `riak_kv_wm_mapred` - [HTTP MapReduce]({{< product-version-root >}}reference/http-api/mapreduce/)
* `riak_kv_wm_object`- [Object/Key Operations]({{< product-version-root >}}reference/http-api/)
* `riak_kv_wm_ping` - [HTTP Ping]({{< product-version-root >}}reference/http-api/ping/)
* `riak_kv_wm_props` - [HTTP Set Bucket Properties]({{< product-version-root >}}reference/http-api/set-bucket-properties/)
* `riak_kv_wm_stats` - [HTTP Status]({{< product-version-root >}}reference/http-api/status/)
