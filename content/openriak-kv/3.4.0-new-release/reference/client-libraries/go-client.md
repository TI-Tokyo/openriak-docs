---
title: Go client
description: The Go client example uses the HTTP API through the language's HTTP library. It does not depend on
...
restructured_from:
- reference/client-libraries/golang.md
related:
- tutorials/first-application/build-a-small-application-with-go
...
The Go client example uses the HTTP API through the language's HTTP library. It does not depend on a Riak-specific Protocol Buffers client.

## Transport and runtime

The accompanying example targets **Go 1.22 or newer** and uses [this HTTP client interface](https://pkg.go.dev/net/http). Object addressing, headers, and response statuses follow the shared HTTP reference.
