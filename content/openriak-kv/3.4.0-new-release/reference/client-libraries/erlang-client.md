---
title: Erlang client
description: The Erlang client example uses the HTTP API through the language's HTTP library. It does not depend
  on a Riak-specific Protocol Buffers client.
weight: 1140
diataxis: reference
product: OpenRiak KV
...
restructured_from:
- reference/client-libraries/erlang.md
related:
- tutorials/first-application/build-a-small-application-with-erlang
...
The Erlang client example uses the HTTP API through the language's HTTP library. It does not depend on a Riak-specific Protocol Buffers client.

## Transport and runtime

The accompanying example targets **Erlang/OTP 26** and uses [this HTTP client interface](https://www.erlang.org/doc/apps/inets/httpc.html). Object addressing, headers, and response statuses follow the shared HTTP reference.
