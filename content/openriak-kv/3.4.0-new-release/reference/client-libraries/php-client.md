---
title: PHP client
description: The PHP client example uses the HTTP API through the language's HTTP library. It does not depend on
...
restructured_from:
- reference/client-libraries/php.md
related:
- tutorials/first-application/build-a-small-application-with-php
...
The PHP client example uses the HTTP API through the language's HTTP library. It does not depend on a Riak-specific Protocol Buffers client.

## Transport and runtime

The accompanying example targets **PHP 8.3 with URL stream wrappers enabled** and uses [this HTTP client interface](https://www.php.net/manual/en/context.http.php). Object addressing, headers, and response statuses follow the shared HTTP reference.
