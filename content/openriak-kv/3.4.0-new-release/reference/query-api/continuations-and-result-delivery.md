...
Small-page Query API requests reproduced a query-server failure on the 3.4.0 Alpine 3.24 / OTP 26 learning image when a vnode returned an empty result batch. The request timed out instead of returning its next page. For a bounded, simple index range, the [legacy secondary-index HTTP endpoint]({{< product-version-root >}}reference/http-api/secondary-index-queries/) and its JSON continuation provide a temporary workaround. That endpoint is deprecated from 3.4; it is a compatibility fallback, not the preferred interface for new applications. See [Retrieve a larger result set]({{< product-version-root >}}tutorials/indexes-and-querying/retrieve-a-larger-result-set/) for a working exercise. Do not retry the same failing query indefinitely or treat the timeout as the end of the result set.
