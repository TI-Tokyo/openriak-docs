---
title: 'Fetch an object with Protocol Buffers'
...
[`RpbPutResp`]({{< product-version-root >}}reference/protocol-buffers/store-object/), respectively):
...
```protobuf
    ...
    `RpbPair` messages are also used to attach [secondary indexes]({{< product-version-root >}}how-to/develop/query-secondary-indexes/) to objects (in the optional
    `indexes` field).
