---
title: 'Store a data type with Protocol Buffers'
...
A request to update the value of a [Riak Data Type]({{< product-version-root >}}reference/data/distributed-data-types/).
...
performed. More on that in the [PBC Data Type Union]({{< product-version-root >}}reference/protocol-buffers/union-data-type/) document.
...
`type` | The bucket type of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map). Learn more about [using bucket types]({{< product-version-root >}}how-to/operate/manage-bucket-types/).
...
[counter]({{< product-version-root >}}reference/protocol-buffers/update-counter/), [set]({{< product-version-root >}}reference/protocol-buffers/update-set/), or [map]({{< product-version-root >}}reference/protocol-buffers/update-map/).
...
per-bucket basis. Please refer to the documentation on [setting bucket properties]({{< product-version-root >}}eference/protocol-buffers/set-bucket-properties/) for more information.
...
`context` | The opaque binary "context" that informs Riak which version of a data type the client has seen, analogous to [vector clocks]({{< product-version-root >}}foundations/foundations/glossary/#vector-clock)
...
[`RpbPutResp`]({{< product-version-root >}}reference/protocol-buffers/store-object/) for KV operations. If the
