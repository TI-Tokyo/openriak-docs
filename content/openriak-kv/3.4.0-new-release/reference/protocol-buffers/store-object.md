---
title: 'Store an object with Protocol Buffers'
...
intended [key]({{< product-version-root >}}explanation/data-model/keys-objects-and-buckets/), [bucket]({{< product-version-root >}}explanation/data-model/keys-objects-and-buckets/), and [bucket type]({{< product-version-root >}}how-to/develop/use-bucket-types/). A bucket must always be specified (via
...
[bucket type]({{< product-version-root >}}how-to/develop/use-bucket-types/) is assigned, Riak will assign
`default`, which means that the [default bucket configuration]({{< product-version-root >}}reference/configuration/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object]({{< product-version-root >}}reference/protocol-buffers/fetch-object/)

###### Optional Parameters

**Note on defaults and special values**
All of the optional parameters below have default values determined on a
per-bucket basis. Please refer to the documentation on [setting bucket properties]({{< product-version-root >}}reference/protocol-buffers/) for more information.
...
`vclock` | Opaque vector clock provided by an earlier <code><a href="../{{< product-version-root >}}explanation/data-model/causal-context/">RpbGetResp</a></code> message. Omit if this is a new key or if you deliberately want to create a sibling.
...
only for messages sent between nodes in an OpenRiak cluster and should not be
set by Riak clients.
...
as well as the object's [causal context]({{< product-version-root >}}explanation/data-model/causal-context/), in the `vclock`
