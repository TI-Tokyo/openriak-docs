---
title: 'Fetch a data type with Protocol Buffers'
...
The equivalent of [`RpbGetReq`]({{< product-version-root >}}reference/protocol-buffers/fetch-object/) for [Riak Data Types]({{< product-version-root >}}reference/data/distributed-data-types/). This request results in a `DtFetchResp`
...
`type` | The [Using Bucket Types]({{< product-version-root >}}how-to/operate/manage-bucket-types/) of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map)
...
per-bucket basis. Please refer to the documentation on [setting bucket properties]({{< product-version-root >}}reference/protocol-buffers/set-bucket-properties/) for more information.
...
The response to a fetch request ([`DtFetchReq`]({{< product-version-root >}}reference/protocol-buffers/fetch-data-type/)) is a `DtFetchResp` message.
