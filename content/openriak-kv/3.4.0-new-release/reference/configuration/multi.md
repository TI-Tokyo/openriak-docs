...
Define exactly what **Multi-backend configuration properties** covers in OpenRiak KV {{< current-version >}} and what belongs in neighbouring reference pages. State the component, interface, file, command, or data type being documented and the supported context in which it is available.
...
- [Configuration reference]({{< product-version-root >}}reference/configuration/) — Define the scope, defaults, units, validation rules, and version applicability of OpenRiak settings.
