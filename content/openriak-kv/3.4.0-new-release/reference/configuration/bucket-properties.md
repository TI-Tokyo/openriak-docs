...
When using [conflict-free replicated data types]({{< product-version-root >}}reference/specialized-apis/data-type-api/), `{allow_mult, true}` must always be used.
...
The `aae_tree_exclude` bucket property may be cached by processes within a cluster, so changing the property will not have immediate effect.  A change to the `aae_tree_exclude` property should be coordinated with a [rolling restart]({{< product-version-root >}}how-to/operate/rolling-restart/).
...
Although configuring `pr`/`pw` to values greater than 1 may be used to indirectly set stronger data reliability guarantees, or to adjust consistency guarantees - there are better ways of achieving this in Riak, which have fewer negative side effects.  Consider using [`node_confirms`]({{< product-version-root >}}reference/configuration/bucket-properties/) or [`sync_on_write`]({{< product-version-root >}}reference/configuration/bucket-properties/) to manage data reliability.  The use of [token-based conditional PUTs]({{< product-version-root >}}reference/http-api/conditional-requests/) is the preferred approach, rather than `pr`/`pw` adjustments for tuning consistency.
