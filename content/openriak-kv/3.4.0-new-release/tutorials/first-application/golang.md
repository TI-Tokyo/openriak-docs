...
If you haven't set up an OpenRiak node and started it, please visit [Running A Cluster]({{< product-version-root >}}how-to/operate/) first and ensure you have
...
[CRUD Operations]({{< product-version-root >}}tutorials/first-application/golang/)
...
We'll be using the bucket `Users` to store our data. We won't be [using bucket types]({{< product-version-root >}}how-to/develop/use-bucket-types/) here, so we don't need to specify one.
