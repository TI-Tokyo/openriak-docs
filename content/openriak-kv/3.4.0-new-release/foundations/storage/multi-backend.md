---
title: 'How multiple backends work'
description: 'Explain how multiple backends work, its constraints, and the workloads for which it is appropriate.'
weight: 8
...
Explain how multiple backends work, its constraints, and the workloads for which it is appropriate.
...
[use admin riak admin cli]: {{< product-version-root >}}reference/commands/riak/ admin
...
You can set up your cluster to use the Multi backend using OpenRiak's
...
> [!WARNING]
> Migration review required: Commands or links derived from the 3.2.5 documentation were version-normalized for {{< current-version >}} and require technical verification; Legacy version text or MDX syntax remains and requires editorial review; The multi backend is deprecated in OpenRiak KV 3.4 unless every configured backend is Bitcask.
