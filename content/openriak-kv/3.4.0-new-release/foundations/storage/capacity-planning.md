...
Riak System]({{< product-version-root >}}foundations/storage/) for more information.
...
Deploying a single OpenRiak cluster across two datacenters is not recommended. If your use case requires this capability, Riak offers a [Multi Data Center Replication: Architecture]({{< product-version-root >}}foundations/replication/multi-datacenter-architecture/) option that is built to keep multiple Riak clusters in
