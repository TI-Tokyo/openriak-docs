---
title: 'OpenRiak use cases'
description: 'Explain openriak use cases and why it matters when designing or operating OpenRiak systems.'
weight: 7
...
legacy_3_2_5_sources:
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\learn\use-cases.md'
source_material:
  - 'legacy-3.2.5'
  - 'live-3.2.5'
...
Explain openriak use cases and why it matters when designing or operating OpenRiak systems.

## Overview

### Use Cases For OpenRiak KV

[dev data model articles etc]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model log data]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model sensor data]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model serve advertisements]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model sess storage]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model user acct]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model user events]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
[dev data model user settings]: {{< product-version-root >}}how-to/plan/map-data-to-objects/
...
[replication properties]: {{< product-version-root >}}foundations/replication/references-and-triggers/
[usage mapreduce]: {{< product-version-root >}}how-to/develop/run-mapreduce/
[usage secondary-indexes]: {{< product-version-root >}}how-to/develop/query-secondary-indexes/
...
with predictably low latency. OpenRiak's content-type agnosticism also imposes no
...
session expiry. OpenRiak's [MapReduce][usage mapreduce] system can also be used to
...
OpenRiak's tunable [apps replication properties][replication properties] can be set
...
use OpenRiak's MapReduce system for aggregation tasks, such as summing the counts of
...
users might consider doing primary storage of logs in an OpenRiak cluster and then
replicating data to a secondary cluster to run heavy analytics jobs, either over
another OpenRiak cluster or another solution such as Hadoop. Because the access
...
OpenRiak's scalable design makes it useful for data sets, like sensor data, that
...
If you find your application frequently writing to the user account or have
...
won't "know" about the object unless it is indexed [using secondary indexes][usage secondary-indexes].
...
buckets, e.g. a bucket for comments that would be stored in the OpenRiak cluster
