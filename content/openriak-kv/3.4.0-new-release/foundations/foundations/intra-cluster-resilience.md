...
Explain the question that **Intra-cluster data resilience** answers for architects, developers, or operators using OpenRiak KV {{< current-version >}}. Establish the problem and vocabulary before describing the mechanism, and keep task instructions in linked how-to guides.
...
- [Availability and failure tolerance]({{< product-version-root >}}foundations/foundations/availability-and-failure-tolerance/) — Explain availability and failure tolerance and why it matters when designing or operating OpenRiak systems.
