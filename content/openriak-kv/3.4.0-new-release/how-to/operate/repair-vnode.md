...
Repair uses handoffs, and so can be tracked as with other cluster change operations.  Once handoffs are complete, Tictac AAE should be re-enabled, e.g. by using `riak_client:tictacaae_resume_node().`.  Once Tictac AAE confirms all vnodes are in-sync - then [`participate_in_coverage` can be re-enabled]({{< product-version-root >}}reference/operations/remote-console/).
...
Where such corruption is limited to a leveled ledger, then [a repair via leveled rebuild]({{< product-version-root >}}how-to/operate/repair-leveled-store/) can be used to recover.  However, in other backends, or with a corruption in the leveled journal - it may be preferable to repair a whole vnode rather than wait for other anti-entropy processes to eventually resolve the impact of the corruption (by repairing each impacted object).

The process to [complete a full node repair]({{< product-version-root >}}how-to/troubleshoot/recover-failed-node/) can be targeted at an individual vnode to repair just that vnode.  To prompt the repair of an individual vnode, the partition number - the [integer identifier of a vnode]({{< product-version-root >}}foundations/foundations/clusters-rings-and-partitions/) - must be passed to the vnode repair function.  The vnode repair function (`riak_kv_vnode_repair/1`) can be called by using the [`remote_console`]({{< product-version-root >}}how-to/operate/use-remote-console/) or directly from the command line through the `riak eval` CLI call:
