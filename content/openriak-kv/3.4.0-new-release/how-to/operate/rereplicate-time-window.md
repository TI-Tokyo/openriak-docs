...
The aae_fold `repl_keys_range` will replicate any key within the defined range to the clusters consuming from a defined queue.  See the [AAE fold API documentation]({{< product-version-root >}}reference/aae-fold-api/) for more information on using `repl_key_range`.
