...
description: 'Introduce routine cluster administration and lifecycle procedures.'
weight: 1
diataxis: 'how-to'
product: 'OpenRiak KV'
...
legacy_3_2_5_sources:
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\using.md'
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\using\cluster-operations.md'
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\using\running-a-cluster.md'
source_material:
  - 'legacy-3.2.5'
...
Introduce routine cluster administration and lifecycle procedures.
...
[use admin index]: {{< product-version-root >}}reference/commands/
[cluster ops index]: {{< product-version-root >}}how-to/operate/
...
[troubleshoot index]: {{< product-version-root >}}how-to/troubleshoot/
[use ref]: {{< product-version-root >}}reference/operations/
...
Information on securing your OpenRiak KV cluster.

[Learn More >>][security index]

###### [Performance][perf index]

Articles on benchmarking your OpenRiak KV cluster and improving performance.
...
[ops add remove node]: {{< product-version-root >}}how-to/operate/
[ops change info]: {{< product-version-root >}}how-to/operate/
[ops replace node]: {{< product-version-root >}}how-to/operate/
[ops inspect node]: {{< product-version-root >}}how-to/operate/
[ops bucket types]: {{< product-version-root >}}explanation/data-model/bucket-types/
[ops log]: {{< product-version-root >}}how-to/operate/
[ops backup]: {{< product-version-root >}}how-to/operate/
[ops handoff]: {{< product-version-root >}}reference/operations/handoff/
[ops strong consistency]: {{< product-version-root >}}how-to/operate/
[ops v3 mdc]: {{< product-version-root >}}how-to/operate/
[ops v2 mdc]: {{< product-version-root >}}how-to/operate/

#### In This Section

##### [Adding / Removing Nodes][ops add remove node]

Describes the process of adding and removing nodes to and from an OpenRiak KV cluster.
...
Tutorial on replacing a node in an OpenRiak KV cluster.

[Learn More >>][ops replace node]

###### [Inspecting a Node][ops inspect node]

Guide on some of the available tools for inspecting an OpenRiak KV node.
...
Covers backing up OpenRiak KV data.
...
Configuring an OpenRiak cluster involves instructing each node to listen on a
non-local interface, i.e. not `127.0.0.1`, and then joining all of the
nodes together to participate in the cluster.

Most configuration changes will be applied to the [configuration file]({{< product-version-root >}}reference/configuration/) located in your `rel/riak/etc` directory (if
...
cluster again.
>
> *Rename the node using the [`riak admin cluster replace`]({{< product-version-root >}}reference/commands/riak-admin/) command. This will not work if you have previously only started Riak with a single node.

#### Configure the First Node

First, stop your OpenRiak node if it is currently running:
...
you'll be using the default port (8087). If you're using the [Protocol Buffers interface]({{< product-version-root >}}reference/protocol-buffers/) to Riak (which we recommend over the HTTP
...
[`riak admin cluster force-replace`]({{< product-version-root >}}reference/commands/riak-admin/) the node.
...
If the OpenRiak node has been previously started, you must use the
...
If your output was similar, then the second OpenRiak node is now part of the
...
examine your OpenRiak cluster's ring:
...
can also find more detailed instructions about [adding and removing nodes]({{< product-version-root >}}tutorials/operations/change-cluster-membership/) from a cluster.
...
with running an OpenRiak cluster.
...
1. Change the ports used by each OpenRiak node to be unique on your host.
...
- [The considerations to make when choosing infrastructure for Riak]({{< product-version-root >}}how-to/plan/size-cluster/)
- [The practical steps to actually make and expand a cluster]({{< product-version-root >}}how-to/operate/add-node/)
...
- [Handling failure - replace, repair and recover]({{< product-version-root >}}explanation/operations/node-failure-and-recovery/)
...
- [Garbage collection - monitoring and tuning]({{< product-version-root >}}explanation/operations/garbage-collection/)
...
- [Backing up a cluster]({{< product-version-root >}}explanation/operations/backups-and-restores/)
