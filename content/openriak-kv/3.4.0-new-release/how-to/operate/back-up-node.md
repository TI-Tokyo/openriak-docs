...
**Deprecation notice**
In previous versions of OpenRiak KV, there was a [`riak admin backup`]({{< product-version-root >}}reference/commands/riak-admin/#backup) command commonly used for
...
1. Mark the original instance down in the cluster using
   [`riak admin down <node>`]({{< product-version-root >}}reference/commands/riak-admin/#down)
2. Join the restored node to the cluster using
   [`riak admin cluster join <node>`]({{< product-version-root >}}reference/commands/riak-admin/)
3. Replace the original instance with the renamed instance with
   [`riak admin cluster force-replace <node1> <node2>`]({{< product-version-root >}}reference/commands/riak-admin/)
...
**Note:**
For more information on the `riak admin cluster` commands, refer to our documentation on [cluster administration]({{< product-version-root >}}reference/commands/).
...
Restoring a cluster from backups is documented [on its own page]({{< product-version-root >}}how-to/troubleshoot/recover-cluster-failure/#cluster-recovery-from-backups).
...
If an individual node fails, do not restore an individual node from backup.  It is generally much more efficient and reliable to use [the `repair` process]({{< product-version-root >}}how-to/operate/replace-node/) to recover data on a node.  It is not normal practice to keep backups simply for the purpose of restoring individual nodes, even where those nodes may rely on ephemeral disks.
...
If a cluster uses only the leveled backend, a hot backup may be taken across the cluster using the `riak_client:hotbackup/4` function from [the `remote_console`]({{< product-version-root >}}how-to/operate/use-remote-console/) on any node within the cluster.
