...
Because hooks are defined at the bucket level, you can create [bucket types]({{< product-version-root >}}how-to/develop/use-bucket-types/)
...
documentation on [object deletion]({{< product-version-root >}}reference/operations/object-deletion/)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{< product-version-root >}}reference/http-api/)) along with a generic error message about why the write was blocked.
