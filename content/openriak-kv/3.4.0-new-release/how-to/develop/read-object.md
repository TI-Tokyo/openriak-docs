...
object that's stored there---including its [siblings]({{< product-version-root >}}how-to/develop/resolve-conflicts/#siblings) \(more on that later)---or it returns `not found` (the
...
`dogs`, which bears the bucket type `animals`. Please note that for this example to work, you must have first created the bucket-type `animals` as per the instructions on the [bucket type]({{< product-version-root >}}how-to/operate/manage-bucket-types/) page.
