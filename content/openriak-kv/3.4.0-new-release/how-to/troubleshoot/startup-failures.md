...
Define the single practical outcome of **Troubleshoot startup failures**, the environments to which it applies, and the supported OpenRiak KV {{< current-version >}} starting state. Keep conceptual background brief and link to an explanation page instead of interrupting the procedure.
...
- [Troubleshoot unexpected or stale data]({{< product-version-root >}}how-to/troubleshoot/unexpected-or-stale-data/) — Show practitioners how to troubleshoot unexpected or stale data from evidence gathering through verification.
