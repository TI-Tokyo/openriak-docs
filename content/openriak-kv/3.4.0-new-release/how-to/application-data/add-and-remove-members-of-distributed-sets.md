---
title: Add and remove members of distributed sets
description: Update a distributed set through the data-type API. Set `RIAK_HTTP` to the intended endpoint and include
...
- C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\developing\data-types\sets.md
tags:
- diataxis
- kv
- how-to
...
Update a distributed set through the data-type API. Set `RIAK_HTTP` to the intended endpoint and include the deployment's authentication options.

## Prepare the bucket type

Create the type if it does not already exist, inspect its properties, and activate it:

{{< cli-example key="shell:riak admin bucket-type create" args=`sets '{"props":{"datatype":"set"}}'` >}}
{{< cli-example key="shell:riak admin bucket-type status" args="sets" >}}
{{< cli-example key="shell:riak admin bucket-type activate" args="sets" >}}
...
```sh
curl --fail -i -X POST "$RIAK_HTTP/types/sets/buckets/example/datatypes/sample" -H 'Content-Type: application/json' --data-binary '{"add_all":["music","walking"]}'
curl --fail "$RIAK_HTTP/types/sets/buckets/example/datatypes/sample"
```

For removal, fetch the set first and include its returned context with `remove_all`. A remove can remove only the additions the client has observed; test concurrent add/remove behaviour.
