---
title: Estimate distinct values with HyperLogLog
description: Update a distributed hll through the data-type API. Set `RIAK_HTTP` to the intended endpoint and include
...
- C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\developing\data-types\hyperloglogs.md
tags:
- diataxis
- kv
- how-to
...
{{< cli-example key="shell:riak admin bucket-type create" args=`hlls '{"props":{"datatype":"hll"}}'` >}}
{{< cli-example key="shell:riak admin bucket-type status" args="hlls" >}}
{{< cli-example key="shell:riak admin bucket-type activate" args="hlls" >}}
...
```sh
curl --fail -i -X POST "$RIAK_HTTP/types/hlls/buckets/example/datatypes/sample" -H 'Content-Type: application/json' --data-binary '{"add_all":["visitor-a","visitor-b","visitor-a"]}'
curl --fail "$RIAK_HTTP/types/hlls/buckets/example/datatypes/sample"
```

The returned value estimates distinct membership; it does not return the original members. Use stable identifiers and test the required error tolerance. Repeating an identifier does not mean a second distinct visitor.

## Verify concurrent behaviour

Run two clients against a disposable key, then fetch the converged value. Preserve opaque context returned by the data-type API; it is not interchangeable with an ordinary object's vector-clock header. See [Distributed data-type operations]({{< product-version-root >}}reference/http-api/distributed-data-type-operations/) for response fields and operations, and [Estimate unique activity with HyperLogLog]({{< product-version-root >}}tutorials/data-and-concurrency/estimate-unique-activity-with-hyperloglog/) for a guided exercise.
