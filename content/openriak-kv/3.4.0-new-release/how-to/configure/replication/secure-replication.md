...
**Deprecation Warning**
v2 Multi-Datacenter Replication is deprecated and will be removed in a future version. Please use [v3]({{< product-version-root >}}how-to/configure/replication/secure-replication/) instead.
...
[config reference#advanced.config]: {{< product-version-root >}}reference/configuration/
...
The real-time connections may be secured, by [enabling security on the source cluster]({{< product-version-root >}}how-to/secure/).  The securing of communications for replication is supported only with the PB transport in Riak 3.4.
