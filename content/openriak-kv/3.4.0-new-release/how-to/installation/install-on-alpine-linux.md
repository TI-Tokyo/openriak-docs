---
title: Install on Alpine Linux
description: Install a published OpenRiak KV package on Alpine Linux. Use a fresh node for a new deployment; use
  [[H78]] when changing a member of an existing cluster.
weight: 150
diataxis: how-to
product: OpenRiak KV
...
restructured_from:
- how-to/install/alpine-linux.md
related:
...
Install a published OpenRiak KV package on Alpine Linux. Use a fresh node for a new deployment; use [Upgrade an OpenRiak KV cluster]({{< product-version-root >}}how-to/cluster-lifecycle/upgrade-a-cluster/) when changing a member of an existing cluster.
...
For Alpine, install the publisher’s signing public key into `/etc/apk/keys` through your trusted provisioning process before installing the APK. Do not bypass signature verification.

Download the package and its published checksum, verify the checksum, then replace `PACKAGE.apk` with the downloaded filename. Let the package manager resolve dependencies from the distribution's configured repositories.
...
{{< service-command action="stop" os="alpine" >}}
...
{{< service-command action="start" os="alpine" >}}
