---
title: 'Tune the Erlang virtual machine'
description: 'Show performance engineers how to tune the erlang virtual machine using measurable before-and-after checks.'
weight: 6
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show performance engineers how to tune the erlang virtual machine using measurable before-and-after checks.
...
itself provides a wide variety of [configurable parameters](http://erlang.org/doc/man/erl.html) that you can use to tune its performance; Riak enables you to tune a subset of those parameters in each node's [configuration files]({{< product-version-root >}}eference/configuration/node/#erlang-vm).
...
use by OpenRiak's Erlang VM. If it does not, you should set `erlang.smp` to
...
available port. This can make it difficult to configure [firewalls]({{< product-version-root >}}how-to/).
...
default on OpenRiak's Erlang VM, i.e. the default for `erlang.K` is `on`.
...
By default, crash dumps from OpenRiak's Erlang distribution are deposited in
...
OpenRiak cluster, one of the functions of the net kernel is to periodically
