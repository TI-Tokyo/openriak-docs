...
description: 'Show performance engineers how to benchmark a cluster using measurable before-and-after checks.'
weight: 2
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show performance engineers how to benchmark a cluster using measurable before-and-after checks.
...
* Erlang must be installed. See [Installing Erlang]({{< product-version-root >}}how-to/install/source/) for instructions
...
* `basho_bench_driver_http_raw` - Uses OpenRiak's HTTP interface to
    get/update/insert data on a Riak server
* `basho_bench_driver_riakc_pb` - Uses OpenRiak's Protocol Buffers
    interface to get/put/update/delete data on a Riak serve
* `basho_bench_driver_riakclient` - Uses OpenRiak's Distributed Erlang
