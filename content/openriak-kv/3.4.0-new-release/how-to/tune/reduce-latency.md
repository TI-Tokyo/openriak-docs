...
description: 'Show performance engineers how to reduce request latency using measurable before-and-after checks.'
weight: 3
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show performance engineers how to reduce request latency using measurable before-and-after checks.
...
[Riak CS](/docs/openriak-cs/2.1.3/), which is intended as a storage system for large objects.

##### Mitigation

The best way to find out if large objects are impacting latency is to
monitor each node's object size stats. If you run [`riak admin status`]({{< product-version-root >}}reference/commands/riak-admin/#status) or make an HTTP `GET` request
to OpenRiak's `/stats` endpoint, you will see the results for the following
...
the following changes to each node's [configuration]({{< product-version-root >}}reference/configuration/):
...
The best way to monitor siblings is through the same [`riak admin status`]({{< product-version-root >}}reference/commands/riak-admin/#status) interface used to monitor
...
* If `allow_mult` is set to `true` for some or all of your buckets, be
  ...
  read our documentation on [conflict resolution]({{< product-version-root >}}how-to/develop/resolve-conflicts/) for a fuller picture of how this can be done. **Note**: In Riak versions 2.0 and later, `allow_mult` is set to `true` by default for all bucket types that you create and activate.
...
The [Bitcask]({{< product-version-root >}}how-to/configure/backends/bitcask/) and [LevelDB]({{< product-version-root >}}how-to/configure/backends/leveldb/) storage backends occasionally go through
...
[tuning Bitcask]({{< product-version-root >}}how-to/configure/backends/bitcask//#tuning-bitcask).
...
`bitcask.max_file_size` in the newer, `riak.conf`-based [configuration files]({{< product-version-root >}}reference/configuration/) and `max_file_size` in the older, `app.config`-based system.
...
your use case, we recommend checking out our guide to [configuring Bitcask]({{< product-version-root >}}how-to/configure/backends/bitcask//#configuring-bitcask).
...
appropriately with LevelDB, check out our guide to [LevelDB parameter planning]({{< product-version-root >}}how-to/configure/backends/leveldb//#parameter-planning).
...
* [Open files limit]({{< product-version-root >}}how-to/tune/)
* General [System performance tuning](../)
* [AWS performance tuning]({{< product-version-root >}}how-to/tune/) if you're running Riak on [Amazon Web Services](http://aws.amazon.com/)
...
simultaneously on a single OpenRiak node. Increased latency can result if a
...
for [configuration files]({{< product-version-root >}}reference/configuration//) in Riak 2.0. If you are using the older, `app.config`-based system, you will not have access to
these settings.
