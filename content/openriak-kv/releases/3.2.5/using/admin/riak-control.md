...
enabled on each Riak node. For more information, see our [security documentation]({{< product-version-root >}}using/security/basics#enabling-ssl). Once SSL is enabled, you can proceed to setting up [authentication](#authentication) for Riak Control.
...
see the health of each [vnode]({{< product-version-root >}}learn/glossary/#vnode).
