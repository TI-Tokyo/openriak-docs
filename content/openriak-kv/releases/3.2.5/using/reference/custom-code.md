...
aliases:
  ...
  - /riak/kv/3.2.5/ops/advanced/install-custom-code/
---

Riak supports the use of Erlang named functions in compiled modules for
[pre/post-commit hooks]({{< product-version-root >}}developing/usage/commit-hooks), and MapReduce operations. This
doc contains installation steps with simple examples for each use case.

Your developers can compile [custom erlang code]({{< product-version-root >}}developing/usage/commit-hooks), which
...
in the [Commands documentation]({{< product-version-root >}}using/admin/riak-admin/#wait-for-service).
