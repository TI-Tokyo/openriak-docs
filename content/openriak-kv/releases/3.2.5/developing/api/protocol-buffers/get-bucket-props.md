...
toc: true
aliases:
  - /riak/3.2.5/dev/references/protocol-buffers/get-bucket-props
  - /riak/kv/3.2.5/dev/references/protocol-buffers/get-bucket-props
---
...
The bucket's name (`bucket`) must be specified. The [bucket type]({{< product-version-root >}}using/cluster-operations/bucket-types) parameter (`type`) is optional. If it is not specified,
...
[configuration file]({{< product-version-root >}}configuring/reference/#default-bucket-properties) documentation.
...
The `RpbReplMode` is of use only to users of Riak CS's [Multi-Datacenter Replication capabilities](https://www.tiot.jp/riak-docs/riak/cs/2.1.1/cookbooks/multi-datacenter-overview/)
{{% /note %}}
