...
aliases:
  ...
  - /riak/kv/3.2.5/dev/references/protocol-buffers/store-object
---

Stores an object under the specified location, as determined by the
intended [key]({{< product-version-root >}}learn/concepts/keys-and-objects), [bucket]({{< product-version-root >}}learn/concepts/buckets), and [bucket type]({{< product-version-root >}}developing/usage/bucket-types). A bucket must always be specified (via
...
[bucket type]({{< product-version-root >}}developing/usage/bucket-types) is assigned, Riak will assign
`default`, which means that the [default bucket configuration]({{< product-version-root >}}configuring/reference/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object]({{< product-version-root >}}developing/api/protocol-buffers/fetch-object)
...
as well as the object's [causal context]({{< product-version-root >}}learn/concepts/causal-context), in the `vclock`
