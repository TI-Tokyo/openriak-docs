...
is Riak's [bucket types]({{< product-version-root >}}using/reference/bucket-types) feature. This feature
...
made both to memory and to disk. Logical clocks, namely [dotted version vectors]({{< product-version-root >}}learn/concepts/causal-context/#dotted-version-vectors), are used in place of [vector clocks]({{< product-version-root >}}learn/concepts/causal-context/#vector-clocks) or timestamps to resolve value conflicts. Values stored as cluster metadata are opaque Erlang
