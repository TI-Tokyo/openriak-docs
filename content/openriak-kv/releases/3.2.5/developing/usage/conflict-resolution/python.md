...
aliases:
  ...
  - /riak/kv/3.2.5/dev/using/conflict-resolution/python
---

For reasons explained in the [Introduction to conflict resolution]({{< product-version-root >}}developing/usage/conflict-resolution), we strongly recommend adopting a
...
application will be stored in buckets that bear the [bucket type]({{< product-version-root >}}developing/usage/bucket-types) `siblings`, and for this bucket type `allow_mult` is set
...
from the official Python client library, in the [Developing with Riak KV: Usage]({{< product-version-root >}}developing/usage) section.
...
resolution is that Riak offers a variety of [Data Types]({{< product-version-root >}}developing/data-types/) that have
specific conflict resolution mechanics built in. If you have data that
can be modeled as a [counter]({{< product-version-root >}}developing/data-types/#counters), [set]({{< product-version-root >}}developing/data-types/#sets), or [map]({{< product-version-root >}}developing/data-types/#maps), then you should seriously
...
resolution that we implemented above, see our [tutorial on Riak sets]({{< product-version-root >}}developing/data-types/#sets).
