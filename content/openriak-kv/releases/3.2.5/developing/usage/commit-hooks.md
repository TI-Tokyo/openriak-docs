...
Pre- and post-commit hooks are applied at the [bucket]({{< product-version-root >}}learn/concepts/buckets) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{< product-version-root >}}developing/usage/bucket-types)
...
documentation on [object deletion]({{< product-version-root >}}using/reference/object-deletion)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{< product-version-root >}}developing/api/http)) along with a generic error message about why the write was blocked.
