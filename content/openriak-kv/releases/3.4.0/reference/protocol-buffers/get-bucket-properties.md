...
The bucket's name (`bucket`) must be specified. The [bucket type]({{< product-version-root >}}how-to/operate/manage-bucket-types/) parameter (`type`) is optional. If it is not specified,
...
[configuration file]({{< product-version-root >}}reference/configuration/#default-bucket-properties) documentation.
...
**Note on `RpbReplMode`**
The `RpbReplMode` is of use only to users of Riak CS's [Multi-Datacenter Replication capabilities](/docs/openriak-cs/2.1.3/)
