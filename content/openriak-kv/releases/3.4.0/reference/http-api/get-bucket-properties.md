...
default). See also [HTTP List Keys]({{< product-version-root >}}reference/http-api/list-keys/).
...
See [HTTP Set Bucket Properties]({{< product-version-root >}}reference/http-api/set-bucket-properties/) for more information about the available
bucket properties. See [Managing Bucket Types Through the Command Line]({{< product-version-root >}}reference/data/buckets-and-bucket-types/#managing-bucket-types-through-the-command-line) for more details about reading bucket types using  the `riak admin bucket-type` interface.
