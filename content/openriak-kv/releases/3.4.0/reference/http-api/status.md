---
title: 'Status with the HTTP API'
...
Reports about the performance and configuration of the OpenRiak node to which it was requested. You must have the `{riak_kv_stat,true}` configuration setting in app.config for this endpoint to be active.
...
The output of `/stats` contains the output of `riak admin status` detailed in the [Inspecting a Node]({{< product-version-root >}}how-to/operate/inspect-node-and-cluster/) doc, plus the below stats generated by the Riak Core application.
...
riak_kv_vnodes_running        | Number of local OpenRiak KV virtual nodes running
riak_kv_vnodeq_min            | Minimum queue size of all local OpenRiak KV virtual nodes in the last minute
riak_kv_vnodeq_median         | Median queue size of all local OpenRiak KV virtual nodes in the last minute
riak_kv_vnodeq_mean           | Mean queue size of all local OpenRiak KV virtual nodes in the last minute
riak_kv_vnodeq_max            | Max queue size of all local OpenRiak KV virtual nodes in the last minute
riak_kv_vnodeq_total          | Total queue size of all local OpenRiak KV virtual nodes in the last minute
