...
At the bottom of the page, you'll find a [screencast]({{< product-version-root >}}explanation/replication/references-and-triggers/#screencast) that briefly explains how to adjust your
replication levels to match your application and business needs.

> **Note on strong consistency**
>
> An option introduced in Riak version 2.0 is to use Riak as a [strongly consistent]({{< product-version-root >}}reference/specialized-apis/strong-consistency-api/) system for data in specified buckets. Using Riak in this way is fundamentally different from adjusting replication properties and fine-tuning the availability/consistency trade-off, as it sacrifices
_all_ availability guarantees when necessary. Therefore, you
should consult the [Using Strong Consistency]({{< product-version-root >}}reference/specialized-apis/strong-consistency-api/) documentation, as this option will not be covered
in this tutorial.
...
of 3, and a `w` of 3 to all of the data in some of the [buckets]({{< product-version-root >}}explanation/data-model/keys-objects-and-buckets/) that
...
[below]({{< product-version-root >}}explanation/replication/references-and-triggers/#symbolic-consistency-names). Each
...
`pr` | PR | `0` | The number of primary <a href="{{< product-version-root >}}explanation/foundations/virtual-nodes/">vnodes</a> that must respond to a read request
`pw` | PW | `0` | The number of primary <a href="{{< product-version-root >}}explanation/foundations/virtual-nodes/">vnodes</a> that must respond to a write request
...
check out the [section below]({{< product-version-root >}}explanation/replication/references-and-triggers/#client-level-replication-settings)
...
In OpenRiak's replication model, there are N [vnodes]({{< product-version-root >}}explanation/foundations/glossary/#vnode),
...
documentation on [Bitcask]({{< product-version-root >}}explanation/storage/bitcask/), [LevelDB]({{< product-version-root >}}explanation/storage/leveldb/), and [multiple backends]({{< product-version-root >}}explanation/storage/multi-backend/).
...
refer to the section on [development usage with OpenRiak KV]({{< product-version-root >}}how-to/develop/)
...
and siblings, so we recommend reading the [Vector Clocks]({{< product-version-root >}}explanation/data-model/causal-context/#vector-clocks) documentation for more information.
...
2 of 3 vnodes agree on the value, and those 2 are the first to reach the coordinating node | The value is returned to the client. Read repair will deal with the conflict per the later scenarios, which means that a future read may return a different value or <a href="{{< product-version-root >}}explanation/data-model/causal-context/#siblings">siblings</a>
2 conflicting values reach the coordinating node and <a href="{{< product-version-root >}}explanation/data-model/causal-context/#vector-clocks">vector clocks</a> allow for resolution | The vector clocks are used to resolve the conflict and return a single value, which is propagated via read repair to the relevant vnodes
...
Replication references may also be generated by [AAE folds]({{< product-version-root >}}reference/aae-fold-api/) when seeding clusters with data, or repairing large deltas.
...
Riak can be configured to operate in one of [three possible delete modes]({{< product-version-root >}}how-to/plan/choose-deletion-policy/).  Using a delete mode other than `keep`is tested with replication, but there will be an increased probability of false-negative reconciliation events that may consume resources on the cluster.
