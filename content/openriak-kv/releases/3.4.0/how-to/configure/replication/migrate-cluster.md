...
Once a Ring Size is set, there is no way of updating a cluster in-place.  However the ring-size can be updated using the [cluster migration process]({{< product-version-root >}}how-to/configure/replication/migrate-cluster/).
...
The [cluster migration process]({{< product-version-root >}}how-to/configure/replication/migrate-cluster/), can be used to migrate from a single cluster environment into a multi-cluster environment.
...
- [Initiate a new cluster]({{< product-version-root >}}how-to/plan/) with the correct configuration (e.g. alternative ring-size).
- Configure a [new real-time source queue]({{< product-version-root >}}how-to/configure/replication/configure-real-time-replication/) on the current cluster for the replacement cluster.
- Enable [sink workers on the new cluster]({{< product-version-root >}}how-to/configure/replication/configure-real-time-replication/) to begin to consume real-time changes.
- Use the [AAE folds]({{< product-version-root >}}reference/aae-fold-api/) `list_buckets` and `repl_range_keys` to queue up data to seed the new cluster;
  ...
  - [Tune the sink workers]({{< product-version-root >}}reference/replication-api/runtime-controls/) on the replacement cluster to avoid overloading the new cluster.
- Enable [reconciliation]({{< product-version-root >}}how-to/configure/replication/configure-fullsync/) on the replacement cluster.
