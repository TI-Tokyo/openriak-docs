...
OpenRiak's [active anti-entropy]({{< product-version-root >}}explanation/replication/active-anti-entropy/) \(AAE) subsystem is a set of background processes that repair object inconsistencies stemming from missing or divergent object values across nodes. Riak operators can turn AAE on and off and configure and monitor its functioning.
...
object inconsistencies using [read repair]({{< product-version-root >}}explanation/replication/active-anti-entropy/#read-repair-vs-active-anti-entropy) alone:
...
[vnode]({{< product-version-root >}}explanation/foundations/virtual-nodes/) in your cluster in these three categories:
...
- The `maxresults` limit controls the scope of repairs per exchange (a limit on [the segment IDs covered by an exchange]({{< product-version-root >}}explanation/replication/active-anti-entropy/)).
...
- A rebuild of a native vnode (i.e. with a single `leveled` backend), will rebuild the cached tree from the [leveled ledger]({{< product-version-root >}}explanation/storage/leveled/) keystore (but also checking for presence of the object in the journal).
