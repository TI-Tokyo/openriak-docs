...
description: 'Show operators how to inspect node and cluster health with prechecks, verification, and recovery guidance.'
weight: 8
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show operators how to inspect node and cluster health with prechecks, verification, and recovery guidance.
...
When inspection of an OpenRiak node to gather metrics on performance or
...
tools for inspecting an OpenRiak node.
...
Stat                | Description
--------------------|---------------------------------------------------
`riak_kv_stat_ts`   | The last time OpenRiak KV stats were generated.
...
makes up an OpenRiak node is present in the `riak admin status` output.  Each
...
`riak_kv_version`       | [OpenRiak KV](http://github.com/basho/riak_kv)
...
The `riak-debug` command is used to identify and diagnose common problems with your OpenRiak KV nodes.

`riak-debug` also runs `riak admin diag`, which runs a small suite of diagnostic checks against an OpenRiak KV node to discover common problems. It often offers recommendations about how to resolve those problems as well.
...
Riak tabulates a variety of stats related to OpenRiak's optional [strong consistency]({{< product-version-root >}}how-to/) feature. The table below lists those stats.
...
* `--user` - The user/UID as which the OpenRiak node runs. In a source
    checkout, it's the current user; on most systems, it's `riak`.

#### Related Resources

* [The riak admin configuration management tool]({{< product-version-root >}}eference/commands/riak-admin/)
* [Riaknostic](http://riaknostic.basho.com/)
* [HTTP API Status]({{< product-version-root >}}reference/http-api/status/)

[start]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[stop]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[restart]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[inspect]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[test]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[nodestatus]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[clusterstatus]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[member]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[clusterstatus]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[ringstatus]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
[debug]: {{< product-version-root >}}how-to/operate/inspect-node-and-cluster/
