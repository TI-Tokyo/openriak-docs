---
title: 'Change cluster identity information'
description: 'Show operators how to change cluster identity information with prechecks, verification, and recovery guidance.'
weight: 4
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show operators how to change cluster identity information with prechecks, verification, and recovery guidance.
...
the OpenRiak cluster need these node names to communicate and coordinate
with each other.
...
By default, OpenRiak's HTTP and Protocol Buffers services are bound to the
...
Previous to Riak version 1.2, a cluster node's name could only be changed with the [`riak admin reip`]({{< product-version-root >}}reference/commands/riak-admin/#reip) command, which involves downtime for the entire cluster. As of Riak version 1.2, that method has been superseded by [`riak admin cluster force-replace`]({{< product-version-root >}}reference/commands/riak-admin/), which is safer and does not require cluster wide downtime.
...
2. Change any IP addresses to *192.168.17.11* in `riak.conf` or `app.config` as previously described in step 3 of [Single Node Clusters]({{< product-version-root >}}how-to/operate/change-cluster-information/).

3. Rename the node's `ring` directory, the location of which is described in step 4 of [Single Node Clusters]({{< product-version-root >}}how-to/operate/change-cluster-information/).  You may rename it to whatever you like, as it will only be used as a backup during the node renaming process.
...
2. Move aside the ring directory. As in [Multi-Node Clusters]({{< product-version-root >}}how-to/operate/change-cluster-information/), we will save this ring directory as a backup until were finished.
