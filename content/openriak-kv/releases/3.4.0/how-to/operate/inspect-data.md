...
To understand more about the data being held in the cluster, information can be found using AAE folds. Refer to the [API guide for AAE Fold]({{< product-version-root >}}reference/aae-fold-api/) for information on triggering data inspection folds - `find_keys`, `find_tombs`, `list_buckets` and `object_stats`.
