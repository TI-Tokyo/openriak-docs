---
title: 'Enable OpenRiak security'
description: 'Show security engineers how to enable openriak security and test the resulting controls.'
weight: 3
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show security engineers how to enable openriak security and test the resulting controls.
...
features. For a look at network security in Riak, see [Security and Firewalls]({{< product-version-root >}}how-to/secure/manage-sources/).

As of version 2.0, Riak administrators can selectively apportion
access to a wide variety of OpenRiak's functionality, including accessing,
...
1. Check any Erlang MapReduce code for invocations of Riak modules other
   ...
   mechanism documented in [Installing Custom Code]({{< product-version-root >}}reference/operations/custom-code/).
1. Make sure that your client software will work properly:
    * It must pass authentication information with each request
    * It must support HTTPS or encrypted [Protocol Buffers]({{< product-version-root >}}reference/protocol-buffers/)
      traffic
    * If using HTTPS, the proper port (presumably 443) is open from
      client to server
    * Code that uses OpenRiak's deprecated link walking feature **will
...
Clients that use [Protocol Buffers]({{< product-version-root >}}reference/protocol-buffers/) will typically have to be
...
permissions) is granted/revoked for all buckets and [bucket types]({{< product-version-root >}}how-to/develop/use-bucket-types/). If you specify a bucket type only, then the permission
...
Permission to perform [MapReduce]({{< product-version-root >}}how-to/develop/run-mapreduce/) jobs can be assigned
...
In versions 2.0 and later, Riak users can manage [bucket types]({{< product-version-root >}}how-to/develop/use-bucket-types/) in addition to setting bucket properties. `riak admin
...
access to some or all of OpenRiak's functionality as described in the [User Management](#user-management) section,
you will then need to define security sources required for
authentication.

An more in-depth tutorial can be found in [Managing Security Sources]({{< product-version-root >}}how-to/secure/manage-sources/).
...
`password`, `pam`, and `certificate` sources, please see our [Managing Security Sources]({{< product-version-root >}}how-to/secure/manage-sources/) document.

##### Security Ciphers

To view a list of currently available security ciphers or change OpenRiak's
...
OpenRiak's cipher preferences were taken from [Mozilla's Server-Side TLS
...
i.e. the [`honor_cipher_order`]({{< product-version-root >}}reference/configuration/#security) setting is set to `on`. If you prefer, however, that clients' preferred cipher
order dictate which cipher is chosen, set `honor_cipher_order` to `off`.

> **Note on Erlang versions**
>
> OpenRiak's default cipher order behavior has the potential to crash Erlang
...
installing it can be found in [Installing Erlang]({{< product-version-root >}}how-to/install/source/). This issue should
...
to enable it prior to enabling security. If you are using [Protocol Buffers]({{< product-version-root >}}reference/protocol-buffers/) as a transport protocol for Riak (which we strongly recommend), enabling SSL on a given node requires only that you specify a [host and port]({{< product-version-root >}}reference/configuration/#client-interfaces) for the node
as well as a [certification configuration](#certificate-configuration).

If, however, you are using the [HTTP API]({{< product-version-root >}}reference/http-api/) for Riak and would like to
...
disabled and others enabled by setting the following [configurable parameters]({{< product-version-root >}}reference/configuration/#security) to `on` or `off`:
...
If you are using any of the available [security sources]({{< product-version-root >}}how-to/secure/manage-sources/), including [trust-based authentication]({{< product-version-root >}}how-to/secure/manage-sources/#trust-based-authentication), you will need to do so
over a secure SSL connection. In order to establish a secure connection,
you will need to ensure that each OpenRiak node's [configuration files]({{< product-version-root >}}reference/configuration/#security) point to the proper paths for your
...
on [configuring directories]({{< product-version-root >}}reference/configuration/#directories).
...
If you are using [certificate-based authentication]({{< product-version-root >}}how-to/secure/manage-sources/#certificate-based-authentication), Riak will check the certificate revocation list (CRL) of connecting clients' certificate by
...
- [TLS enablement]({{< product-version-root >}}how-to/secure/configure-tls/);
- [Enabling Security and restricting the Source of requests]({{< product-version-root >}}how-to/secure/enable-security/);
- [Granting permissions for specific actions]({{< product-version-root >}}how-to/secure/manage-permissions/).
