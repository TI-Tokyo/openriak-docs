...
that can be useful in application development, such as [Search]({{< product-version-root >}}reference/specialized-apis/legacy-query-api/), [secondary indexes (2i)]({{< product-version-root >}}how-to/develop/query-secondary-indexes/), and [Riak Data Types]({{< product-version-root >}}reference/data/distributed-data-types/), Riak almost always performs best when you
...
for each of them or consulting our guide to [building applications with Riak]({{< product-version-root >}}tutorials/first-application/) for a better sense of which features you might need.
...
look like (for the [HTTP API]({{< product-version-root >}}reference/http-api/)):
...
One way to determine this is to [list all keys]({{< product-version-root >}}reference/protocol-buffers/list-keys/) in the
...
A better possibility is to use [Riak sets]({{< product-version-root >}}reference/data/distributed-data-types/) to
store lists of keys in a bucket. Riak sets are a [Riak Data Type]({{< product-version-root >}}reference/data/distributed-data-types/) that enable you to store lists of binaries or strings in Riak.
...
`usernames`. The following will also assume that we've [set up a bucket type]({{< product-version-root >}}reference/data/distributed-data-types/) called
`sets`.
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{< product-version-root >}}reference/client-libraries/), you can find more information about getting started with your client in [Developing with OpenRiak KV: Getting Started]({{< product-version-root >}}tutorials/first-application/).
...
that simply extend OpenRiak's [defaults]({{< product-version-root >}}how-to/develop/use-bucket-types/) or multiple bucket types that have
...
- Plan index entries for future query needs.  Joining the results of multiple queries within an individual request, is possible across indexes as of Riak 3.4 [via the Query API]({{< product-version-root >}}tutorials/query-api/), but best performance is generally met by pre-concatenating index terms and running range queries with filters on concatenated terms.
