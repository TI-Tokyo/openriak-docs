---
title: 'The Dynamo model'
description: 'Explain the dynamo model and why it matters when designing or operating OpenRiak systems.'
weight: 5
...
legacy_3_2_5_sources:
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\learn\dynamo.md'
...
Explain the dynamo model and why it matters when designing or operating OpenRiak systems.
...
>
> Throughout the paper you will find notes containing OpenRiak KV-specifics that
...
> (either entirely or partially) by this paper. In addition to OpenRiak KV,
...
> Like Dynamo, OpenRiak KV employs consistent hashing to partition and replicate
...
>
> OpenRiak KV uses vector clocks for object versioning. Scroll down to section 4.4
> to read up on this in depth.
>
> OpenRiak KV makes use of gossiping in the same way that Dynamo does: to
...
[Consistent Hashing]: {{< product-version-root >}}explanation/foundations/glossary/#consistent-hashing
[Gossip Protocol]: {{< product-version-root >}}explanation/foundations/glossary/#gossiping
...
> **Brief Background on OpenRiak KV**
>
> Basho Technologies started to develop OpenRiak KV back in 2007 to solve an
...
>
> After using OpenRiak KV in production for several successful applications that
...
> OpenRiak KV is a highly available, scalable, open source key/value database. These
> notes describe where OpenRiak KV's design decisions emulated and diverged from
> Dynamo's (as described in this paper).
>
> OpenRiak KV offers several query methods in addition to the standard key/value
...
> 4. Full-text Search
>
> OpenRiak KV's realistic object size limit is around 5MB.
...
> **ACID?**
>
> OpenRiak KV offers no traditional "ACID" semantics around transactions. Instead,
...
> **Efficiency**
>
> Agreed. OpenRiak KV is made to (and will!) scale linearly on commodity hardware
...
> **OpenRiak KV Loves SLAs**
>
> Much like Amazon built Dynamo to guarantee their applications were always
> available to retail shoppers, the design decisions in OpenRiak KV were taken to
...
> related to their applications and services built on OpenRiak KV. This was not an
> accident.
...
> **OpenRiak KV's Design Considerations**
>
> Availability under any circumstances was something we stressed when designing
> OpenRiak KV, too. Most databases didn't enable developers to do this in a simple
...
> that OpenRiak KV could withstand network, server and other failures by sacrificing
...
> No conflict here (pun intended). OpenRiak KV also follows this approach to
> conflict resolution.
...
> We refer to hosts as "nodes", too. OpenRiak KV provides a simple set of commands
...
> A OpenRiak cluster is completely decentralized. No single node is special and this
...
> Dynamo distributed database functions, especially OpenRiak KV. It's still an
...
> OpenRiak KV's gossip protocol communicates between nodes with O(1) routing, and
...
> This all applies to OpenRiak KV, as well.
...
> level at which several OpenRiak KV properties can be configured (primarily the "N"
> value, or the replication value.) In addition to the bucket+key identifier and
> value, OpenRiak KV will also return the associated metadata for a given object
> with each get or put.
>
> OpenRiak KV has two APIs: an [HTTP API] and a [Protocol Buffers API].

[HTTP API]: {{< product-version-root >}}reference/http-api/
[Protocol Buffers API]: {{< product-version-root >}}reference/protocol-buffers/
...
> OpenRiak KV concatenates the bucket with the key and runs it through the SHA1 hash
> to generate a 160 bit identifier which is then used to determine where in the
> database each datum is stored. OpenRiak KV treats data as an opaque binary, thus
...
> **Partitioning in OpenRiak KV**
>
> As mentioned above, OpenRiak KV uses consistent hashing to distribute data around
...
>
> OpenRiak KV also breaks the ring into a set number of partitions. This number is
...
> OpenRiak KV also has the concept of virtual nodes and they are used to the same
...
> All of these properties for vnodes in Dynamo hold true for OpenRiak KV, too.
...
[Further Reading on Partitioning in OpenRiak KV]: {{< product-version-root >}}explanation/foundations/clusters-rings-and-partitions/
[All about the OpenRiak KV Ring]: {{< product-version-root >}}explanation/foundations/clusters-rings-and-partitions/#the-ring
...
> Replication in OpenRiak KV, like in Dynamo, is fundamental and automatic. Remember
> the concept of a bucket we covered above? In OpenRiak KV, the replication
> parameter, "N" (also called "n&#95;val"), is configurable at the bucket level.
> The default n_val in OpenRiak KV is 3, meaning that out of the box OpenRiak KV will
> store three replicas of your data on three different partitions on the ring.
>
> The diagram is applicable to OpenRiak KV and the manner in which it replicates
> data. The preference list is present in OpenRiak KV, too, and is the reason why
...
> OpenRiak KV is an "eventually consistent" database. All replication is done
...
> Much like Dynamo was suited to the design of the shopping cart, OpenRiak KV, and
...
> The same holds true for OpenRiak KV. If, by way of some failure and concurrent
> update (rare but quite possible), there come to exist multiple versions of the
> same object, OpenRiak KV will push this decision down to the client (who are we to
...
>
> [Vector Clock on OpenRiak KV Glossary]({{< product-version-root >}}explanation/foundations/glossary/#vector-clock)
>
...
> OpenRiak KV does a certain amount of vector clock pruning to ensure their growth
...
> Any node in the OpenRiak KV ring can coordinate a request. The OpenRiak KV information
...
> OpenRiak KV makes use of the same values. But, thanks to our concept of buckets,
...
> setting in OpenRiak KV.
>
>Some more resources on R and W:
>
>[REST API]({{< product-version-root >}}reference/http-api/)
>
>[Writing Data]({{< product-version-root >}}how-to/develop/create-object/)
>
>[Reading Data]({{< product-version-root >}}how-to/develop/read-object/)
...
> In OpenRiak KV a write is considered successful when the total number of
> responding writes equals W. This need not be a durable write, which is a
> separate value in OpenRiak KV labeled DW.
...
> [Read Repair]({{< product-version-root >}}explanation/replication/#read-repair).
...
> [Hinted handoff] is built into OpenRiak KV's core.
>
> You can glimpse at OpenRiak KV's preference list (or *preflist*) calculation in
> the [Replication] walkthrough.

[Hinted handoff]: {{< product-version-root >}}explanation/foundations/glossary/#hinted-handoff
[Replication]: {{< product-version-root >}}explanation/replication/
...
> As mentioned previously, OpenRiak KV does not require that a write be durable,
...
[Multi Datacenter Replication]: {{< product-version-root >}}explanation/replication/multi-datacenter-architecture/
...
> OpenRiak KV implements a Merkel-Tree based Active Anti-Entropy (*AAE*).
...
[Adding and Removing Nodes]: {{< product-version-root >}}tutorials/operations/change-cluster-membership/
[Failure Scenarios]: {{< product-version-root >}}explanation/consistency/eventual-consistency/
...
[riak admin command-line tool]: {{< product-version-root >}}reference/commands/riak-admin/
...
[gossiping]: {{< product-version-root >}}explanation/foundations/glossary/#gossiping
...
> These tokens are vnodes (virtual nodes) in OpenRiak KV.
...
> To rectify these sorts of logical partitions, multiple OpenRiak cluster changes
...
[The Node Join Process]: {{< product-version-root >}}tutorials/operations/change-cluster-membership/#joining-nodes-to-form-a-cluster
...
> OpenRiak KV follows the same mechanism, by manually triggering permanent ring
...
> OpenRiak KV does not randomly assign vnodes, but rather, iterates through the list
...
> OpenRiak KV is implemented in Erlang. Request coordination and membership behavior
> is defined by [riak_core] and implemented by [OpenRiak KV].

[riak_core]: http://github.com/basho/riak_core
[OpenRiak KV]: http://github.com/basho/riak_kv
...
> OpenRiak KV ships with various [backend options]. [Bitcask] is the default, but
...
[backend options]: {{< product-version-root >}}explanation/storage/choosing-backend/
[Bitcask]: {{< product-version-root >}}explanation/storage/bitcask/
[LevelDB]: {{< product-version-root >}}explanation/storage/leveldb/
[Memory]: {{< product-version-root >}}explanation/storage/memory/
[secondary indexes]: {{< product-version-root >}}how-to/develop/query-secondary-indexes/
...
> Request coordination in OpenRiak KV uses Erlang message passing, but follows a
...
> OpenRiak KV currently supports simple conflict resolution by way of read-repair,
> remanding more complex reconciliation to the client. There are several tools
> to help simplify this task, such as [Statebox].
>
> OpenRiak KV supports a simple reconciliation strategy, called [CRDTs (Commutative
> Replicated Data Types)], for reconciling common data types like sets and
> counters.

[Statebox]: https://github.com/mochi/statebox_riak
[CRDTs (Commutative Replicated Data Types)]: {{< product-version-root >}}reference/data/distributed-data-types/
...
> This is more similar to OpenRiak's W value, since only DW requires a durable write
...
[partitioning]: {{< product-version-root >}}explanation/replication/#understanding-replication-by-example
...
[The Node Join Process]: {{< product-version-root >}}tutorials/operations/change-cluster-membership/#joining-nodes-to-form-a-cluster
[Replacing a Node]: {{< product-version-root >}}how-to/operate/replace-node/
...
[Load Balancing]: {{< product-version-root >}}how-to/configure/load-balancing-proxy/
...
> ```
>
> Note that the Riak clients do not coordinate with OpenRiak's preference list, but
> simply round-robin requests, letting the OpenRiak cluster handle routing.
