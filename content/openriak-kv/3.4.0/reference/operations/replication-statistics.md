...
description: 'Define the names, fields, states, limits, and version applicability for replication statistics.'
weight: 8
diataxis: 'reference'
product: 'OpenRiak KV'
...
Define the names, fields, states, limits, and version applicability for replication statistics.
...
Field | Description
------|------------
`node` | A unique ID for the OpenRiak node that the sink in running on
...
Field | Description
------|------------
`node` | A unique ID for the OpenRiak node that the source is running on
...
The [Riak status statistics]({{< product-version-root >}}reference/operations/statistics-and-monitoring/) set includes stats relevant to monitoring replication:
