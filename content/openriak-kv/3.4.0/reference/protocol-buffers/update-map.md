---
title: 'Update a map with Protocol Buffers'
...
The `MapField` parameter is explained above. The operations used to update fields depend on the Data Type in that field, i.e. `CounterOp` messages to update counters, `SetOp` messages to update sets, etc. Updating counters is covered in [PBC Data Type Counter Store]({{< product-version-root >}}reference/protocol-buffers/update-counter/) while updating sets is covered in [PBC Data Type Set Store]({{< product-version-root >}}reference/protocol-buffers/update-set/).
