...
3. Start the new `riak7` node with [`riak start`]({{< product-version-root >}}reference/commands/riak/):
...
participating in the cluster; for example `riak0` with the [`riak admin cluster join`]({{< product-version-root >}}reference/commands/riak-admin/#cluster) command executed on the new `riak7` node:
...
`riak7` node using the [`riak admin cluster replace`]({{< product-version-root >}}reference/commands/riak-admin/#cluster) command:
...
6. Examine the proposed cluster changes with the [`riak admin cluster plan`]({{< product-version-root >}}reference/commands/riak-admin/#cluster) command executed on the new
`riak7` node:
...
[`riak admin cluster commit`]({{< product-version-root >}}reference/commands/riak-admin/#cluster) command:

```bash
    riak admin cluster commit
    ```

If you need to clear the proposed plan and start over, use [`riak admin cluster clear`]({{< product-version-root >}}reference/commands/riak-admin/#cluster):
...
with the [`riak admin ringready`]({{< product-version-root >}}reference/commands/riak-admin/#ringready)
and [`riak admin member-status`]({{< product-version-root >}}reference/commands/riak-admin/#member-status)
commands.
...
- [ensuring the node is downed]({{< product-version-root >}}how-to/operate/replace-node/);
- [forcing the replace]({{< product-version-root >}}how-to/operate/replace-node/);
- [repairing the new node]({{< product-version-root >}}how-to/troubleshoot/recover-failed-node/).
...
The new node should be started with [`participate_in_coverage` disabled]({{< product-version-root >}}reference/operations/remote-console/), as it will at this stage be a full member of the cluster but have no data.  It is also more efficient to suspend anti-entropy until the repair is complete.
