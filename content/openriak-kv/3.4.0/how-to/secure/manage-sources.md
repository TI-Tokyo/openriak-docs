...
be best to start with our general guide to [authentication and authorization]({{< product-version-root >}}how-to/secure/enable-security/).
...
[created]({{< product-version-root >}}how-to/secure/enable-security/#user-management) and that
security has been [enabled]({{< product-version-root >}}how-to/secure/enable-security/).
...
interface, and can be used only through the [protocol buffers interface]({{< product-version-root >}}reference/protocol-buffers/).
...
a different directory in your [`riak.conf`]({{< product-version-root >}}reference/configuration/) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`]({{< product-version-root >}}reference/configuration/) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your OpenRiak cluster, you need to perform a [rolling restart]({{< product-version-root >}}how-to/operate/rolling-restart/). Once that process is complete, you can use the client
...
[Managing group]: {{< product-version-root >}}how-to/secure/manage-sources/
