---
title: 'Tune an AWS deployment'
description: 'Show performance engineers how to tune an aws deployment using measurable before-and-after checks.'
weight: 5
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show performance engineers how to tune an aws deployment using measurable before-and-after checks.
...
This guide introduces best practices for tuning OpenRiak cluster performance
...
The most commonly used [instance types](http://aws.amazon.com/ec2/instance-types/) for OpenRiak cluster nodes are `large` and `xlarge` `m` class (General Purpose), such as `m4.xlarge`. In cases where 10-gigabit Ethernet networking is desired, the Cluster Compute class of EC2 instances, such as `cc2.8xlarge` can be used.
...
OpenRiak's primary bottleneck will be disk and network I/O, meaning that in
most cases, standard EBS will incur too much latency and iowait. OpenRiak's
...
that the other nodes in the OpenRiak cluster are still operating normally
...
approximately-compatible data payloads with the OpenRiak cluster directly.
...
benchmarking Riak clusters with [Basho Bench]({{< product-version-root >}}how-to/tune/).
...
1. Stop one or more nodes normally and restart them after a few moments
    (simulates [rolling upgrade]({{< product-version-root >}}how-to/operate/upgrade-cluster/)).
...
needs to be scaled out. While the OpenRiak node is out, other nodes may also
...
ensure that you follow the steps outlined in the [Node Name Changed]({{< product-version-root >}}how-to/troubleshoot/recover-failed-node/#node-name-changed) section to replace
it.
...
* [Failure and Recovery]({{< product-version-root >}}how-to/)
