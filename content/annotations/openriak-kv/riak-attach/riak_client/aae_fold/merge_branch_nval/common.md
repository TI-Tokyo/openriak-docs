# Metadata

command: erlang:riak_client:aae_fold/1:merge_branch_nval
versions: 3.4.0, 3.4.1

# Summary

Merge selected branches of cached AAE trees.

# Description

Returns the selected branch identifiers and their binary hashes.

# Notes

The named fields belong inside the query tuple in the order shown by the type declaration. These operations read the AAE store and can impose cluster-wide load.

# Arguments

## NVal

datatype: positive integer
required: true
repeatable: false

### Description

See the shared `NVal` argument on the AAE fold parent page.

## BranchFilter

datatype: list of non-negative integers
required: true
repeatable: false

### Description

List of branch IDs to merge, for example `[0]`.

## Client

datatype: riak_client handle
required: false
repeatable: false

### Description

See the shared `Client` argument on the parent module page.

# Errors

## reference-error-1

### Condition

The cluster-wide fold does not finish within its timeout.

### Description

The API can return `{error, timeout}` or another error term from its coverage workers.

### Remedy

Check node availability, AAE state and filter size. Reduce the range before retrying a costly operation.

# Examples

## erlang-riak-client-aae-fold-merge-branch-nval:seed-five-objects

title: Create five objects

### Description

Create five objects to follow the range-query examples. Wait for AAE to include all five before checking counts or applying a range operation.

## erlang-riak-client-aae-fold-merge-branch-nval:populated-aae

### Description

Choose branches containing the five stored keys. Verify every requested branch is returned and its hash data is nonzero.

# Reviewed against

3.4.0: 69d6ecbc79e923c065ef9a574168dc46b585c61ddf5538cc7ebd02c25cc6bb35
3.4.1: 69d6ecbc79e923c065ef9a574168dc46b585c61ddf5538cc7ebd02c25cc6bb35
