...
author: "Sean Cribbs"
original_url: "http://basho.com/posts/technical/protobuffs-in-riak-1-2/"
archive_url: "https://web.archive.org/web/20170427162458http://basho.com/posts/technical/protobuffs-in-riak-1-2"
categories:
  - "Developer Tools"
  - "Releases"
---
...
You might remember that back in April, we [sent around a survey](http://lists.basho.com/pipermail/riak-users_lists.basho.com/2012-April/008203.html) to get input about what features developers use and want in Riak clients. All in all, we had about 87 developers respond to the survey.
...
- While the HTTP interface of Riak has always been built atop [Webmachine](http://wiki.basho.com/Webmachine.html), making it easy to  
...
**Second, we separated the Protobuffs message definitions from the****Erlang client library.** We put the `.proto` file in a new library application called [`riak_pb`](https://github.com/basho/riak_pb), and actually split it out into several files, grouped by the component of the server they represent; this means there’s a `riak.proto`, `riak_kv.proto`, and `riak_search.proto`. In addition to removing the coupling between the Erlang client and the server, we now have a project whose only responsibility is to describe the messages of the protocol. It’s like the equivalent of an RFC, but in code! In the near future we will have build targets in the project that let us generate Java or Python shims from the included messages and that we can distribute as standalone `.jar` and `.egg` files.
...
Client-side support for these new requests and responses has already landed in the [Ruby client](https://github.com/basho/riak-ruby-client) and will soon be landing in [Java](https://github.com/basho/riak-java-client), [Erlang](https://github.com/basho/riak-erlang-client), and [Python](https://github.com/basho/riak-python-client). You can track support for the new features on our updated [Client Libraries](http://wiki.basho.com/Client-Libraries.html) wiki page.
