...
original_url: "http://basho.com/posts/technical/riak-and-hadoop-word-count-example/"
archive_url: "https://web.archive.org/web/20170801120853http://basho.com/posts/technical/riak-and-hadoop-word-count-example/"
categories:
...
My last post was a brief description of the integration between Riak and Hadoop MapReduce. This, the follow up post, will be a bit more hands on and walk you through an example [riak-hadoop](https://github.com/basho/riak-hadoop) MapReduce job. If you want to have a go, you need to follow these steps. I won’t cover all the details of installing everything, but I’ll point you at resources to help.

## Getting Set Up

First you need both a Riak and Hadoop install. I went with a local [devrel Riak cluster](http://wiki.basho.com/Building-a-Development-Environment.html) and a local Hadoop install in [pseudo distributed mode](http://hadoop.apache.org/common/docs/r0.20.2/quickstart.html#PseudoDistributed). (A singleton node of both Hadoop and Riak installed from a package will do, if you’re pushed for time.)

The example project makes use of Riak’s [Secondary Indexes](http://wiki.basho.com/Secondary-Indexes.html) (“2i”) so you will need to switch the backend on Riak to use LevelDB. (NOTE: This is not a requirement for riak-hadoop, but this demo uses 2i, which does require it). So, for each Riak node, change the storage backend from `bitcask` to `eleveldb` by editing the [`app.config`](http://wiki.basho.com/Configuration-Files.html#app.config).
...
Next you need to pull the [riak-hadoop-wordcount](https://github.com/russelldb/riak-hadoop-wordcount) sample project from GitHub. Checkout the project and build it:
...
*Warning*: Hadoop has some class loading issues. There is no class namespace isolation. The [Riak Java Client](https://github.com/basho/riak-java-client) depends on [Jackson](http://wiki.fasterxml.com/JacksonHome) for JSON handling, and so does Hadoop, but different versions (naturally). When the Riak-Hadoop driver is finished it will come with a custom classloader. Until then, however, you’ll need to replace your Hadoop `lib/jackson*.jar` libraries with the ones in the `lib` folder of this repo on your`JobTracker` / `Namenode` only. On your `tasknodes`, you need only *remove* the Jackson jars from your `hadoop/lib` directory, since the classes in the job jar are at least loaded. (There is an [open bug](https://issues.apache.org/jira/browse/MAPREDUCE-1700) about this in Hadoop’s JIRA. It has been open for 18 months, so I doubt it will be fixed anytime soon. I’m very sorry about this. I will address it in the next version of the driver.)

## Loading Data

The repo includes a copy of Mark Twain’s [Adventures Of Huckleberry Finn](http://www.gutenberg.org/ebooks/76) from [Project Gutenberg](http://www.gutenberg.org/), and a class that chunks the book into chapters and loads the chapters into Riak.
...
This will load the data (with an index on the `Author` field.) Have a look at the [`Chapter` class](https://github.com/russelldb/riak-hadoop-wordcount/blob/master/src/main/java/com/basho/riak/hadoop/Chapter.java) if you’d like to see how easy it is to store a model instance in Riak.
...
We just performed a full roundtrip MapReduce, starting with data stored in Riak, feeding it to Hadoop for the actual MapReduce processing, and then storing the results back into Riak. It was a trivial task with a small amount of data, but it illustrates the principle and the potential. Have a look at the [`RiakWordCount` class](https://github.com/russelldb/riak-hadoop-wordcount/blob/master/src/main/java/com/basho/riak/hadoop/RiakWordCount.java). You can see that only a few lines of configuration and code are needed to perform a Hadoop MapReduce job with Riak data. Hopefully the [riak-hadoop-wordcount](https://github.com/russelldb/riak-hadoop-wordcount) repo can act as a template for some further exploration. If you have any trouble running this example, please let me know by raising a [GitHub issue](https://github.com/russelldb/riak-hadoop-wordcount/issues) against the project or by [jumping on the Riak Mailing List](http://lists.basho.com/mailman/listinfo/riak-users_lists.basho.com) to ask some questions.
