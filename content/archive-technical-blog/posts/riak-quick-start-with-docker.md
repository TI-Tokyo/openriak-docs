...
date: "2014-04-21T13:00:51+00:00"
author: "Hector Castro"
original_url: "http://basho.com/posts/technical/riak-quick-start-with-docker/"
archive_url: "https://web.archive.org/web/20170521110512http://basho.com/posts/technical/riak-quick-start-with-docker"
categories:
  - "Cloud & Deployment"
---
This post is now outdated. Please refer to this more recent post: [http://basho.com/posts/technical/running-riak-in-docker/](./running-riak-in-docker.md)

*April 21, 2014*

In the Riak documentation, one of the first sections contains a [quick start](http://docs.basho.com/riak/latest/quickstart/). The goal of the quick start is to get Riak on your workstation and then establish a five-node cluster (in under five minutes).
...
In a world with [Linux Containers](https://linuxcontainers.org/) (LXC) and [Docker](https://www.docker.io/), is there a way we can leverage these technologies to make the Riak quick start process more streamlined for developers? At the same time, can the isolation and portability of containers ease the transition of clusters between environments for operators?

Below is a first pass at it.

## Prerequisites

Docker and LXC are key prerequisites for a container-based quick start. Luckily, Docker’s website has [installation instructions](https://www.docker.io/gettingstarted/) for almost every flavor of Linux, Windows, and Mac OS. Furthermore, those instructions also include the installation of LXC.
...
To get the `Dockerfile`, simply clone the [docker-riak repository](https://github.com/hectcastro/docker-riak). From there, use the `build` `Makefile` target to build the container:

After the image building process is complete, you should see something like this from the output of `docker images`:

(See the entire `Dockerfile` contents [here](https://github.com/hectcastro/docker-riak/blob/develop/Dockerfile).)

## Bring Up a Cluster

After the Docker image for Riak is created, the next step is to create a container out of it. But, since Riak is a distributed database, we don’t want to spin up just one Riak container – we want to spin up [at least 5](http://basho.com/why-your-riak-cluster-should-have-at-least-five-nodes/).

We also want the Riak containers to communicate with each other. Within Docker, this can be accomplished by [linking](http://docs.docker.io/en/latest/use/working_with_links_names/) containers. Linking connects one container to others by populating the target container’s environment with variables containing IP and port information of [exposed](http://docs.docker.io/en/latest/use/port_redirection/) endpoints associated with the source container.
...
From outside the container, we can interact with Riak’s [HTTP](http://docs.basho.com/riak/latest/dev/references/http/) or [Protocol Buffers](http://docs.basho.com/riak/latest/dev/references/protocol-buffers/) interfaces. For testing, we’re going to use the HTTP interface.

**`/stats`**  
The HTTP interface has an endpoint called `/stats` that emits [Riak statistics](http://docs.basho.com/riak/latest/ops/running/stats-and-monitoring/). The `test-cluster` `Makefile` target hits a random container’s `/stats` endpoint and pretty-prints its output to the console.
