...
date: "2017-02-21T19:05:56+00:00"
author: "Dorothy Pults"
original_url: "http://basho.com/posts/technical/create-your-first-riak-ts-table/"
archive_url: "https://web.archive.org/web/20170331104126http://basho.com/posts/technical/create-your-first-riak-ts-table/"
categories:
  - "Data Modeling"
  - "Developer Tools"
---
[![](../images/create-your-first-riak-ts-table/Blog-Racing-flag.png)](../images/create-your-first-riak-ts-table/Blog-Racing-flag.png)Customers like [Intellicore](http://basho.com/posts/press/fia-formula-e-championship-offers-innovative-viewing-experience-powered-by-intellicore-and-riak-ts/) are doing real-time analysis of IoT data using Riak TS. We want to make it easy for you to get started with Riak TS and take it for a test drive.
...
Before running Riak TS on your Mac, you will want to change the [Open Files Limit.](https://docs.basho.com/riak/kv/latest/using/performance/open-files-limit/#mac-os-x)

The examples below use Riak TS 1.5.2. You will want to [download and install](https://docs.basho.com/riak/ts/latest/downloads/#mac-os-x) the latest release of Riak. Once you have downloaded the file, untar it, and cd to the riak directory.
...
```
Bashos-MacBook-Air:riak-ts-1.5.2 bashotechnologies$ pwd
/Users/bashotechnologies/riak-ts-1.5.2
Bashos-MacBook-Air:riak-ts-1.5.2 bashotechnologies$ bin/riak start
Bashos-MacBook-Air:riak-ts-1.5.2 bashotechnologies$ bin/riak ping
pong
```

We will use [riak-shell](http://docs.basho.com/riak/ts/latest/using/riakshell/) to create a table and run queries. To see a list of riak-shell commands use ‘help;’ or ‘help sql;’ . To quit riak-shell use ‘q;’.

```
Bashos-MacBook-Air:riak-ts-1.5.2 bashotechnologies$ bin/riak-shell
Connected...
...
- *Column definition*s – define the structure of the data including the column name, data type, and inline constraint. Columns that are part of the primary key must have the inline constraint of NOT NULL.
...
We won’t go into [data modeling](http://docs.basho.com/riak/ts/latest/learn-about/tablearchitecture/#data-modeling) or the details on how to determine your [keys](http://docs.basho.com/riak/ts/latest/using/planning/#primary-key) or [quantum.](http://docs.basho.com/riak/ts/latest/using/planning/#quantum) You can find out more about these in the Riak TS documentation.
...
In this sample code, we have used an easy to read ISO 8601 string. When you create your actual app and store your data, you will want to use the Riak TS supported client libraries to pass [timestamps](http://docs.basho.com/riak/ts/latest/learn-about/timestamps/) as integers (Unix Epoch Time).

### Queries

Next, we will look at a few simple [Riak TS queries.](http://docs.basho.com/riak/ts/latest/using/querying/select/) Let’s start with a query of the rows we just added to our table.
...
There are a variety of [aggregation functions](http://docs.basho.com/riak/ts/latest/using/querying/select/aggregate-functions/#aggregate-functions) support by Riak TS including COUNT, SUM, MEAN, AVG, MIN, MAX, and STDDEV. Below are several example queries that use aggregation functions.
...
[Arithmetic operations](http://docs.basho.com/riak/ts/latest/using/querying/select/arithmetic-operations/) are also supported. Assuming that the values in the table are Fahrenheit temperatures, we can convert them to Celcius.
...
[GROUP BY](http://docs.basho.com/riak/ts/latest/using/querying/select/group-by/) and [ORDER BY](http://docs.basho.com/riak/ts/latest/using/querying/select/order-by/) are also supported in SELECT statements. There is more information about optimizing your queries in the Riak TS documentation. When using ORDER BY, you can also use the optional keywords LIMIT, OFFSET, ASC, DESC, NULLS FIRST, and NULLS LAST.
...
When you are ready to develop your first app, you will want to select from the list of supported [operating systems](http://docs.basho.com/riak/ts/latest/#supported-operating-systems) and [client libraries](http://docs.basho.com/riak/ts/latest/developing/). In production, we recommend no fewer than 5 nodes to ensure availability of your data. Below is a list of additional resources to help you learn more about Riak TS.
