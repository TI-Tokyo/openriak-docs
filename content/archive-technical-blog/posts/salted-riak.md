---
title: "Salted Riak"
date: "2015-08-30T13:57:09+00:00"
author: "Stephen Condon"
original_url: "http://basho.com/posts/technical/salted-riak/"
archive_url: "https://web.archive.org/web/20170704132443http://basho.com/posts/technical/salted-riak/"
categories:
  - "Security"
  - "Operations"
---
*This article was authored by Basho’s good friend Matt Davis and was originally posted on the [OpenX blog](http://codinginadtech.com/posts/salted-riak/) earlier this month.  We thank OpenX and Matt for being an active contributor to the Basho Community and for allowing us to repost this blog.*
...
Equal distribution of resources matter, it enables our ability to quickly and efficiently manipulate the building blocks without fear; it must be as easy to replace a node as it is to let it die. To provide a framework that ensures Riak configurations are consistent and repeatable, we employ [SaltStack](http://saltstack.com/).
...
State definitions and files are often peppered with [Jinja templates](http://jinja.pocoo.org/). These are used within Salt state files and module definitions, providing a way to dynamically target things by either a defined variable or a stored key/value datapoint known as a **Grain**. These bits of information hold vital statistics about the host, like the IP address in `/etc/riak/advanced.config`:

```
{riak_core, [
  {cluster_mgr, {"{{ salt['network.interfaces']()['eth0']['inet'][0]['address'] }}", 9080 } }
]}
```
