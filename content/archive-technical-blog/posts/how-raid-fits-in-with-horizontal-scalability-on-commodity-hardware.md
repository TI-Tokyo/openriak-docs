...
date: "2013-06-18T18:49:41+00:00"
author: "Hector Castro"
original_url: "http://basho.com/posts/technical/how-raid-fits-in-with-horizontal-scalability-on-commodity-hardware/"
archive_url: "https://web.archive.org/web/20170801115337http://basho.com/posts/technical/how-raid-fits-in-with-horizontal-scalability-on-commodity-hardware/"
categories:
  - "Operations"
  - "Architecture & Distributed Systems"
---
Riak shines when scaling horizontally with commodity hardware. A question frequently arises: should a [redundant array of independent disks (RAID)](http://en.wikipedia.org/wiki/RAID) be used, and if so, how?
...
| --- | --- | --- | --- | --- | --- | --- |
| RAID 0 | Stripe | Excellent | Excellent | None | 2 | 1000GB |
| RAID 1 | Mirror | Excellent | Poor | Excellent | 2 | 500GB |
| RAID 10 | Stripe + Mirror | Excellent | Good | Good | 4 | 1000GB |
| RAID 5/6/z | Combination/Parity | Good | Poor | Excellent | 3 | 1000GB |

*\*Usable space calculations assume all disks are `500GB`.*

Tools such as `basho_bench` can help you evaluate whether such a boost is useful for your environment and application needs.
...
1. Disk failure detected by RAID
2. Leave node up until a spare disk is acquired
3. Shut down Riak and take the node offline
4. Rebuild the disks completely
5. Bring the node back online and start Riak
...
**mdadm**  
On Linux, the most popular software RAID solution is [mdadm](http://en.wikipedia.org/wiki/Mdadm). It can create, assemble, report on, and monitor arrays.  
 `# RAID 0  
...
**ZFS**  
On Solaris or FreeBSD, the most common solution is [ZFS](http://en.wikipedia.org/wiki/ZFS).  
 `# Striped zpool  
...
Either way, Riak provides redundancy by generating replicas of data and automatically spinning up fallback [vnodes](http://docs.basho.com/riak/latest/references/appendices/concepts/Riak-Glossary/#Vnode). In addition, having [at least 5 nodes](http://basho.com/why-your-riak-cluster-should-have-at-least-five-nodes/) in a cluster decreases the likelihood of any single disk/node failure causing a service degradation or disruption.

Shout out to [Brent Woodruff](https://twitter.com/fprimex), [John Daily](https://twitter.com/macintux), Paul Hagan, and [Seth Thomas](https://twitter.com/cheeseplus) for their help in assembling this post.
