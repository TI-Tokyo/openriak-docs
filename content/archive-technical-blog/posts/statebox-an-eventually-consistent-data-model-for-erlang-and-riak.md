...
date: "2011-05-13T23:25:26+00:00"
author: "Basho"
original_url: "http://basho.com/posts/technical/statebox-an-eventually-consistent-data-model-for-erlang-and-riak/"
archive_url: "https://web.archive.org/web/20170801115501http://basho.com/posts/technical/statebox-an-eventually-consistent-data-model-for-erlang-and-riak/"
categories:
  - "Erlang & BEAM"
  - "Data Modeling"
---
...
A few weeks ago when I was on call at work I was chasing down a bug in friendwad [[1]](#id6) and I realized that we had made a big mistake. The data model was broken, it could only work with transactions but we were using [Riak](http://www.basho.com/products_riak_overview.php). The original prototype was built with Mnesia, which would’ve been able to satisfy this constraint, but when it was refactored for an eventually consistent data model it just wasn’t correct anymore. Given just a little bit of concurrency, such as a popular user, it would produce inconsistent data. Soon after this discovery, I found another service built with the same invalid premise and I also realized that a general solution to this problem would allow us to migrate several applications from Mnesia to Riak.

When you choose an eventually consistent data store you’re prioritizing availability and partition tolerance over consistency, but this doesn’t mean your application has to be inconsistent. What it does mean is that you have to move your conflict resolution from writes to reads. [Riak](http://www.basho.com/products_riak_overview.php) does almost all of the hard work for you [[2]](#id7), but if it’s not acceptable to discard some writes then you will have to set allow\_mult to true on your bucket(s) and handle siblings [[3]](#id8) from your application. In some cases, this might be trivial. For example, if you have a set and only support adding to that set, then a merge operation is just the union of those two sets.

[statebox](http://github.com/mochi/statebox) is my solution to this problem. It bundles the value with repeatable operations [[4]](#id9) and provides a means to automatically resolve conflicts. Usage of statebox feels much more declarative than imperative. Instead of modifying the values yourself, you provide statebox with a list of operations and it will apply them to create a new statebox. This is necessary because it may apply this operation again at a later time when resolving a conflict between siblings on read.
...
Here’s what typical statebox usage looks like for a trivial application (note: Riak metadata is not merged [[5]](#id10)). In this case we are storing an orddict in our statebox, and this orddict has the keys following and followers.
...
Well, that’s about it! alice is following both bob and charlie despite the concurrency. No locks were harmed during this experiment, and we’ve arrived at eventual consistency by using [statebox\_riak](http://github.com/mochi/statebox_riak), [statebox](http://github.com/mochi/statebox), and [Riak](http://www.basho.com/products_riak_overview.php) without having to write any conflict resolution code of our own.

[Bob](https://twitter.com/etrepum)

And if you’re at all interested in getting paid to do stuff like this, Mochi is hiring[.](http://www.mochimedia.com/jobs/)

**References**

|  |  |
| --- | --- |
| [[1]](#id1) | friendwad manages our social graph for Mochi Social and MochiGames.  It is also evidence that naming things is a hard problem in  computer science. |

|  |  |
| --- | --- |
| [[2]](#id2) | See Basho’s articles on ~~Why Vector Clocks are Easy~~ and  ~~Why Vector Clocks are Hard.~~ |

|  |  |
| --- | --- |
| [[3]](#id3) | When multiple writes happen to the same place and they have  branching history, you’ll get multiple values back on read.  These are called siblings in [Riak](http://www.basho.com/products_riak_overview.php). |

|  |  |
| --- | --- |
| [[4]](#id4) | An operation F is repeatable if and only if F(V) = F(F(V)).  You could also call this an [idempotent unary operation](http://en.wikipedia.org/wiki/Idempotence#Unary_operation). |

|  |  |
| --- | --- |
| [[5]](#id5) | The default conflict resolution algorithm in statebox\_riak  chooses metadata from one sibling arbitrarily. If you use  metadata, you’ll need to come up with a clever way to merge it  (such as putting it in the statebox and specifying a custom  resolve\_metadatas in your call to statebox\_riak:new/1). |
