...
original_url: "http://basho.com/posts/technical/why-mapreduce-is-hard/"
archive_url: "https://web.archive.org/web/20170801120812http://basho.com/posts/technical/why-mapreduce-is-hard/"
categories:
  - "Search & Analytics"
---
**April 20, 2011**

At the highest level, MapReduce can be a fundamentally hard concept to grasp. In our [last blog post on MapReduce,](./why-mapreduce-is-easy.md) you learned how easy it is. After all, it’s just code, right?
...
~~Search~~. It does a much better job at these reverse lookup queries (which some call secondary indexes), and you can neatly ~~feed the results into a MapReduce request~~, something we’ve implemented in [Riaktant](https://github.com/basho/riaktant/), our little syslog collector and aggregator. But we’ll leave dissecting that code to another blog post.
...
The small fix: test your MapReduce functions on a cluster with a single node before cranking it up a notch, or have a look at Sean Cribbs’ [riak-qc.js](https://github.com/seancribbs/riak-qc.js) framework for testing Riak MapReduce functions.

## The MapReduce bottom line

MapReduce is hard, but we’re not going shopping just yet. Instead, we’re keeping a close eye on what our users are trying to achieve, improving Riak along the way. For more details on Riak’s MapReduce, check out the ~~wiki~~, read a ~~blog post on more practical MapReduce~~, or watch last year’s [webinar on MapReduce](http://vimeo.com/13554436). If you’re feeling adventurous, have a look at the [examples](https://github.com/basho/riaktant/tree/master/examples) and [MapReduce](https://github.com/basho/riaktant/blob/master/lib/map_reduce.js) [code](https://github.com/basho/riaktant/blob/master/lib/map_reduce.js) used in Riaktant. I’ll be sure to get back to you with a proper introduction on how it uses MapReduce to sift through syslog data.

— [Mathias](http://twitter.com/roidrage)
