...
date: "2017-01-04T14:28:04+00:00"
author: "Christopher Mancini"
original_url: "http://basho.com/posts/technical/what-in-the-hell-is-hyperloglog/"
archive_url: "https://web.archive.org/web/20170421104542http://basho.com/posts/technical/what-in-the-hell-is-hyperloglog/"
categories:
  - "Search & Analytics"
---
The first time someone hears the term, HyperLogLog, a common response is to wonder what in the HeLL they are talking about. Personally, it sounds like something from a sci-fi movie; but it isn’t. I am going to introduce you to what [HyperLogLog](http://docs.basho.com/riak/kv/latest/developing/data-types/hyperloglogs/) is, why you might want to use it, and how to use it with Riak KV.

With a basic understanding of what HyperLogLog is, we can better appreciate how it can be valuable to any business that has large sets of data. In the simplest of terms, HyperLogLog is an algorithm that makes it easy to estimate the number of unique values within a very large set, which is also known as [cardinality](https://en.wikipedia.org/wiki/Cardinality). For instance, if I define a set of 8 numbers, {4,3,6,2,2,6,1,7}, the cardinality of the membership set would be 6.
...
- [Wikipedia HyperLogLog article](https://en.wikipedia.org/wiki/HyperLogLog)
- [Hyperloglog Datatypes in Riak by Zeeshan Lakhani](https://github.com/basho/riak_kv/blob/develop/docs/hll/hll.pdf)
...
With the release of [Riak KV 2.2](./riak-kv-2-2-release-highlights.md), we introduced HyperLogLog support as a new data type. The new HyperLogLog data type is easy to use, allowing you to add new values like you would with a [Riak Set](http://docs.basho.com/riak/kv/2.2.0/developing/data-types/sets/) and retrieve the cardinality value like you would fetch the value from a [Riak Counter](http://docs.basho.com/riak/kv/2.2.0/developing/data-types/counters/).
...
[@chrismancini](https://twitter.com/@chrismancini)  
[mancini.io](http://mancini.io/)

![](../images/what-in-the-hell-is-hyperloglog/tier-1-line.png)  
References:
