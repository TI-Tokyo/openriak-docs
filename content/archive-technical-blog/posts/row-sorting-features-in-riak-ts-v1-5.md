...
When a query with an `ORDER BY` clause is issued, an instance of LevelDB is created on the node that received the request. When the query results are retrieved by that node, they are put into the temporary LevelDB instance and a key is created for each row using the `ORDER BY` clause. When `DESC` is applied to a column in the `ORDER BY` clause. The sort order is the same as described in the [LevelDB Sort Order](#user-content-leveldb-sort-order) section above and the value “negation” for local keys is used, described in the [Changing the Storage Order](#user-content-changing-the-storage-order) section above.
...
- [Download](http://docs.basho.com/riak/ts/latest/downloads/) Open Source
- [Sign In](https://basho.zendesk.com) to download Enterprise Edition
- [Install](http://docs.basho.com/riak/ts/1.5.0/setup/installing/) on a supported distribution
- Use [Riak Shell](http://docs.basho.com/riak/ts/latest/using/riakshell/) to execute the example statements and explore the new features.
