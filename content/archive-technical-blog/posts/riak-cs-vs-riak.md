...
date: "2013-10-02T09:00:29+00:00"
author: "John Daily"
original_url: "http://basho.com/posts/technical/riak-cs-vs-riak/"
archive_url: "https://web.archive.org/web/20170423104626http://basho.com/posts/technical/riak-cs-vs-riak"
categories:
...
[Riak CS](http://basho.com/riak-cloud-storage/) is Basho’s open source large object (aka cloud storage) software, built on the rock-solid [Riak](http://basho.com/riak/) database. It is API-compatible with Amazon’s S3 and OpenStack’s Swift object storage solutions.

In May of this year, we posted the [top 5 questions](http://basho.com/top-five-questions-about-riak-cs/) we heard from customers and our community about Riak CS; today we’ll take a deeper dive into the technical details, specifically the differences between Riak CS and Riak itself.
...
Riak CS carves large objects into small chunks of data to be distributed throughout a Riak cluster and, when used with [Riak CS Enterprise](http://basho.com/riak-cloud-storage/), synchronized with remote data centers.

### S3/OpenStack APIs

Without Riak CS, developers have the choice of using Riak’s native [HTTP](http://docs.basho.com/riak/latest/dev/references/http/) or [Protocol Buffers](http://docs.basho.com/riak/latest/dev/references/protocol-buffers/) APIs when developing solutions.

Riak CS adds compatibility with Amazon’s S3 and OpenStack’s Swift APIs. These offer very different semantics than Riak, and the advanced search capabilities in Riak such as [Secondary Indexes](http://docs.basho.com/riak/latest/dev/using/2i/) and [full text search](http://docs.basho.com/riak/latest/dev/using/search/) are not available using S3 or Swift clients.
...
[Work is actively underway](https://github.com/basho/riak/issues/355) to add a security model to Riak in the upcoming 2.0 release.
...
Basho offers multi-datacenter replication with its Enterprise software licenses, and [Riak CS Enterprise](http://basho.com/riak-cloud-storage/) takes full advantage of that feature. Data can be written to one or more clusters in multiple data centers and be synchronized automatically between them.
...
A common mistake while installing Riak CS is to configure it using information specific to Riak rather than Riak CS. As an example, per the [Riak CS installation instructions](http://docs.basho.com/riakcs/latest/cookbooks/configuration/Configuring-Riak/) the relevant backend data store must be configured to `riak_cs_kv_multi_backend`, which is forked from Riak’s `riak_kv_multi_backend`. Using the latter will cause problems.

### Riak (CS) Control

[Riak Control](https://github.com/basho/riak_control) is a web management console for Riak clusters; [Riak CS Control](https://github.com/basho/riak_cs_control) is a web management console for Riak CS *user accounts*. Both are optional and both are useful in a Riak CS cluster.
...
By building Riak CS atop the most robust open source distributed database in the world, we’ve created a very operationally friendly, powerful storage solution that can evolve to meet present and future needs. Feel free to [give it a try](http://docs.basho.com/riakcs/latest/) if you aren’t already using it.
