...
date: "2012-09-18T05:00:04+00:00"
author: "Basho"
original_url: "http://basho.com/posts/technical/rolling-with-eventual-consistency-2/"
archive_url: "https://web.archive.org/web/20170801115430http://basho.com/posts/technical/rolling-with-eventual-consistency-2/"
categories:
  - "Architecture & Distributed Systems"
---
*September 18, 2012*

In a [previous post](./choosing-the-right-tool.md) I wrote about the different mindset that a software engineer should have when building for a key-value database as opposed to a relational database. When working with a relational database, you describe the model first and then query the data later. With a key-value database, you focus first on what you want the result of the query to look like, and then work backward toward a model.
...
Using the client library [ripple](https://github.com/basho/ripple), we might define the object as follows:  
 `class DataPoint  
...
We can rely on Riak’s [vector clocks](http://basho.com/blog/technical/2010/01/29/why-vector-clocks-are-easy/) to solve this problem. First, we set the **allow\_mult** property on the bucket for **Statistic** objects so that Riak will store both copies of an object when two come in during a race condition.  
...
A working example of this solution is available in my [riak-rolling-average](https://github.com/clr/riak-rolling-average) repo. As usual, the good stuff is in the tests. You can run the tests with `bundle exec rake CLIENT=someclient` where *someclient* is unique to that Ruby thread.
...
If you aren’t familiar with my previous advice on [approaching key-value architecture](http://basho.com/blog/technical/2012/08/10/choosing-the-right-tool/), you might be tempted to solve a use case like the average described above using Riak’s map/reduce functionality. There are several reasons why the solution above would be preferred, but let us play the devil’s advocate and explore the map/reduce solution.
...
[Casey](https://twitter.com/caseyrosenthal)

#### Related

- A working example of this code is available in my [riak-rolling-average](https://github.com/clr/riak-rolling-average) repo. Check out the *mapreduce* branch for the production-unfriendly version using Riak’s map/reduce.
- [Vector Clocks are Easy](http://basho.com/blog/technical/2010/01/29/why-vector-clocks-are-easy/)
- [Vector Clocks are Hard](http://basho.com/blog/technical/2010/04/05/why-vector-clocks-are-hard/)
- [Meangirls](https://github.com/aphyr/meangirls) provides serializable data types for eventually consistent systems, similar to but more comprehensive than my example.
- [Hanover](https://github.com/bkerley/hanover) also provides eventually consistent data types.
- The **Statistic** object is an eventually consistent data type, an example of a CRDT: Convergent or Commutative Replicated Data Type. Read the comprehensive [research paper on CRDT](http://www.researchgate.net/publication/50949847_A_comprehensive_study_of_Convergent_and_Commutative_Replicated_Data_Types).
- Bryce Kerley speaking about [CRDTs at SRCFringe Edinburgh 2012](http://vimeo.com/45433300)
- I put some hours into a gem [ripple-statistics](https://github.com/clr/ripple-statistics) which adds this and other simple calculations to the [ripple](https://github.com/seancribbs/ripple) library. Note the limitations of the current code there.
