...
date: "2011-01-26T19:45:01+00:00"
author: "Bryan Fink"
original_url: "http://basho.com/posts/technical/fixing-the-count/"
archive_url: "https://web.archive.org/web/20170801115533http://basho.com/posts/technical/fixing-the-count/"
categories:
  - "Search & Analytics"
  - "Developer Tools"
---
...
Many thanks to commenter [Mike](http://blog.basho.com/2011/01/20/baseball-batting-average,-using-riak-map/reduce/#comment-133241522) for taking up the challenge I offered in [my last post](http://basho.com/blog/technical/2011/01/20/Baseball-Batting-Averages-Riak-Map-Reduce/). The flaw I was referring to was, indeed, the possibility that Luwak would split one of my records across two blocks.
...
If you’ve watched [Guy Steel’s recent talk about parallel programming](http://www.infoq.com/presentations/Thinking-Parallel-Programming), or read through the example luwak\_mr file [luwak\_mr\_words.erl](https://github.com/beerriot/luwak_mr/blob/master/src/luwak_mr_words.erl), you already know how chunk-merging works.
...
I’ve updated [baseball.erl](https://github.com/beerriot/baseball/blob/e826840a07c304eeba4d13ea111dbc99b70b7355/src/baseball.erl) to do just this. The map function now uses regexes much like those earlier in this post to [produce “suffix” and](https://github.com/beerriot/baseball/blob/e826840a07c304eeba4d13ea111dbc99b70b7355/src/baseball.erl#L186) [“prefix” results](https://github.com/beerriot/baseball/blob/e826840a07c304eeba4d13ea111dbc99b70b7355/src/baseball.erl#L186) for unhandled data at the start and end of the block. The reduce function then [combines these chunks and produces additional](https://github.com/beerriot/baseball/blob/e826840a07c304eeba4d13ea111dbc99b70b7355/src/baseball.erl#L205) [hit:at-bat results](https://github.com/beerriot/baseball/blob/e826840a07c304eeba4d13ea111dbc99b70b7355/src/baseball.erl#L205) that can be summed with the normal map output.
...
I’ve added [baseball\_flr.erl](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl) to the baseball project to demonstrate using fixed-length records. The two records needed from the “play” record for the batting average calculation are the player’s Retrosheet ID (the third field in the CSV format) and the play description (the sixth CSV field). The player ID is easy to handle: it’s already a fixed length of eight characters. The play description is, unfortunately, variable in length.
...
Setting up the block size is done at load time in [baseball\_flr:load\_events/1](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl#L57). The map function to calculate the batting average on this format has to change the way in which it [extracts each record from the block](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl#L140), but the analysis of the play data remains the same, and there is no need to worry about partial records. The [reduce](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl#L130)  
 [function](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl#L130) is exactly the same as it was before learning about chunks (though the chunk-handling version would also work; it just wouldn’t find any chunks to merge).
...
Aha: 1231 hits from both, but the new code found an extra at-bat — 4322 instead of 4321. The division says 0.28482 instead of 0.28488. I introduced more error by coding bad math ([truncating instead of rounding](https://github.com/beerriot/baseball/blob/59e44990a8a4bb1ebed73752c669b07be4d4d6e4/src/baseball_flr.erl#L117)) than I did by missing a record!
