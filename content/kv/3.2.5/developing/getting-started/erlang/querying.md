...
A more comprehensive discussion can be found in [Key/Value Modeling]({{< baseurl >}}kv/3.2.5/developing/key-value-modeling).
...
{{% note %}}
Secondary indexes in Riak KV require a sorted backend: [Memory]({{< baseurl >}}kv/3.2.5/setup/planning/backend/memory) or [LevelDB]({{< baseurl >}}kv/3.2.5/setup/planning/backend/leveldb). [Bitcask]({{< baseurl >}}kv/3.2.5/setup/planning/backend/bitcask) does not support secondary indexes.

See [Using Secondary Indexes (2i)]({{< baseurl >}}kv/3.2.5/developing/usage/secondary-indexes) for more information on developing with secondary indexes.
{{% /note %}}
