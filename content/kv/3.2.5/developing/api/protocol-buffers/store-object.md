...
intended [key]({{< baseurl >}}kv/3.2.5/learn/concepts/keys-and-objects), [bucket]({{< baseurl >}}kv/3.2.5/learn/concepts/buckets), and [bucket type]({{< baseurl >}}kv/3.2.5/developing/usage/bucket-types). A bucket must always be specified (via
...
[bucket type]({{< baseurl >}}kv/3.2.5/developing/usage/bucket-types) is assigned, Riak will assign
`default`, which means that the [default bucket configuration]({{< baseurl >}}kv/3.2.5/configuring/reference/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object]({{< baseurl >}}kv/3.2.5/developing/api/protocol-buffers/fetch-object)
...
as well as the object's [causal context]({{< baseurl >}}kv/3.2.5/learn/concepts/causal-context), in the `vclock`
