...
[cluster ops add remove node]: {{< baseurl >}}kv/3.2.5/using/cluster-operations/adding-removing-nodes
[config reference#strong-cons]: {{< baseurl >}}kv/3.2.5/configuring/reference/#strong-consistency
[use admin riak cli]: {{< baseurl >}}kv/3.2.5/using/admin/riak-cli
...
[glossary vnode]: {{< baseurl >}}kv/3.2.5/learn/glossary/#vnode
[concept buckets]: {{< baseurl >}}kv/3.2.5/learn/concepts/buckets
[cluster ops bucket types]: {{< baseurl >}}kv/3.2.5/using/cluster-operations/bucket-types
[use admin riak admin#ensemble]: {{< baseurl >}}kv/3.2.5/using/admin/riak-admin/#ensemble-status
[use admin riak admin]: {{< baseurl >}}kv/3.2.5/using/admin/riak-admin
[config reference#advanced]: {{< baseurl >}}kv/3.2.5/configuring/reference/#advanced-configuration
[plan cluster capacity]: {{< baseurl >}}kv/3.2.5/setup/planning/cluster-capacity
...
[concept causal context]: {{< baseurl >}}kv/3.2.5/learn/concepts/causal-context
[dev data types]: {{< baseurl >}}kv/3.2.5/developing/data-types
[glossary aae]: {{< baseurl >}}kv/3.2.5/learn/glossary/#active-anti-entropy-aae
[cluster ops 2i]: {{< baseurl >}}kv/3.2.5/using/reference/secondary-indexes
[usage commit hooks]: {{< baseurl >}}kv/3.2.5/developing/usage/commit-hooks
[cluster ops obj del]: {{< baseurl >}}kv/3.2.5/using/reference/object-deletion
...
currently existing ensembles can be checked using [`riak admin ensemble-status`][use admin riak admin#ensemble].
...
Item | Meaning
:----|:-------
`Enabled` | Whether the consensus subsystem is enabled on the current node, i.e. whether the `strong_consistency` parameter in [`riak.conf`][config reference#strong-cons] has been set to `on`. If this reads `off` and you wish to enable strong consistency, see our documentation on <a href="{{< baseurl >}}kv/3.2.5/configuring/reference/#strong-consistency">enabling strong consistency</a>.
...
`Ring Ready` | If `true`, then all of the [vnodes][glossary vnode] in the cluster have seen the current <a href="{{< baseurl >}}kv/3.2.5/learn/concepts/clusters/#the-ring">ring</a>, which means that the strong consistency subsystem can be used; if `false`, then the system is not yet ready. If you have recently added or removed one or more nodes to/from the cluster, it may take some time for `Ring Ready` to change.
`Validation` | This will display `strong` if the `tree_validation` setting in <code><a href="{{< baseurl >}}kv/3.2.5/configuring/reference/#strong-consistency">strong consistency</a></code> has been set to `on` and `weak` if set to `off`.
`Metadata` | This depends on the value of the `synchronous_tree_updates` setting in <code><a href="{{< baseurl >}}kv/3.2.5/configuring/reference/#strong-consistency">strong consistency</a></code>, which determines whether strong consistency-related Merkle trees are updated synchronously or asynchronously. If `best-effort replication (asynchronous)`, then `synchronous_tree_updates` is set to `false`; if `guaranteed replication (synchronous)` then `synchronous_tree_updates` is set to `true`.
...
* **Delete** operations work mostly like they do against
  ...
  [tombstones][cluster ops obj del] are not harvested, which is
