...
be best to start with our general guide to [authentication and authorization]({{< baseurl >}}kv/3.2.5/using/security/basics).
...
[created]({{< baseurl >}}kv/3.2.5/using/security/basics/#user-management) and that
security has been [enabled]({{< baseurl >}}kv/3.2.5/using/security/basics/#the-basics).
...
interface, and can be used only through the [protocol buffers interface]({{< baseurl >}}kv/3.2.5/developing/api/protocol-buffers/).
...
a different directory in your [`riak.conf`]({{< baseurl >}}kv/3.2.5/configuring/reference/) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`]({{< baseurl >}}kv/3.2.5/configuring/reference/) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your Riak cluster, you need to perform a [rolling restart]({{< baseurl >}}kv/3.2.5/using/repair-recovery/rolling-restart/). Once that process is complete, you can use the client
