...
At the bottom of the page, you'll find a [screencast]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/#screencast) that briefly explains how to adjust your
...
<a href="{{< baseurl >}}kv/3.4.0/reference/specialized-apis/strong-consistency-api/">strongly
...
<a href="{{< baseurl >}}kv/3.4.0/reference/specialized-apis/strong-consistency-api/">Using
...
of 3, and a `w` of 3 to all of the data in some of the [buckets]({{< baseurl >}}kv/3.4.0/explanation/data-model/keys-objects-and-buckets/) that
...
`pr` | PR | `0` | The number of primary <a href="{{< baseurl >}}kv/3.4.0/explanation/foundations/virtual-nodes/">vnodes</a> that must respond to a read request
`pw` | PW | `0` | The number of primary <a href="{{< baseurl >}}kv/3.4.0/explanation/foundations/virtual-nodes/">vnodes</a> that must respond to a write request
...
that they can be at the bucket level. You can use [bucket types]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/)
...
In OpenRiak's replication model, there are N [vnodes]({{< baseurl >}}kv/3.4.0/explanation/foundations/glossary/#vnode),
...
documentation on [Bitcask][plan backend bitcask], [LevelDB][plan backend leveldb], and [multiple backends]({{< baseurl >}}kv/3.4.0/explanation/storage/multi-backend/).
...
All of Basho's [official Riak clients]({{< baseurl >}}kv/3.4.0/reference/client-libraries/) enable you to
set replication properties this way. For more detailed information,
refer to the tutorial on [basic key/value operations in OpenRiak KV]({{< baseurl >}}kv/3.4.0/tutorials/first-application/)
...
and siblings, so we recommend reading the [Vector Clocks]({{< baseurl >}}kv/3.4.0/explanation/data-model/causal-context/#vector-clocks) documentation for more information.
...
2 of 3 vnodes agree on the value, and those 2 are the first to reach the coordinating node | The value is returned to the client. Read repair will deal with the conflict per the later scenarios, which means that a future read may return a different value or <a href="{{< baseurl >}}kv/3.4.0/explanation/data-model/causal-context/#siblings">siblings</a>
2 conflicting values reach the coordinating node and <a href="{{< baseurl >}}kv/3.4.0/explanation/data-model/causal-context/#vector-clocks">vector clocks</a> allow for resolution | The vector clocks are used to resolve the conflict and return a single value, which is propagated via read repair to the relevant vnodes
...
[concept clusters]: {{< baseurl >}}kv/3.4.0/explanation/foundations/clusters-rings-and-partitions/
[concept vnodes]: {{< baseurl >}}kv/3.4.0/explanation/foundations/virtual-nodes/
...
bucket's [bucket type]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/).
...
[Multi-Datacenter Replications]({{< baseurl >}}kv/3.4.0/how-to/configure/replication/configure-v3-multi-datacenter/) capabilities.
...
type]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/) with `n_val` set to your desired value and
...
queries, e.g. [secondary index]({{< baseurl >}}kv/3.4.0/how-to/develop/query-secondary-indexes/) and
[MapReduce]({{< baseurl >}}kv/3.4.0/how-to/develop/run-mapreduce/) queries. Lowering an object or bucket's
...
- The now legacy, [`riak_repl` replication]({{< baseurl >}}kv/3.4.0/explanation/replication/v2-and-v3-replication/) which was the recommended replication approach prior to Riak 3.0.10.
...
This guide covers the NextGen replication solution, and further information on alternatives are linked from the [legacy replication section]({{< baseurl >}}kv/3.4.0/explanation/replication/v2-and-v3-replication/).

Replication is considered to have three stages:

- Seeding; populating data in one cluster from another cluster.
  - Not covered in this guide.
  - The recommended approach to seeding using `repl_keys_range` is explained as part of [the AAE Fold API guide]({{< baseurl >}}kv/3.4.0/reference/aae-fold-api/).
...
- [An overview of the concepts]({{< baseurl >}}kv/3.4.0/explanation/replication/).
- [Configuration of real-time replication]({{< baseurl >}}kv/3.4.0/how-to/configure/replication/configure-real-time-replication/).
- [Configuration of all-cluster reconciliation]({{< baseurl >}}kv/3.4.0/how-to/configure/replication/configure-fullsync/).
- [Configuration of per-bucket reconciliation]({{< baseurl >}}kv/3.4.0/how-to/configure/replication/per-bucket-reconciliation/).
- [Managing a cluster migration]({{< baseurl >}}kv/3.4.0/how-to/configure/replication/migrate-cluster/).
- [The external Replication API]({{< baseurl >}}kv/3.4.0/reference/replication-api/).
- [Operations and the troubleshooting of replication]({{< baseurl >}}kv/3.4.0/how-to/operate/monitor-reconciliation/).
- [Configuring `riak_repl`]({{< baseurl >}}kv/3.4.0/explanation/replication/v2-and-v3-replication/).
- [Replication scope]({{< baseurl >}}kv/3.4.0/explanation/replication/reconciliation-scope/).
...
- [Queues and workers]({{< baseurl >}}kv/3.4.0/explanation/replication/queues/);
- [Replication references]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/);
- [Replication triggers]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/).
...
- [Replication sink nodes]({{< baseurl >}}kv/3.4.0/explanation/replication/sink-nodes/) — Explain replication sink nodes, its data flow, failure behavior, and operational trade-offs.
