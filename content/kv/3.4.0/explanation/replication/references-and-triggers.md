---
title: 'Replication references and triggers'
description: 'Explain how replication references and triggers select, queue, and transmit changes.'
weight: 12
...
Explain how replication references and triggers select, queue, and transmit changes.
...
>
> An option introduced in Riak version 2.0 is to use Riak as a [strongly consistent]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/strong-consistency-api/) system for data in specified buckets. Using Riak in this way is fundamentally different from adjusting replication properties and fine-tuning the availability/consistency trade-off, as it sacrifices
_all_ availability guarantees when necessary. Therefore, you
should consult the [Using Strong Consistency]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/strong-consistency-api/) documentation, as this option will not be covered
in this tutorial.
...
[below]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/#symbolic-consistency-names). Each
...
check out the [section below]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/#client-level-replication-settings)
...
documentation on [Bitcask]({{< baseurl >}}kv/3.4.0/explanation/storage/bitcask/), [LevelDB]({{< baseurl >}}kv/3.4.0/explanation/storage/leveldb/), and [multiple backends]({{< baseurl >}}kv/3.4.0/explanation/storage/multi-backend/).
...
refer to the section on [development usage with OpenRiak KV]({{< baseurl >}}kv/3.4.0/how-to/develop/)
...
### Multi-Datacenter Replication Reference: Per Bucket
...
#### How Bucket Properties Work in OpenRiak KV
...
Replication references may also be generated by [AAE folds]({{< baseurl >}}kv/3.4.0/reference/aae-fold-api/) when seeding clusters with data, or repairing large deltas.
...
Riak can be configured to operate in one of [three possible delete modes]({{< baseurl >}}kv/3.4.0/how-to/plan/choose-deletion-policy/).  Using a delete mode other than `keep`is tested with replication, but there will be an increased probability of false-negative reconciliation events that may consume resources on the cluster.
