...
[config reference#strong-cons]: /kv/3.4.0/reference/configuration/#strong-consistency
...
[use admin riak admin#ensemble]: /kv/3.4.0/reference/commands/riak-admin/#ensemble-status
...
[cluster ops obj del]: /kv/3.4.0/reference/operations/object-deletion/
...
`Ring Ready` | If `true`, then all of the [vnodes][glossary vnode] in the cluster have seen the current <a href="/kv/3.4.0/explanation/foundations/clusters-rings-and-partitions/#the-ring">ring</a>, which means that the strong consistency subsystem can be used; if `false`, then the system is not yet ready. If you have recently added or removed one or more nodes to/from the cluster, it may take some time for `Ring Ready` to change.
