...
It is possible to manage the potential for consistency issues [using conditional PUT logic (with token-based consensus)]({{< baseurl >}}kv/3.4.0/reference/http-api/conditional-requests/) that will block all but one of a concurrent update when the cluster is in a non-exceptional state.  However this control is only possible within a single cluster, there are no controls possible to block the creation of multi-value siblings when concurrent updates are allowed by the application into different clusters.
