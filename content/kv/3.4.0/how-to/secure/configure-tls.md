...
For the PB interface, it is not possible to enable TLS in isolation without [adding further security measures]({{< baseurl >}}kv/3.4.0/how-to/secure/enable-security/).  The configuration of file paths to certificate and key files is required as a prerequisite for applying those measures.  No independent listener is used for PB when security is enabled, the standard listener will expect TLS negotiation if and only if security is enabled.
