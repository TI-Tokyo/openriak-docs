...
features. For a look at network security in Riak, see [Security and Firewalls](/kv/3.4.0/how-to/secure/manage-sources/).
...
1. Check any Erlang MapReduce code for invocations of Riak modules other
   ...
   mechanism documented in [Installing Custom Code](/kv/3.4.0/reference/operations/custom-code/).
1. Make sure that your client software will work properly:
    * It must pass authentication information with each request
    * It must support HTTPS or encrypted [Protocol Buffers](/kv/3.4.0/reference/protocol-buffers/)
      traffic
...
Clients that use [Protocol Buffers](/kv/3.4.0/reference/protocol-buffers/) will typically have to be
...
permissions) is granted/revoked for all buckets and [bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/). If you specify a bucket type only, then the permission
...
Permission to perform [MapReduce](/kv/3.4.0/how-to/develop/run-mapreduce/) jobs can be assigned
...
In versions 2.0 and later, Riak users can manage [bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/) in addition to setting bucket properties. `riak admin
...
An more in-depth tutorial can be found in [Managing Security Sources](/kv/3.4.0/how-to/secure/manage-sources/).
...
`password`, `pam`, and `certificate` sources, please see our [Managing Security Sources](/kv/3.4.0/how-to/secure/manage-sources/) document.
...
i.e. the [`honor_cipher_order`](/kv/3.4.0/reference/configuration/#security) setting is set to `on`. If you prefer, however, that clients' preferred cipher
...
installing it can be found in [Installing Erlang](/kv/3.4.0/how-to/install/source/). This issue should
...
to enable it prior to enabling security. If you are using [Protocol Buffers](/kv/3.4.0/reference/protocol-buffers/) as a transport protocol for Riak (which we strongly recommend), enabling SSL on a given node requires only that you specify a [host and port](/kv/3.4.0/reference/configuration/#client-interfaces) for the node
as well as a [certification configuration](#certificate-configuration).

If, however, you are using the [HTTP API](/kv/3.4.0/reference/http-api/) for Riak and would like to
...
disabled and others enabled by setting the following [configurable parameters](/kv/3.4.0/reference/configuration/#security) to `on` or `off`:
...
If you are using any of the available [security sources](/kv/3.4.0/how-to/secure/manage-sources/), including [trust-based authentication](/kv/3.4.0/how-to/secure/manage-sources/#trust-based-authentication), you will need to do so
over a secure SSL connection. In order to establish a secure connection,
you will need to ensure that each OpenRiak node's [configuration files](/kv/3.4.0/reference/configuration/#security) point to the proper paths for your
...
on [configuring directories](/kv/3.4.0/reference/configuration/#directories).
...
If you are using [certificate-based authentication](/kv/3.4.0/how-to/secure/manage-sources/#certificate-based-authentication), Riak will check the certificate revocation list (CRL) of connecting clients' certificate by
