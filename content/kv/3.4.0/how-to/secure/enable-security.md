...
features. For a look at network security in Riak, see [Security and Firewalls]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/).
...
1. Check any Erlang MapReduce code for invocations of Riak modules other
   ...
   mechanism documented in [Installing Custom Code]({{< baseurl >}}kv/3.4.0/reference/operations/custom-code/).
1. Make sure that your client software will work properly:
    * It must pass authentication information with each request
    * It must support HTTPS or encrypted [Protocol Buffers]({{< baseurl >}}kv/3.4.0/reference/protocol-buffers/)
      traffic
...
Clients that use [Protocol Buffers]({{< baseurl >}}kv/3.4.0/reference/protocol-buffers/) will typically have to be
...
permissions) is granted/revoked for all buckets and [bucket types]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/). If you specify a bucket type only, then the permission
...
Permission to perform [MapReduce]({{< baseurl >}}kv/3.4.0/how-to/develop/run-mapreduce/) jobs can be assigned
...
In versions 2.0 and later, Riak users can manage [bucket types]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/) in addition to setting bucket properties. `riak admin
...
An more in-depth tutorial can be found in [Managing Security Sources]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/).
...
`password`, `pam`, and `certificate` sources, please see our [Managing Security Sources]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/) document.
...
i.e. the [`honor_cipher_order`]({{< baseurl >}}kv/3.4.0/reference/configuration/#security) setting is set to `on`. If you prefer, however, that clients' preferred cipher
...
installing it can be found in [Installing Erlang]({{< baseurl >}}kv/3.4.0/how-to/install/source/). This issue should
...
to enable it prior to enabling security. If you are using [Protocol Buffers]({{< baseurl >}}kv/3.4.0/reference/protocol-buffers/) as a transport protocol for Riak (which we strongly recommend), enabling SSL on a given node requires only that you specify a [host and port]({{< baseurl >}}kv/3.4.0/reference/configuration/#client-interfaces) for the node
as well as a [certification configuration](#certificate-configuration).

If, however, you are using the [HTTP API]({{< baseurl >}}kv/3.4.0/reference/http-api/) for Riak and would like to
...
disabled and others enabled by setting the following [configurable parameters]({{< baseurl >}}kv/3.4.0/reference/configuration/#security) to `on` or `off`:
...
If you are using any of the available [security sources]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/), including [trust-based authentication]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/#trust-based-authentication), you will need to do so
over a secure SSL connection. In order to establish a secure connection,
you will need to ensure that each OpenRiak node's [configuration files]({{< baseurl >}}kv/3.4.0/reference/configuration/#security) point to the proper paths for your
...
on [configuring directories]({{< baseurl >}}kv/3.4.0/reference/configuration/#directories).
...
If you are using [certificate-based authentication]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-sources/#certificate-based-authentication), Riak will check the certificate revocation list (CRL) of connecting clients' certificate by
...
- [TLS enablement]({{< baseurl >}}kv/3.4.0/how-to/secure/configure-tls/);
- [Enabling Security and restricting the Source of requests]({{< baseurl >}}kv/3.4.0/how-to/secure/enable-security/);
- [Granting permissions for specific actions]({{< baseurl >}}kv/3.4.0/how-to/secure/manage-permissions/).
