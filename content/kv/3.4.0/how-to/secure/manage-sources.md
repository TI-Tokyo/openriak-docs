...
be best to start with our general guide to [authentication and authorization](/kv/3.4.0/how-to/secure/enable-security/).
...
[created](/kv/3.4.0/how-to/secure/enable-security/#user-management) and that
security has been [enabled](/kv/3.4.0/how-to/secure/enable-security/).
...
interface, and can be used only through the [protocol buffers interface](/kv/3.4.0/reference/protocol-buffers/).
...
a different directory in your [`riak.conf`](/kv/3.4.0/reference/configuration/) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`](/kv/3.4.0/reference/configuration/) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your OpenRiak cluster, you need to perform a [rolling restart](/kv/3.4.0/how-to/operate/rolling-restart/). Once that process is complete, you can use the client
...
[Managing group]: /kv/3.4.0/how-to/secure/manage-sources/
