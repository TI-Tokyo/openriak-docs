...
In Riak versions 2.0 and later, Basho suggests that you [use bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/) to namespace and configure all buckets you use. Bucket types have a lower overhead within the cluster than the
...
In Riak 2.0 the handling of siblings was improved by the enabling of dotted [version vectors](/kv/3.4.0/explanation/data-model/version-vectors-and-siblings/).  All buckets should use `{dvv_enabled, true}`.  The introduction of DVV did not force non-typed buckets to use DVV, and by default non-typed buckets will continue to use legacy vector clocks.
...
When using [conflict-free replicated data types](/kv/3.4.0/reference/specialized-apis/data-type-api/), `{allow_mult, true}` must always be used.
