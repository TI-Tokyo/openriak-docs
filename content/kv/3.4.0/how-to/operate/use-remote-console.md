...
Advanced information and debugging tools are available from the command line via `riak remote_console`.  This will attach a remote shell to the running node.  With this shell Erlang functions can be called as if on the local node, and this can be used for: [accessing objects]({{< baseurl >}}kv/3.4.0/how-to/operate/inspect-data/), [running AAE folds]({{< baseurl >}}kv/3.4.0/how-to/operate/aae-fold/run-from-command-line/), [access to specific administration commands]({{< baseurl >}}kv/3.4.0/reference/operations/remote-console/) as well as [advanced debugging and troubleshooting]({{< baseurl >}}kv/3.4.0/how-to/troubleshoot/erlang-vm/).
