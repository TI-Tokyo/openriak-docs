...
your OpenRiak cluster (which is different from [recovering a failed node](/kv/3.4.0/explanation/operations/node-failure-and-recovery/)). Here is the recommended way to go
...
3. Start the new `riak7` node with [`riak start`](/kv/3.4.0/reference/commands/riak/):
...
participating in the cluster; for example `riak0` with the [`riak admin cluster join`](/kv/3.4.0/reference/commands/riak-admin/#cluster) command executed on the new `riak7` node:
...
`riak7` node using the [`riak admin cluster replace`](/kv/3.4.0/reference/commands/riak-admin/#cluster) command:
...
6. Examine the proposed cluster changes with the [`riak admin cluster plan`](/kv/3.4.0/reference/commands/riak-admin/#cluster) command executed on the new
`riak7` node:
...
[`riak admin cluster commit`](/kv/3.4.0/reference/commands/riak-admin/#cluster) command:

```bash
    riak admin cluster commit
    ```

If you need to clear the proposed plan and start over, use [`riak admin cluster clear`](/kv/3.4.0/reference/commands/riak-admin/#cluster):
...
with the [`riak admin ringready`](/kv/3.4.0/reference/commands/riak-admin/#ringready)
and [`riak admin member-status`](/kv/3.4.0/reference/commands/riak-admin/#member-status)
commands.
...
- [ensuring the node is downed](/kv/3.4.0/how-to/operate/replace-node/);
- [forcing the replace](/kv/3.4.0/how-to/operate/replace-node/);
- [repairing the new node](/kv/3.4.0/how-to/troubleshoot/recover-failed-node/).
...
The new node should be started with [`participate_in_coverage` disabled](/kv/3.4.0/reference/operations/remote-console/), as it will at this stage be a full member of the cluster but have no data.  It is also more efficient to suspend anti-entropy until the repair is complete.
