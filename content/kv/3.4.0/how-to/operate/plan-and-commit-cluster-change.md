...
- The `target_n_val`; which should be greater than or equal to the `n_val`.
  ...
  - The `target_n_val` is the number of [primaries and fallbacks]({{< baseurl >}}kv/3.4.0/explanation/foundations/clusters-rings-and-partitions/) which must be on distinct nodes.
