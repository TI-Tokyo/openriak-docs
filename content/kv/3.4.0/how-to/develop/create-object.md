...
[usage content types]: /kv/3.4.0/how-to/develop/use-content-types/
...
As an example, let's store an object containing information about a dog named Rufus. We'll store that object in the key `rufus` in the bucket `dogs`, which bears the `animals` [bucket type](/kv/3.4.0/how-to/operate/manage-bucket-types/).
...
Now, you run the same read operation as in [Reading Objects](/kv/3.4.0/how-to/develop/read-object/). If the write operation was successful, you should be able to successfully read the object. Please note that the operation will fail if you don't first create the bucket-type `animals` as per the page on [bucket types](/kv/3.4.0/how-to/operate/manage-bucket-types/).
...
[`default`](/kv/3.4.0/how-to/develop/use-bucket-types/) will be applied.
