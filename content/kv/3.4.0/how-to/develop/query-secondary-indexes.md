...
recommend [Riak Search](/kv/3.4.0/reference/specialized-apis/legacy-query-api/) rather than secondary indexes for
...
be used to find multiple objects in Riak. If you're storing [user data](/kv/3.4.0/how-to/plan/map-data-to-objects/), for example, you could tag each object
...
[Riak Search](/kv/3.4.0/reference/specialized-apis/legacy-query-api/) serves analogous purposes but is quite
...
* Query results can be used as input to a [MapReduce](/kv/3.4.0/how-to/develop/run-mapreduce/)
  query
...
[strongly consistent buckets](/kv/3.4.0/reference/specialized-apis/strong-consistency-api/) and attach
...
mechanism that does not require a schema (as does [Riak Search](/kv/3.4.0/reference/specialized-apis/legacy-query-api/)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search](/kv/3.4.0/reference/specialized-apis/legacy-query-api/).
* When you want to use composite queries. A query like
  ...
  [MapReduce](/kv/3.4.0/how-to/develop/run-mapreduce/)).
...
>
> If you are connecting to Riak using one of Basho's official [client libraries](/kv/3.4.0/reference/client-libraries/), you can find more information about getting started with
your client in the [Developing with OpenRiak KV: Getting Started](/kv/3.4.0/tutorials/first-application/) section.
...
>
> For all 2i queries, the [R](/kv/3.4.0/explanation/replication/references-and-triggers/#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs](/kv/3.4.0/explanation/foundations/glossary/#hinted-handoff) and related operations are underway may not
