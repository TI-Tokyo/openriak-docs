...
description: 'Show developers how to query secondary indexes with a minimal verified example.'
weight: 5
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show developers how to query secondary indexes with a minimal verified example.
...
recommend [Riak Search]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/legacy-query-api/) rather than secondary indexes for
...
be used to find multiple objects in Riak. If you're storing [user data]({{< baseurl >}}kv/3.4.0/how-to/plan/map-data-to-objects/), for example, you could tag each object
...
[Riak Search]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/legacy-query-api/) serves analogous purposes but is quite
...
* Query results can be used as input to a [MapReduce]({{< baseurl >}}kv/3.4.0/how-to/develop/run-mapreduce/)
  query
...
[strongly consistent buckets]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/strong-consistency-api/) and attach
...
mechanism that does not require a schema (as does [Riak Search]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/legacy-query-api/)) and a basic query interface, i.e. an interface that
...
* When you need more than the exact match and range searches that 2i
  supports. If that's the case, we recommend checking out [Riak Search]({{< baseurl >}}kv/3.4.0/reference/specialized-apis/legacy-query-api/).
* When you want to use composite queries. A query like
  ...
  [MapReduce]({{< baseurl >}}kv/3.4.0/how-to/develop/run-mapreduce/)).
...
obj.getIndexes().getIndex(StringBinIndex.named("email")).add("jsmith@example.com");
...
obj.indexes['email_bin'] = %w{ jsmith@example.com }
obj.store
...
$object = (new \Basho\Riak\Object('{"user_data":{ ... }}', ['Content-type' => 'application/json']))
  ...
  ->addValueToIndex('email_bin', 'jsmith@example.com');
...
obj.add_index('email_bin', 'jsmith@example.com')
obj.store()
```
...
obj.BinIndex("email").Set"jsmith@example.com");
...
riakObj.addToIndex('email_bin', 'jsmith@example.com');
...
MD2 = riakc_obj:set_secondary_index(
    ...
    [{{binary_index, "twitter"}, [<<"jsmith123">>]},
     {{binary_index, "email"}, [<<"jsmith@example.com">>]}]),
...
obj.AddToIndex("email_bin", "jsmith@example.com")
...
curl -XPOST localhost:8098/types/default/buckets/users/keys/john_smith \
  ...
  -H 'x-riak-index-email_bin: jsmith@example.com' \
...
>
> If you are connecting to Riak using one of Basho's official [client libraries]({{< baseurl >}}kv/3.4.0/reference/client-libraries/), you can find more information about getting started with
your client in the [Developing with OpenRiak KV: Getting Started]({{< baseurl >}}kv/3.4.0/tutorials/first-application/) section.
...
>
> For all 2i queries, the [R]({{< baseurl >}}kv/3.4.0/explanation/replication/references-and-triggers/#r-value-and-read-failure-tolerance) parameter is set to 1,
which means that queries that are run while [handoffs]({{< baseurl >}}kv/3.4.0/explanation/foundations/glossary/#hinted-handoff) and related operations are underway may not
