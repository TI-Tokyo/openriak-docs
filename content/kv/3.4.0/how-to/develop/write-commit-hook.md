---
title: 'Write a commit hook'
description: 'Show developers how to write a commit hook with a minimal verified example.'
weight: 17
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show developers how to write a commit hook with a minimal verified example.
...
Pre- and post-commit hooks are applied at the [bucket]({{< baseurl >}}kv/3.4.0/explanation/data-model/keys-objects-and-buckets/) level,
...
Because hooks are defined at the bucket level, you can create [bucket types]({{< baseurl >}}kv/3.4.0/how-to/develop/use-bucket-types/)
...
will be usable through our OpenRiak cluster:
...
documentation on [object deletion]({{< baseurl >}}kv/3.4.0/reference/operations/object-deletion/)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API]({{< baseurl >}}kv/3.4.0/reference/http-api/)) along with a generic error message about why the write was blocked.
