...
Pre- and post-commit hooks are applied at the [bucket](/kv/3.4.0/explanation/data-model/keys-objects-and-buckets/) level,
...
Because hooks are defined at the bucket level, you can create [bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/)
...
documentation on [object deletion](/kv/3.4.0/reference/operations/object-deletion/)) to determine whether a delete is
occurring.
...
- `fail` - The atom `fail` will cause Riak to fail the write and send a 403 Forbidden error response (in the [HTTP API](/kv/3.4.0/reference/http-api/)) along with a generic error message about why the write was blocked.
