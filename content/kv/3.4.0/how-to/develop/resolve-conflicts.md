...
One of OpenRiak's [central goals](../../../learn/why-riak-kv) is high availability. It was built as a [clustered](/kv/3.4.0/explanation/foundations/clusters-rings-and-partitions/) system in which any [node](/kv/3.4.0/explanation/foundations/glossary/#node) is capable of receiving requests without requiring that
every node participate in each request.

If you are using Riak in an [eventually consistent](/kv/3.4.0/explanation/consistency/eventual-consistency/) way, conflicts between object values on different nodes is
unavoidable. Often, Riak can resolve these conflicts on its own
internally if you use causal context, i.e. [vector clocks](/kv/3.4.0/explanation/data-model/causal-context/#vector-clocks) or [dotted version vectors](/kv/3.4.0/explanation/data-model/causal-context/#dotted-version-vectors), when updating objects. Instructions on this can be found in the section [below](#siblings).
...
available options can be found in the [section below](/kv/3.4.0/how-to/develop/resolve-conflicts/). For now,
...
1. If your data can be modeled as one of the currently available [Riak
   Data Types](/kv/3.4.0/reference/data/distributed-data-types/), we recommend using one of these types,
...
>
> * [Using Strong Consistency](/kv/3.4.0/reference/specialized-apis/strong-consistency-api/) - A guide for developers
> * [Managing Strong Consistency](/kv/3.4.0/how-to/configure/strong-consistency/) - A guide for operators
...
While Riak _does_ have a set of [defaults](/kv/3.4.0/explanation/replication/references-and-triggers/#available-parameters), there are a variety of general
...
[using bucket types][usage bucket types]. The most important [bucket properties](/kv/3.4.0/explanation/data-model/keys-objects-and-buckets/)
...
[`last_write_wins`](/kv/3.4.0/explanation/data-model/keys-objects-and-buckets/). If `last_write_wins` is set to `false`,
...
* [Java](/kv/3.4.0/how-to/develop/resolve-conflicts/)
* [Ruby](/kv/3.4.0/how-to/develop/resolve-conflicts/)
* [Python](/kv/3.4.0/how-to/develop/resolve-conflicts/)
* [C#](/kv/3.4.0/how-to/develop/resolve-conflicts/)
* [Node.js](/kv/3.4.0/how-to/develop/resolve-conflicts/)

In Riak versions 2.0 and later, `allow_mult` is set to `true` by default
for any [bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/) that you create. This means
that if you wish to avoid client-side sibling resolution, you have a few
options:

* Explicitly create and activate [bucket types](/kv/3.4.0/how-to/develop/use-bucket-types/)
  that set `allow_mult` to `false`
* Use OpenRiak's [Configuration Files](/kv/3.4.0/reference/configuration/) to change the [default bucket properties](/kv/3.4.0/reference/configuration/#default-bucket-properties) for your
...
on what value you set for `dvv_enabled`. If set to `true`, [dotted version vectors](/kv/3.4.0/explanation/data-model/causal-context/#dotted-version-vectors) will be used; if set to `false` (the default), [vector clocks](/kv/3.4.0/explanation/data-model/causal-context/#vector-clocks) will be used.
...
[sibling explosion](/kv/3.4.0/how-to/tune/reduce-latency/#siblings), which can
...
Basho's official [client libraries](/kv/3.4.0/reference/client-libraries/). Examples can be found for each
client library in the [Object Updates](/kv/3.4.0/how-to/develop/update-object/) document.
...
[causal context](/kv/3.4.0/explanation/data-model/causal-context/). This is a less likely scenario if a client updates
...
[client libraries](/kv/3.4.0/reference/client-libraries/), you can find more information about getting started with your client in [Developing with OpenRiak KV: Getting Started](/kv/3.4.0/tutorials/first-application/) section.
...
the entire node. Other issues include [increased cluster latency](/kv/3.4.0/how-to/tune/reduce-latency/) as the object is replicated and out-of-memory errors.
...
For reasons explained in the [Introduction to conflict resolution](/kv/3.4.0/how-to/develop/resolve-conflicts/), we strongly recommend adopting a conflict resolution strategy that requires applications to resolve siblings according to use-case-specific
...
For reasons explained in the [Introduction to conflict resolution](/kv/3.4.0/how-to/develop/resolve-conflicts/), we strongly recommend adopting a conflict resolution strategy that
...
For reasons explained in the [Introduction to conflict resolution](/kv/3.4.0/how-to/develop/resolve-conflicts/), we strongly recommend adopting a
...
[bucket type](/kv/3.4.0/how-to/develop/use-bucket-types/) `siblings`, and for this bucket type
...
the official Java client library, in the [Developing with OpenRiak KV: Usage](/kv/3.4.0/how-to/develop/) section.
...
resolution is that Riak offers a variety of [Data Types](/kv/3.4.0/reference/data/distributed-data-types/) that have
specific conflict resolution mechanics built in. If you have data that
can be modeled as a [counter](/kv/3.4.0/reference/data/distributed-data-types/), [set](/kv/3.4.0/reference/data/distributed-data-types/), or [map](/kv/3.4.0/reference/data/distributed-data-types/), then you should seriously
...
resolution that we implemented above, see our [tutorial on Riak sets](/kv/3.4.0/reference/data/distributed-data-types/).
...
You can find more on writing objects to Riak, including examples from
the official PHP client library, in the [Developing with OpenRiak KV: Usage](/kv/3.4.0/how-to/develop/) section.
...
application will be stored in buckets that bear the [bucket type](/kv/3.4.0/how-to/develop/use-bucket-types/) `siblings`, and for this bucket type `allow_mult` is set
...
from the official Python client library, in the [Developing with OpenRiak KV: Usage](/kv/3.4.0/how-to/develop/) section.
...
You can find more on writing objects to Riak, including examples from
the official Ruby client library, in the [Developing with OpenRiak KV: Usage](/kv/3.4.0/how-to/develop/) section.
