...
description: 'Answer common OpenRiak KV development and operational questions.'
weight: 90
diataxis: 'reference'
product: 'OpenRiak KV'
...
legacy_3_2_5_sources:
  - 'C:\Users\pjacl\Downloads\riak-docs-fork\kv\3.2.5\developing\faq.md'
...
Answer common OpenRiak KV development and operational questions.
...
[Bucket Properties]: {{< baseurl >}}kv/3.4.0/how-to/develop/
...
[Configuration Files]: {{< baseurl >}}kv/3.4.0/reference/configuration/
[contrib.basho.com]: https://github.com/basho/riak_function_contrib
[Erlang Riak Client]: {{< baseurl >}}kv/3.4.0/reference/client-libraries/
...
[System Planning]: {{< baseurl >}}kv/3.4.0/how-to/plan/#network-configuration-load-balancing
[vector clocks]: {{< baseurl >}}kv/3.4.0/explanation/data-model/causal-context/#vector-clocks
...
**A:**
  ...
  Bucket properties for non-default bucket types are stored in the cluster metadata system.  The cluster metadata system is a more efficient way of replicating this information around an OpenRiak cluster.
...
**Q: How do I spread requests across---i.e. load balance---an OpenRiak cluster?**

**A:**
  There are at least two acceptable strategies for load balancing requests across your OpenRiak cluster: **virtual IPs** and **reverse-proxy**.

For further information see [System Planning].

<a name="restart-merges"></a>
**Q: Why does it seem that Bitcask merging is only triggered when an OpenRiak node is restarted?**
  There have been situations where the data directory for an OpenRiak node (e.g. `data/bitcask`) grows continually and does not seem to merge. After restarting the node a series of merges are kicked off and the total size of the data directory shrinks. Why does this happen?
...
You could also run `keys=stream` and close the connection when you have the designated number. This will not, however, reduce load on the OpenRiak cluster. It will only reduce load on your client.
