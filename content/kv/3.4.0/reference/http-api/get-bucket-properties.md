...
default). See also [HTTP List Keys]({{< baseurl >}}kv/3.4.0/reference/http-api/list-keys/).
...
See [HTTP Set Bucket Properties]({{< baseurl >}}kv/3.4.0/reference/http-api/set-bucket-properties/) for more information about the available
bucket properties. See [Managing Bucket Types Through the Command Line]({{< baseurl >}}kv/3.4.0/reference/data/buckets-and-bucket-types/#managing-bucket-types-through-the-command-line) for more details about reading bucket types using  the `riak admin bucket-type` interface.
