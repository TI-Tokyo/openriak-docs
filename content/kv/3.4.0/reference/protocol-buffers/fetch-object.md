...
[`RpbPutResp`]({{< baseurl >}}kv/3.4.0/reference/protocol-buffers/store-object/), respectively):
...
* `content_encoding` - The content encoding of the object, e.g.
  `video/mp4`
* `vtag` - The object's [vtag]({{< baseurl >}}kv/3.4.0/explanation/foundations/glossary/#vector-clock)
...
```protobuf
    ...
    `RpbPair` messages are also used to attach [secondary indexes]({{< baseurl >}}kv/3.4.0/how-to/develop/query-secondary-indexes/) to objects (in the optional
    `indexes` field).
