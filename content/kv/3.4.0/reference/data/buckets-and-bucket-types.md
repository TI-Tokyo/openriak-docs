...
older configuration system based on [bucket properties](/kv/3.4.0/how-to/develop/use-bucket-types/).
...
[HTTP](/kv/3.4.0/reference/http-api/set-bucket-properties/) or [Protocol Buffers](/kv/3.4.0/reference/protocol-buffers/set-bucket-properties/). With this approach, you can take any given bucket and
...
* Nearly all bucket properties can be updated using bucket types, except the
  ...
  [Riak data types](/kv/3.4.0/reference/data/distributed-data-types/), and [strong consistency](/kv/3.4.0/reference/specialized-apis/strong-consistency-api/) respectively
...
way that they are configured when using [bucket properties](/kv/3.4.0/how-to/develop/use-bucket-types/). You cannot simply take a
...
>
> If you are connecting to Riak using one of Basho's official [client libraries](/kv/3.4.0/reference/client-libraries/), you can find more information about getting started with your client in our [Developing with OpenRiak KV: Getting Started](/kv/3.4.0/tutorials/first-application/) section.
...
Prior to Riak 2.0, Riak created [siblings](/kv/3.4.0/explanation/data-model/causal-context/#siblings) in the case of conflicting updates only when explicitly instructed to do so, i.e. when `allow_mult` is to `true`. The default `allow_mult` setting was `false`.
...
do so. New applications, however, are encouraged to retain and [resolve siblings](/kv/3.4.0/how-to/develop/resolve-conflicts/) with the appropriate application-side business logic.
...
`user_account_bucket` with a [pre-commit hook](/kv/3.4.0/how-to/develop/write-commit-hook/#pre-commit-hooks) called `syntax_check` and two [post-commit
hooks](/kv/3.4.0/how-to/develop/write-commit-hook/) called `welcome_email` and `update_registry`. This would involve four steps:
...
because buckets act as a [separate namespace](/kv/3.4.0/reference/data/buckets-and-bucket-types/#buckets-as-namespaces) in Riak, in addition to [buckets](/kv/3.4.0/explanation/data-model/keys-objects-and-buckets/) and [keys](/kv/3.4.0/explanation/data-model/keys-objects-and-buckets/).
...
[buckets]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
[howmany]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
[buckettypes]: #bucket-types
[howwork]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
[when]: #when-to-use-bucket-types
[createactivate]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
[namespace]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
[allowmulti]: /kv/3.4.0/reference/data/buckets-and-bucket-types/
