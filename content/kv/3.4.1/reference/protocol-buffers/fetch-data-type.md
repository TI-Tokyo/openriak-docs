...
The equivalent of [`RpbGetReq`](/kv/3.4.1/reference/protocol-buffers/fetch-object/) for [Riak Data Types](/kv/3.4.1/reference/data/distributed-data-types/). This request results in a `DtFetchResp`
...
`type` | The [Using Bucket Types](/kv/3.4.1/how-to/operate/manage-bucket-types/) of the bucket in which the Data Type is stored, _not_ the type of Data Type (i.e. counter, set, or map)
...
The response to a fetch request ([`DtFetchReq`](/kv/3.4.1/reference/protocol-buffers/fetch-data-type/)) is a `DtFetchResp` message.
...
would send a [vclock](/kv/3.4.1/explanation/foundations/glossary/#vector-clock) for standard KV updates.
