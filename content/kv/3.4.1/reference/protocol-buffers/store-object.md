...
intended [key](/kv/3.4.1/explanation/data-model/keys-objects-and-buckets/), [bucket](/kv/3.4.1/explanation/data-model/keys-objects-and-buckets/), and [bucket type](/kv/3.4.1/how-to/develop/use-bucket-types/). A bucket must always be specified (via
...
[bucket type](/kv/3.4.1/how-to/develop/use-bucket-types/) is assigned, Riak will assign
`default`, which means that the [default bucket configuration](/kv/3.4.1/reference/configuration/#default-bucket-properties) will be used.
...
`content` | The new or updated contented of the object. Uses the same `RpbContent` message returned as part of an `RpbGetResp` message, documented in [PBC Fetch Object](/kv/3.4.1/reference/protocol-buffers/fetch-object/)
...
as well as the object's [causal context](/kv/3.4.1/explanation/data-model/causal-context/), in the `vclock`
