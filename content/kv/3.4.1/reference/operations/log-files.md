...
* `file` - Console logs will be emitted to a file. This is OpenRiak's
    ...
    `./log/console.log` on an installation from [source]({{< baseurl >}}kv/3.4.1/how-to/install/source/), but will differ on platform-specific installation,
...
* `console` - Console logs will be emitted to standard output, which
    can be viewed by running the [`riak attach-direct`]({{< baseurl >}}kv/3.4.1/reference/commands/riak/#attach-direct) command
