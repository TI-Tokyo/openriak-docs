---
title: 'Handoff behavior and state'
description: 'Define the names, fields, states, limits, and version applicability for handoff behavior and state.'
weight: 3
diataxis: 'reference'
product: 'OpenRiak KV'
...
Define the names, fields, states, limits, and version applicability for handoff behavior and state.
...
* **fault tolerance**, whereby an OpenRiak cluster can withstand node
...
Hinted handoff occurs when a [vnode]({{< baseurl >}}kv/3.4.1/explanation/foundations/glossary/#vnode) temporarily takes over responsibility for some data and then returns that data to its original "owner." Imagine a 3-node cluster with nodes A, B, and C. If node C goes offline, e.g. during a network partition, nodes A and B will pick
...
It occurs when a [vnode]({{< baseurl >}}kv/3.4.1/explanation/foundations/glossary/#vnode) no longer belongs to the node on which it's running. This typically happens when the very
...
particular nodes or all nodes and of configuring key aspects of OpenRiak's
handoff behavior. More information can be found below.

#### Configuring Handoff

A full listing of configurable parameters can be found in our
[configuration files]({{< baseurl >}}kv/3.4.1/reference/configuration/#intra-cluster-handoff)
document. The sections below provide a more narrative description of
handoff configuration.

##### SSL

If you want to encrypt handoff behavior within an OpenRiak cluster, you need
...
duration that a [vnode]({{< baseurl >}}kv/3.4.1/explanation/foundations/glossary/#vnode) can be blocked by multiplying the
`handoff.max_rejects` setting by the value of
[`vnode_management_timer`]({{< baseurl >}}kv/3.4.1/reference/configuration/#vnode_management_timer).
