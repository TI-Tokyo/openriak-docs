...
>
> An option introduced in Riak version 2.0 is to use Riak as a [strongly consistent](/kv/3.4.1/reference/specialized-apis/strong-consistency-api/) system for data in specified buckets. Using Riak in this way is fundamentally different from adjusting replication properties and fine-tuning the availability/consistency trade-off, as it sacrifices
_all_ availability guarantees when necessary. Therefore, you
should consult the [Using Strong Consistency](/kv/3.4.1/reference/specialized-apis/strong-consistency-api/) documentation, as this option will not be covered
in this tutorial.
...
[below](/kv/3.4.1/explanation/replication/references-and-triggers/#symbolic-consistency-names). Each
...
check out the [section below](/kv/3.4.1/explanation/replication/references-and-triggers/#client-level-replication-settings)
...
documentation on [Bitcask](/kv/3.4.1/explanation/storage/bitcask/), [LevelDB](/kv/3.4.1/explanation/storage/leveldb/), and [multiple backends](/kv/3.4.1/explanation/storage/multi-backend/).
...
refer to the section on [development usage with OpenRiak KV](/kv/3.4.1/how-to/develop/)
...
Replication references may also be generated by [AAE folds](/kv/3.4.1/reference/aae-fold-api/) when seeding clusters with data, or repairing large deltas.
...
Riak can be configured to operate in one of [three possible delete modes](/kv/3.4.1/how-to/plan/choose-deletion-policy/).  Using a delete mode other than `keep`is tested with replication, but there will be an increased probability of false-negative reconciliation events that may consume resources on the cluster.
