...
are available. More on this in the section on [trade-offs](/kv/3.4.1/explanation/consistency/strong-consistency/).
...
status of a node. This data is available by running the [`riak admin status`](/kv/3.4.1/reference/commands/riak-admin/#status) command. That data now
includes statistics specific to strongly consistent operations.

A full listing of these stats is available in [Inspecting a Node](/kv/3.4.1/how-to/operate/inspect-node-and-cluster/).
