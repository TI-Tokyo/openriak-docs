...
- Not persisted, all data on an individual node will be lost on restart.
  - Note that Riak clusters are resilient to the loss of data on a single node, but constraining the ability to perform [rolling restarts](/kv/3.4.1/how-to/operate/rolling-restart/) of Riak due to data loss, may cause operational overheads.
