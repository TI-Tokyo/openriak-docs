...
object that's stored there---including its [siblings]({{< baseurl >}}kv/3.4.1/how-to/develop/resolve-conflicts/#siblings) \(more on that later)---or it returns `not found` (the
...
`dogs`, which bears the bucket type `animals`. Please note that for this example to work, you must have first created the bucket-type `animals` as per the instructions on the [bucket type]({{< baseurl >}}kv/3.4.1/how-to/operate/manage-bucket-types/) page.
