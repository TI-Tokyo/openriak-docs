...
[aae read repair]: {{< baseurl >}}kv/3.4.1/explanation/replication/active-anti-entropy/#read-repair-vs-active-anti-entropy
...
Data could exist on some nodes and not others at the exact time a backup is made. Any inconsistency will be corrected once a backup is restored, either by OpenRiak's [active anti-entropy]({{< baseurl >}}kv/3.4.1/explanation/replication/active-anti-entropy/) processes or when the object is read, via [read repair]({{< baseurl >}}kv/3.4.1/explanation/replication/active-anti-entropy/#read-repair-vs-active-anti-entropy).
...
**Deprecation notice**
In previous versions of OpenRiak KV, there was a [`riak admin backup`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/#backup) command commonly used for
...
1. Mark the original instance down in the cluster using
   [`riak admin down <node>`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/#down)
2. Join the restored node to the cluster using
   [`riak admin cluster join <node>`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/)
3. Replace the original instance with the renamed instance with
   [`riak admin cluster force-replace <node1> <node2>`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/)
...
**Note:**
For more information on the `riak admin cluster` commands, refer to our documentation on [cluster administration]({{< baseurl >}}kv/3.4.1/reference/commands/).
...
Restoring a cluster from backups is documented [on its own page]({{< baseurl >}}kv/3.4.1/how-to/troubleshoot/recover-cluster-failure/#cluster-recovery-from-backups).
...
If a cluster uses only the leveled backend, a hot backup may be taken across the cluster using the `riak_client:hotbackup/4` function from [the `remote_console`]({{< baseurl >}}kv/3.4.1/how-to/operate/use-remote-console/) on any node within the cluster.
