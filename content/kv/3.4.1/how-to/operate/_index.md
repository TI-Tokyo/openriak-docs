...
Most configuration changes will be applied to the [configuration file]({{< baseurl >}}kv/3.4.1/reference/configuration/) located in your `rel/riak/etc` directory (if
...
cluster again.
>
> *Rename the node using the [`riak admin cluster replace`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/) command. This will not work if you have previously only started Riak with a single node.
...
you'll be using the default port (8087). If you're using the [Protocol Buffers interface]({{< baseurl >}}kv/3.4.1/reference/protocol-buffers/) to Riak (which we recommend over the HTTP
...
[`riak admin cluster force-replace`]({{< baseurl >}}kv/3.4.1/reference/commands/riak-admin/) the node.
...
can also find more detailed instructions about [adding and removing nodes]({{< baseurl >}}kv/3.4.1/tutorials/operations/change-cluster-membership/) from a cluster.
...
- [The considerations to make when choosing infrastructure for Riak]({{< baseurl >}}kv/3.4.1/how-to/plan/size-cluster/)
- [The practical steps to actually make and expand a cluster]({{< baseurl >}}kv/3.4.1/how-to/operate/add-node/)
