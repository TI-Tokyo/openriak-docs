...
The location of the `bin` directory will depend on whether `make rel` or `make devrel` has been used to create the Riak release.  By default `make rel` will copy the release into the `rel/riak` folder in the base folder to which Riak was cloned - so the control script can be found at `./rel/riak/bin/riak`. For clusters generated with `make devrel`, use the matching node script at `./dev/dev{n}/riak/bin/riak`.  Those nodes are independent until [they are joined into a cluster]({{< baseurl >}}kv/3.4.1/how-to/operate/add-node/).
