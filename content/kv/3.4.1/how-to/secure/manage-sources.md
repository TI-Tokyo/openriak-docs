...
be best to start with our general guide to [authentication and authorization]({{< baseurl >}}kv/3.4.1/how-to/secure/enable-security/).
...
[created]({{< baseurl >}}kv/3.4.1/how-to/secure/enable-security/#user-management) and that
security has been [enabled]({{< baseurl >}}kv/3.4.1/how-to/secure/enable-security/).
...
interface, and can be used only through the [protocol buffers interface]({{< baseurl >}}kv/3.4.1/reference/protocol-buffers/).
...
a different directory in your [`riak.conf`]({{< baseurl >}}kv/3.4.1/reference/configuration/) by either uncommenting those lines if you choose to use the defaults or setting the paths yourself:
...
find the node's name in [`riak.conf`]({{< baseurl >}}kv/3.4.1/reference/configuration/) under the parameter `nodename`. And so if the `nodename` for a cluster is
...
the nodes in your OpenRiak cluster, you need to perform a [rolling restart]({{< baseurl >}}kv/3.4.1/how-to/operate/rolling-restart/). Once that process is complete, you can use the client
...
[Managing group]: {{< baseurl >}}kv/3.4.1/how-to/secure/manage-sources/
