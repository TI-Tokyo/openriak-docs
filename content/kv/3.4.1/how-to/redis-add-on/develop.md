---
title: 'Extend the Redis add-on'
description: 'Show developers how to add and test a supported Redis add-on feature.'
weight: 2
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show developers how to add and test a supported Redis add-on feature.
...
### Developing with OpenRiak Redis Add-on

[redis-clients]: http://redis.io/clients
[usage bucket types]: {{< baseurl >}}kv/3.4.1/how-to/develop/use-bucket-types/
[dev api http]: {{< baseurl >}}kv/3.4.1/reference/http-api/
...
Riak Redis Add-on (RRA) packages a cache proxy service. The cache proxy service provides accessibility to OpenRiak KV, as a persistent data store, with Redis, as a cache through the various Redis client libraries and command-line interface tool `redis-cli`.

As with OpenRiak KV, the cache proxy service almost always performs best and most
...
For a deeper explanation of OpenRiak KV's configurable behaviors, see John Daily's
...
The following is an example, using OpenRiak KV's default HTTP port, of setting `allow_mult` to 'true' and `last_write_wins` to 'false':
...
OpenRiak KV organizes data into buckets, keys, and values, with
[bucket types][usage bucket types] acting as an additional namespace in OpenRiak KV
...
While buckets are a flat namespace in OpenRiak KV and you can name them
whatever you'd like (`bucket` or `a90bf521c` or `___`), within the cache proxy
service, Redis bucket_type:bucket:key is mapped to OpenRiak KV
...
OpenRiak KV [bucket types][usage bucket types] enable you to provide common
...
Reads via the cache proxy service are analogous to a Redis `GET`, with the added benefit of reading-through to OpenRiak KV which results in greater resilience through node outages and network partitions.

To request a value at a bucket/key in OpenRiak KV, issue the following:
...
consistency concern) may optionally be set within the nutcracker.conf. This will  result in an override of the setting value at the bucket-level in OpenRiak KV.
...
benefit of writing to OpenRiak KV followed by a `PEXPIRE` to Redis, invalidating
cache. As with HTTP PUT, `SET` semantically covers both create and update
operations.

To set a value at a bucket/key in OpenRiak KV, issue the following:
...
in an override of the setting value at the bucket-level in OpenRiak KV.
...
As noted in the section "Sibling Resolution" above, OpenRiak KV provides for a line of
...
If a client does not provide the causal context, OpenRiak KV makes no assumptions and treats the write as a new causal context, semantically equivalent to a
...
To delete a value at a bucket/key in OpenRiak KV, issue the following:
