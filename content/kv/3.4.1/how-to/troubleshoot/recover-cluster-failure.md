---
title: 'Recover from a cluster-wide failure'
description: 'Show practitioners how to recover from a cluster-wide failure from evidence gathering through verification.'
weight: 6
diataxis: 'how-to'
product: 'OpenRiak KV'
...
Show practitioners how to recover from a cluster-wide failure from evidence gathering through verification.
...
that the other nodes in the OpenRiak cluster are still operating normally and
...
returns to service (also called [hinted handoff](/kv/3.4.1/explanation/foundations/glossary/#hinted-handoff)).
...
Sometimes extreme latency spikes can be caused by [sibling explosion](/kv/3.4.1/how-to/develop/resolve-conflicts/#siblings). This condition occurs when the client application does not resolve conflicts properly or in a timely fashion. In that scenario, the size of the value on disk grows in proportion to
...
See [Changing Cluster Information](/kv/3.4.1/how-to/operate/change-cluster-information/#clusters-from-backups) for instructions on cluster recovery.
