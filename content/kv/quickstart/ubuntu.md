...
4. You want dont care about specific configurations or advanced features
...
5. We will listen on `127.0.0.1` on the default ports (`8098` for web and `8087` for Protocol Buffers).
...
3. Next we need to make some changes to the `riak.conf` file, so we will change to that directory with `cd /etc/riak/`

4. Open the `riak.conf` file using your preferred editor. In this case, we are using Nano, but you can use any.
