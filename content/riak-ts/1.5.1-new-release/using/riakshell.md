...
linkTitle: "riak shell"
weight: 309
---

[nodename]: {{<baseurl>}}riak/kv/2.2.0/using/cluster-operations/changing-cluster-info/
[creating]: {{<baseurl>}}openriak-ts/1.5.1/using/creating-activating
[writing]: {{<baseurl>}}openriak-ts/1.5.1/using/writingdata
...
1a\. You can attach to any node in your cluster from riak shell. To do this, locate your riak_shell.config file. On most systems, it will be in the `/etc/riak` directory with the other Riak TS configuration files. On Mac OS X, the configuration files are in the `~/riak-ts-1.5.1/etc` directory. Open riak_shell.config, and add the nodename and IP addresses you wish to connect to to nodes:
...
As of TS 1.5.1 (the first release to support comments), there is a
...
Logging : on
Date Log : off
Logfile : "/users/myusername/riakts/riak-ts-1.5.1/bin/../log/riak_shell/riak_shell"
