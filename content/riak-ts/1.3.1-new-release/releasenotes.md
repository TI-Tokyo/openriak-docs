...
We've also added AWS AMI support. You can find instructions for installing Riak TS on AWS [here]({{<baseurl>}}openriak-ts/1.3.0/installing/aws/).
...
* TS now supports MDC replication on TS tables. At this time, MDC support in TS does not include AAE fullsync. To use MDC, you will need to create your TS tables in your clusters and then [configure]({{<baseurl>}}openriak-ts/1.3.1/using/mdc) MDC.
...
*  AWS AMI is now available. You can find instructions for installing Riak TS on AWS [here]({{<baseurl>}}openriak-ts/1.3.0/installing/aws/). [[PR #89](https://github.com/basho/aws-ansible/pull/89)]
