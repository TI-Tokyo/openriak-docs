...
pagination_label: "Running a Cluster"
...
Most configuration changes will be applied to the [configuration file](./../../configuring/reference) located in your `rel/riak/etc` directory (if
...
cluster again.
>
> *Rename the node using the [`riak-admin cluster replace`](./../admin/riak-admin/#cluster-replace) command. This will not work if you have previously only started Riak with a single node.
...
you'll be using the default port (8087). If you're using the [Protocol Buffers interface](./../../developing/api/protocol-buffers) to Riak (which we recommend over the HTTP
...
<RiakDocsNote title="Note on upgrading to 2.0">
...
[`riak-admin cluster force-replace`](./../admin/riak-admin/#cluster-force-replace) the node.
...
<RiakDocsNote title="Note on single nodes">
...
can also find more detailed instructions about [adding and removing nodes](./../cluster-operations/adding-removing-nodes) from a cluster.
